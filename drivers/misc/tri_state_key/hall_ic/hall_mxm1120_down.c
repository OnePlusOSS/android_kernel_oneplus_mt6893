...
static int tri_key_m1120_i2c_drv_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	TRI_KEY_LOG("allocation memory for p_m1120_data down %s   =========ccy down\n", __func__);
	...
	TRI_KEY_LOG("init mutex variable  =========ccy down\n");
	/*(3) config i2c client*/
	...
	if (client->dev.of_node) {
		TRI_KEY_LOG("Use client->dev.of_node   =========ccy down\n");
	...
	if (gpio_is_valid(p_data->irq_gpio)) {
		...
		if (err) {
			...
			TRI_KEY_LOG("======> irq : %d  =========ccy down", p_data->irq);
		}
	}
	...
	TRI_KEY_LOG("%s was found =========ccy down", id->name);
