...
static int mtkts_flashlight_debug_log;
...
#define MTKTS_FLASHLIGHT_SW_FILTER (0)
#define MTKTS_FLASHLIGHT_TEMP_CRIT 60000	/* 60.000 degree Celsius */

#define mtkts_flashlight_dprintk(fmt, args...)   \
do {                                    \
	if (mtkts_flashlight_debug_log) {                \
...
pr_notice("[Thermal/TZ/FLASHLIGHT]" fmt, ##args)
...
/* AP_NTC_TSM_1 */
static struct FLASHLIGHT_TEMPERATURE FLASHLIGHT_Temperature_Table2[] = {
	{-40, 70603},		/* FIX_ME */
	{-35, 70603},		/* FIX_ME */
	{-30, 70603},		/* FIX_ME */
	{-25, 70603},		/* FIX_ME */
...
/* AP_NTC_10_SEN_1 */
static struct FLASHLIGHT_TEMPERATURE FLASHLIGHT_Temperature_Table3[] = {
	{-40, 74354},		/* FIX_ME */
	{-35, 74354},		/* FIX_ME */
	{-30, 74354},		/* FIX_ME */
	{-25, 74354},		/* FIX_ME */
...
#if 0
/* AP_NTC_10 */
static struct FLASHLIGHT_TEMPERATURE FLASHLIGHT_Temperature_Table4[] = {
	{-20, 68237},
	{-15, 53650},
	{-10, 42506},
	{-5, 33892},
...
/* AP_NTC_47 */
static struct FLASHLIGHT_TEMPERATURE FLASHLIGHT_Temperature_Table5[] = {
	{-40, 483954},		/* FIX_ME */
	{-35, 483954},		/* FIX_ME */
	{-30, 483954},		/* FIX_ME */
	{-25, 483954},		/* FIX_ME */
...
/* NTCG104EF104F(100K) */
static struct FLASHLIGHT_TEMPERATURE FLASHLIGHT_Temperature_Table6[] = {
	{-40, 4251000},
	{-35, 3005000},
	{-30, 2149000},
	{-25, 1554000},
...
static struct FLASHLIGHT_TEMPERATURE FLASHLIGHT_Temperature_Table7[] = {
	{-40, 4397119},
	{-35, 3088599},
	{-30, 2197225},
	{-25, 1581881},
...
static __s32 mtkts_flashlight_thermistor_conver_temp(__s32 Res)
{
	int i = 0;
	int asize = 0;
	__s32 RES1 = 0, RES2 = 0;
...
#endif
	asize = (ntc_tbl_size / sizeof(struct FLASHLIGHT_TEMPERATURE));

	/* mtkts_flashlight_dprintk("mtkts_flashlight_thermistor_conver_temp() :
	 * asize = %d, Res = %d\n",asize,Res);
	 */
	if (Res >= FLASHLIGHT_Temperature_Table[0].TemperatureR) {
		...
		for (i = 0; i < asize; i++) {
			...
			/* mtkts_flashlight_dprintk("%d :i=%d, RES1 = %d,TMP1 = %d\n",
	...
	mtkts_flashlight_dprintk("%s() : Res = %d\n", __func__,
									Res);

	mtkts_flashlight_dprintk("%s() : RES1 = %d\n", __func__
									RES1);

	mtkts_flashlight_dprintk("%s() : RES2 = %d\n", __func__,
									RES2);

	mtkts_flashlight_dprintk("%s() : TMP1 = %d\n", __func__,
									TMP1);

	mtkts_flashlight_dprintk("%s() : TMP2 = %d\n", __func__,
									TMP2);
#endif
...
static __s32 mtk_ts_flashlight_volt_to_temp(__u32 dwVolt)
{
	__s32 TRes;
	__u64 dwVCriAP = 0;
...
static int get_hw_flashlight_temp(void)
{
	...
	mtkts_flashlight_dprintk("get_hw_flashlight_temp val=%d\n", val);

	if (ret < 0) {
		mtkts_flashlight_printk("IIO channel read failed %d\n", ret);
		return ret;
	}
...
#define DELTA_TEMP 2000
	...
	if ((t_ret > 100000) || (t_ret < -30000)) {
		mtkts_flashlight_printk(
			"[Power/FLASHLIGHT_Thermal] drop this data\n");
		t_ret = pre_temp1;
	} else if ((pre_temp1 != 0)
		&& (((pre_temp1 - t_ret) >= DELTA_TEMP)
			|| ((t_ret - pre_temp1) >= DELTA_TEMP))) {
		mtkts_flashlight_dprintk(
			"[Power/FLASHLIGHT_Thermal] t_ret=%d,(t_ret-pre_temp1)=%d,(pre_temp1-t_ret)=%d\n",
			t_ret, (t_ret - pre_temp1), (pre_temp1 - t_ret));
		t_ret = pre_temp1;
	} else {
		/* update previous temp */
		pre_temp1 = t_ret;
		mtkts_flashlight_dprintk("[Power/FLASHLIGHT_Thermal] pre_temp1=%d\n",
								pre_temp1);
	}
#endif
...
static int mtkts_flashlight_get_temp(struct thermal_zone_device *thermal, int *t)
{
...
static int mtkts_flashlight_bind(struct thermal_zone_device *thermal,
...
static int mtkts_flashlight_unbind(struct thermal_zone_device *thermal,
				struct thermal_cooling_device *cdev)
{
	int table_val = 0;
...
static int mtkts_flashlight_get_mode(struct thermal_zone_device *thermal,
...
static int mtkts_flashlight_set_mode(struct thermal_zone_device *thermal,
				  enum thermal_device_mode mode)
{
	kernelmode = mode;
	return 0;
...
static int mtkts_flashlight_get_trip_type(
...
static int mtkts_flashlight_get_trip_temp(
...
static int mtkts_flashlight_get_crit_temp(
struct thermal_zone_device *thermal, int *temperature)
{
	*temperature = MTKTS_FLASHLIGHT_TEMP_CRIT;
	return 0;
}

/* bind callback functions to thermalzone */
static struct thermal_zone_device_ops mtkts_flashlight_dev_ops = {
	.bind = mtkts_flashlight_bind,
	.unbind = mtkts_flashlight_unbind,
	.get_temp = mtkts_flashlight_get_temp,
	.get_mode = mtkts_flashlight_get_mode,
	.set_mode = mtkts_flashlight_set_mode,
	.get_trip_type = mtkts_flashlight_get_trip_type,
	.get_trip_temp = mtkts_flashlight_get_trip_temp,
	.get_crit_temp = mtkts_flashlight_get_crit_temp,
};



static int mtkts_flashlight_read(struct seq_file *m, void *v)
{
...
static int mtkts_flashlight_register_thermal(void);
static void mtkts_flashlight_unregister_thermal(void);

static ssize_t mtkts_flashlight_write(
struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
	int len = 0;
	int i;
	struct mtktsflashlight_data {
		int trip[10];
		int t_type[10];
	...
	struct mtktsflashlight_data *ptr_mtktsflashlight_data = kmalloc(
					sizeof(*ptr_mtktsflashlight_data), GFP_KERNEL);

	if (ptr_mtktsflashlight_data == NULL)
		return -ENOMEM;


	len = (count < (sizeof(ptr_mtktsflashlight_data->desc) - 1)) ?
				count : (sizeof(ptr_mtktsflashlight_data->desc) - 1);

	if (copy_from_user(ptr_mtktsflashlight_data->desc, buffer, len)) {
		kfree(ptr_mtktsflashlight_data);
		return 0;
	}

	ptr_mtktsflashlight_data->desc[len] = '\0';

	if (sscanf
	    (ptr_mtktsflashlight_data->desc,
	     "%d %d %d %19s %d %d %19s %d %d %19s %d %d %19s %d %d %19s %d %d %19s %d %d %19s %d %d %19s %d %d %19s %d %d %19s %d",
		&num_trip,
		&ptr_mtktsflashlight_data->trip[0], &ptr_mtktsflashlight_data->t_type[0],
		ptr_mtktsflashlight_data->bind0,
		&ptr_mtktsflashlight_data->trip[1], &ptr_mtktsflashlight_data->t_type[1],
		ptr_mtktsflashlight_data->bind1,
		&ptr_mtktsflashlight_data->trip[2], &ptr_mtktsflashlight_data->t_type[2],
		ptr_mtktsflashlight_data->bind2,
		&ptr_mtktsflashlight_data->trip[3], &ptr_mtktsflashlight_data->t_type[3],
		ptr_mtktsflashlight_data->bind3,
		&ptr_mtktsflashlight_data->trip[4], &ptr_mtktsflashlight_data->t_type[4],
		ptr_mtktsflashlight_data->bind4,
		&ptr_mtktsflashlight_data->trip[5], &ptr_mtktsflashlight_data->t_type[5],
		ptr_mtktsflashlight_data->bind5,
		&ptr_mtktsflashlight_data->trip[6], &ptr_mtktsflashlight_data->t_type[6],
		ptr_mtktsflashlight_data->bind6,
		&ptr_mtktsflashlight_data->trip[7], &ptr_mtktsflashlight_data->t_type[7],
		ptr_mtktsflashlight_data->bind7,
		&ptr_mtktsflashlight_data->trip[8], &ptr_mtktsflashlight_data->t_type[8],
		ptr_mtktsflashlight_data->bind8,
		&ptr_mtktsflashlight_data->trip[9], &ptr_mtktsflashlight_data->t_type[9],
		ptr_mtktsflashlight_data->bind9,
		&ptr_mtktsflashlight_data->time_msec) == 32) {

		down(&sem_mutex);
		mtkts_flashlight_dprintk(
			"[%s] mtkts_flashlight_unregister_thermal\n", __func__);

		mtkts_flashlight_unregister_thermal();

		if (num_trip < 0 || num_trip > 10) {
			#ifdef CONFIG_MTK_AEE_FEATURE
			aee_kernel_warning_api(__FILE__, __LINE__,
					DB_OPT_DEFAULT, "mtkts_flashlight_write",
					"Bad argument");
			#endif
			mtkts_flashlight_dprintk("[%s] bad argument\n", __func__);
			kfree(ptr_mtktsflashlight_data);
			up(&sem_mutex);
			return -EINVAL;
		}
		...
		for (i = 0; i < num_trip; i++)
			g_THERMAL_TRIP[i] = ptr_mtktsflashlight_data->t_type[i];
		...
		for (i = 0; i < 20; i++) {
			g_bind0[i] = ptr_mtktsflashlight_data->bind0[i];
			g_bind1[i] = ptr_mtktsflashlight_data->bind1[i];
			g_bind2[i] = ptr_mtktsflashlight_data->bind2[i];
			g_bind3[i] = ptr_mtktsflashlight_data->bind3[i];
			g_bind4[i] = ptr_mtktsflashlight_data->bind4[i];
			g_bind5[i] = ptr_mtktsflashlight_data->bind5[i];
			g_bind6[i] = ptr_mtktsflashlight_data->bind6[i];
			g_bind7[i] = ptr_mtktsflashlight_data->bind7[i];
			g_bind8[i] = ptr_mtktsflashlight_data->bind8[i];
			g_bind9[i] = ptr_mtktsflashlight_data->bind9[i];
		}
		...
		for (i = 0; i < num_trip; i++)
			trip_temp[i] = ptr_mtktsflashlight_data->trip[i];

		interval = ptr_mtktsflashlight_data->time_msec / 1000;
		...
		mtkts_flashlight_dprintk(
			"[%s] mtkts_flashlight_register_thermal\n", __func__);
		mtkts_flashlight_register_thermal();
		up(&sem_mutex);
	...
	aee_kernel_warning_api(__FILE__, __LINE__, DB_OPT_DEFAULT,
					"mtkts_flashlight_write", "Bad argument");
	#endif
	kfree(ptr_mtktsflashlight_data);
	return -EINVAL;
}

void mtkts_flashlight_prepare_table(int table_num)
{

	switch (table_num) {
	case 1:		/* AP_NTC_BL197 */
		FLASHLIGHT_Temperature_Table = FLASHLIGHT_Temperature_Table1;
		ntc_tbl_size = sizeof(FLASHLIGHT_Temperature_Table1);
		break;
	case 2:		/* AP_NTC_TSM_1 */
		FLASHLIGHT_Temperature_Table = FLASHLIGHT_Temperature_Table2;
		ntc_tbl_size = sizeof(FLASHLIGHT_Temperature_Table2);
		break;
	case 3:		/* AP_NTC_10_SEN_1 */
		FLASHLIGHT_Temperature_Table = FLASHLIGHT_Temperature_Table3;
		ntc_tbl_size = sizeof(FLASHLIGHT_Temperature_Table3);
		break;
	case 4:		/* AP_NTC_10 */
		FLASHLIGHT_Temperature_Table = FLASHLIGHT_Temperature_Table4;
		ntc_tbl_size = sizeof(FLASHLIGHT_Temperature_Table4);
		break;
	case 5:		/* AP_NTC_47 */
		FLASHLIGHT_Temperature_Table = FLASHLIGHT_Temperature_Table5;
		ntc_tbl_size = sizeof(FLASHLIGHT_Temperature_Table5);
		break;
	case 6:		/* NTCG104EF104F */
		FLASHLIGHT_Temperature_Table = FLASHLIGHT_Temperature_Table6;
		ntc_tbl_size = sizeof(FLASHLIGHT_Temperature_Table6);
		break;
	case 7:		/* NCP15WF104F03RC */
		FLASHLIGHT_Temperature_Table = FLASHLIGHT_Temperature_Table7;
		ntc_tbl_size = sizeof(FLASHLIGHT_Temperature_Table7);
		break;
	default:		/* AP_NTC_10 */
		FLASHLIGHT_Temperature_Table = FLASHLIGHT_Temperature_Table4;
		ntc_tbl_size = sizeof(FLASHLIGHT_Temperature_Table4);
...
struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
	...
	if (sscanf
		...
		if (!strcmp(ptr_param_data->NTC_TABLE, "NTC_TABLE")) {
			...
			mtkts_flashlight_dprintk("g_RAP_ntc_table=%d\n",
							g_RAP_ntc_table);
		} else {
			kfree(ptr_param_data);
			mtkts_flashlight_printk(
		...
		mtkts_flashlight_prepare_table(g_RAP_ntc_table);

		kfree(ptr_param_data);
		return count;
	}

	mtkts_flashlight_printk("[mtkts_flashlight_write] bad argument\n");
	kfree(ptr_param_data);
	return -EINVAL;
}

/* int  mtkts_flashlight_register_cooler(void)
 * {
 *	cooling devices
 *	cl_dev_sysrst = mtk_thermal_cooling_device_register(
 *				"mtflashlightdpatery-sysrst", NULL,
 *				&mtkts_flashlight_cooling_sysrst_ops);
 *	return 0;
 * }
 */
...
{
	/* cancel timer
	 * mtkts_flashlight_printk("mtkts_flashlight_cancel_thermal_timer\n");
	 *
...
{
	/* mtkts_flashlight_printk("mtkts_flashlight_start_thermal_timer\n");
...
static int mtkts_flashlight_register_thermal(void)
{
	...
	/* trips : trip 0~1 */
	thz_dev = mtk_thermal_zone_device_register("mtktsflashlight", num_trip,
					NULL, &mtkts_flashlight_dev_ops, 0, 0, 0,
					interval * 1000);

	return 0;
}

/* void mtkts_flashlight_unregister_cooler(void) */
/* { */
	/* if (cl_dev_sysrst) { */
...
static int mtkts_flashlight_open(struct inode *inode, struct file *file)
{
	return single_open(file, mtkts_flashlight_read, NULL);
}

static const struct file_operations mtkts_flashlight_fops = {
	.owner = THIS_MODULE,
	.open = mtkts_flashlight_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.write = mtkts_flashlight_write,
...
static int mtkts_flashlight_probe(struct platform_device *pdev)
{
	...
	if (!pdev->dev.of_node) {
		mtkts_flashlight_printk("[%s]Only DT based supported\n",
		__func__);
		return -ENODEV;
	}
	...
	mtkts_flashlight_printk("[%s]get auxadc iio ch: %d\n", __func__,
...
static int __init mtkts_flashlight_init(void)
{
	...
	if (err) {
		mtkts_flashlight_printk("thermal driver callback register failed.\n");
		return err;
	}
#endif
	/* setup default table */
	...
	mtkts_flashlight_register_thermal();
#if 0
	mtkTTimer_register("mtktsflashlight", mtkts_flashlight_start_thermal_timer,
					mtkts_flashlight_cancel_thermal_timer);
#endif
	return 0;
}

static void __exit mtkts_flashlight_exit(void)
{
	mtkts_flashlight_dprintk("[%s]\n", __func__);
	mtkts_flashlight_unregister_thermal();
#if 0
	mtkTTimer_unregister("mtktsflashlight");
#endif
	/* mtkts_flashlight_unregister_cooler(); */
}

module_init(mtkts_flashlight_init);
module_exit(mtkts_flashlight_exit);
