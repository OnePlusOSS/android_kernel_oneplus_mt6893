...
#include "gc02m0mipimono2_Sensor.h"
...
#define PFX "GC02M0_camera_sensor_mono2"
...
static imgsensor_info_struct imgsensor_info = {
       .sensor_id = GC02M0_SENSOR_ID2,        //record sensor id defined in Kd_imgsensor.h
        ...
        do {
            *sensor_id = return_sensor_id()+2;
            if (*sensor_id == imgsensor_info.sensor_id) {
                ...
                Oplusimgsensor_Registdeviceinfo(gImgEepromInfo.pCamModuleInfo[5].name,
                                                gImgEepromInfo.pCamModuleInfo[5].version,
...
static kal_uint32 open(void)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            sensor_id = return_sensor_id()+2;
            if (sensor_id == imgsensor_info.sensor_id) {
                LOG_INF("i2c mono2 write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
                break;
            }
            LOG_INF("Read mono2 sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        if (sensor_id == imgsensor_info.sensor_id)
