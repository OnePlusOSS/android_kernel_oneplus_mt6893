...
static kal_uint8 gGc02m0b_SN[CAMERA_MODULE_SN_LENGTH];
static kal_uint8 gGc02m0b_CamInfo_CamInfo[CAMERA_MODULE_INFO_LENGTH];
static void read_eeprom_CamInfo(void)
{
	...
	gGc02m0b_CamInfo_CamInfo[0] = get_byte[0];
	gGc02m0b_CamInfo_CamInfo[1] = get_byte[1];
	gGc02m0b_CamInfo_CamInfo[2] = get_byte[6];
	gGc02m0b_CamInfo_CamInfo[3] = get_byte[7];
	gGc02m0b_CamInfo_CamInfo[4] = get_byte[8];
	gGc02m0b_CamInfo_CamInfo[5] = get_byte[9];
	gGc02m0b_CamInfo_CamInfo[6] = get_byte[10];
	gGc02m0b_CamInfo_CamInfo[7] = get_byte[11];
}

static void read_eeprom_SN(void)
{
	kal_uint16 idx = 0;
	kal_uint8 *get_byte= &gGc02m0b_SN[0];
	for (idx = 0; idx <CAMERA_MODULE_SN_LENGTH; idx++) {
		char pusendcmd[2] = {0x00 , (char)((0x40 + idx) & 0xFF) };
		iReadRegI2C(pusendcmd , 2, (u8*)&get_byte[idx],1, 0xA8);
		LOG_INF("gc02m0b_SN[%d]: 0x%x  0x%x\n", idx, get_byte[idx], gGc02m0b_SN[idx]);
	}
}
...
static kal_int32 write_Module_data(ACDK_SENSOR_ENGMODE_STEREO_STRUCT * pStereodata)
{
	...
	UINT32 i = 0;
	if(pStereodata != NULL) {
		pr_debug("gc02m0b SET_SENSOR_OTP: 0x%x %d 0x%x %d\n",
		...
		if ((pStereodata->uSensorId == GC02M0_SENSOR_ID) && (data_length == DUALCAM_CALI_DATA_LENGTH)
			&& (data_base == GC02M0B_STEREO_START_ADDR)) {
			pr_debug("gc02m0b Write: %x %x %x %x %x %x %x %x\n", pData[0], pData[39], pData[40], pData[1556],
			...
			write_eeprom_protect(1);
			msleep(6);
			pr_debug("com_0:0x%x\n", read_cmos_eeprom_8(GC02M0B_STEREO_START_ADDR));
			pr_debug("com_39:0x%x\n", read_cmos_eeprom_8(GC02M0B_STEREO_START_ADDR+39));
			pr_debug("innal_40:0x%x\n", read_cmos_eeprom_8(GC02M0B_STEREO_START_ADDR+40));
			pr_debug("innal_1556:0x%x\n", read_cmos_eeprom_8(GC02M0B_STEREO_START_ADDR+1556));
			pr_debug("tail1_1557:0x%x\n", read_cmos_eeprom_8(GC02M0B_STEREO_START_ADDR+1557));
			pr_debug("tail2_1558:0x%x\n", read_cmos_eeprom_8(GC02M0B_STEREO_START_ADDR+1558));
			pr_debug("tail3_1559:0x%x\n", read_cmos_eeprom_8(GC02M0B_STEREO_START_ADDR+1559));
			pr_debug("tail4_1560:0x%x\n", read_cmos_eeprom_8(GC02M0B_STEREO_START_ADDR+1560));
			pr_debug("gc02m0bwrite_Module_data Write end\n");
		} else {
			pr_err("Invalid Sensor id:0x%x write_gm1 eeprom\n", pStereodata->uSensorId);
			return IMGSENSOR_RETURN_ERROR;
		}
	} else {
		pr_err("gc02m0b write_Module_data pStereodata is null\n");
		return IMGSENSOR_RETURN_ERROR;
	}
	return ret;
}
                             ...
                             UINT8 *feature_para,UINT32 *feature_para_len)
{
    ...
    switch (feature_id) {
        case SENSOR_FEATURE_GET_MODULE_INFO:
            LOG_INF("gc02m0b GET_MODULE_CamInfo:%d %d\n", *feature_para_len, *feature_data_32);
            *(feature_data_32 + 1) = (gGc02m0b_CamInfo_CamInfo[1] << 24)
                        | (gGc02m0b_CamInfo_CamInfo[0] << 16)
                        | (gGc02m0b_CamInfo_CamInfo[3] << 8)
                        | (gGc02m0b_CamInfo_CamInfo[2] & 0xFF);
            *(feature_data_32 + 2) = (gGc02m0b_CamInfo_CamInfo[5] << 24)
                        | (gGc02m0b_CamInfo_CamInfo[4] << 16)
                        | (gGc02m0b_CamInfo_CamInfo[7] << 8)
                        | (gGc02m0b_CamInfo_CamInfo[6] & 0xFF);
            break;
        case SENSOR_FEATURE_GET_MODULE_SN:
            LOG_INF("gc02m0b GET_MODULE_SN:%d %d\n", *feature_para_len, *feature_data_32);
            if (*feature_data_32 < CAMERA_MODULE_SN_LENGTH/4)
                *(feature_data_32 + 1) = (gGc02m0b_SN[4*(*feature_data_32) + 3] << 24)
                            | (gGc02m0b_SN[4*(*feature_data_32) + 2] << 16)
                            | (gGc02m0b_SN[4*(*feature_data_32) + 1] << 8)
                            | (gGc02m0b_SN[4*(*feature_data_32)] & 0xFF);
            break;
