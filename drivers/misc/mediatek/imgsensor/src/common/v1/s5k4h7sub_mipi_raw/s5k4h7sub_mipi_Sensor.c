...
{3264, 2448, 0, 0, 3264, 2448, 1632,
	1224, 0000, 0000, 1632, 1224, 0, 0, 1632, 1224},	/* Preview */
{3264, 2448, 0, 0, 3264, 2448, 3264,
	2448, 0000, 0000, 3264, 2448, 0, 0, 3264, 2448},	/* capture */
{3264, 2448, 0, 0, 3264, 2448, 3264,
	2448, 0000, 0000, 3264, 2448, 0, 0, 3264, 2448},	/* video */
{3264, 2448, 352, 264, 2560, 1920,
	640, 480, 0000, 0000, 640, 480, 0, 0, 640,  480},	/* hight speed video */
{3264, 2448, 352, 504, 2560, 1440,
	1280, 720, 0000, 0000, 1280, 720, 0, 0, 1280, 720},	/* slim video */
	...
	if (flag1 != 0) {
		...
		for (page_start = 1, page_end = 6; page_start <= page_end; page_start++) {
			...
			if (page_start == 6) {
				...
				/* LOG_INF("flag =%d\n", flag); */
				if (flag) {
					sum_slc += sum_awb_flag_lsc(
						0x0A04, 0x0A2A, data_p[page_start-1]);
					continue;
				}
			}
		...
		if (flag2 != 0) {
			...
			for (page_start = 6, page_end = 12; page_start <= page_end; page_start++) {
				...
				if (page_start == 12) {
					...
					if (flag) {
						sum_slc += sum_awb_flag_lsc(
							0x0A04, 0x0A12, data_p[page_start-1]);
					continue;
					}
				}
				if (page_start == 6) {
				...
				sum_slc += sum_awb_flag_lsc(
							0x0A04, 0X0A43, data_p[page_start-1]);
			}
		}
		if (!bresult)
...
{
	memcpy(data, g_sub_reg_data, 40*sizeof(kal_uint8));
	return 5376;
}
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	pr_debug("zrl1 M10 MAIN S5K4H7 get imgsensor id\n");
			  ...
			  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	if (imgsensor.current_fps == imgsensor_info.cap.max_framerate) {
		...
		imgsensor.pclk = imgsensor_info.cap1.pclk;/* PIP capture:15fps */
