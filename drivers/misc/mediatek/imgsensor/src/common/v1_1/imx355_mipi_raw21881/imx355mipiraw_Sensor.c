...
static struct imgsensor_info_struct imgsensor_info = {
    .sensor_id = IMX355_SENSOR_ID_21881 - SENSOR_ID_OFFSET_21881,
...
static void read_module_data(void)
{
    ...
    gImgEepromInfo_21881.camNormdata[2][0] = Eeprom_1ByteDataRead(0x00, 0xA2);
    gImgEepromInfo_21881.camNormdata[2][1] = Eeprom_1ByteDataRead(0x01, 0xA2);
    gImgEepromInfo_21881.camNormdata[2][48] = 0xFF;
    gImgEepromInfo_21881.camNormdata[2][49] = 0xFF;
    imgsensor_info.module_id = Eeprom_1ByteDataRead(0x00, 0xA2);
    Oplusimgsensor_Registdeviceinfo(gImgEepromInfo_21881.pCamModuleInfo[2].name,
                                    gImgEepromInfo_21881.pCamModuleInfo[2].version,
                                    imgsensor_info.module_id);
    for(i = 2; i < 8; i++) {
        gImgEepromInfo_21881.camNormdata[2][i] = Eeprom_1ByteDataRead(0x04+i, 0xA2);
        gImgEepromInfo_21881.camNormdata[2][i+48] = 0xFF;
    }

    for (i = 0; i < OPLUS_CAMERASN_LENS; i ++) {
        gImgEepromInfo_21881.camNormdata[2][8+i] = Eeprom_1ByteDataRead(0xB0+i, 0xA2);
    }
    ...
    memcpy(&gImgEepromInfo_21881.camNormdata[2][40], &vcmID, 4);
    memcpy(&gImgEepromInfo_21881.camNormdata[2][44], &lensID, 4);
    gImgEepromInfo_21881.camNormdata[2][39] = 2;
    /*Read stereo eeprom data*/
    for (i = 0; i < CALI_DATA_SLAVE_LENGTH; i ++) {
        gImgEepromInfo_21881.stereoMWdata[CALI_DATA_MASTER_LENGTH+i] =
                    Eeprom_1ByteDataRead(IMX355_STEREO_START_ADDR+i, 0xA2);
    }
    gImgEepromInfo_21881.i4CurSensorIdx = 2;
    gImgEepromInfo_21881.i4CurSensorId = IMX355_SENSOR_ID_21881;
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            *sensor_id = ((read_cmos_sensor_8(0x0016) << 8)
            ...
            LOG_INF("Read sensor id fail, id: 0x%x read_sensor_id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
            retry--;
        } while (retry > 0);
        i++;
        retry = 2;
