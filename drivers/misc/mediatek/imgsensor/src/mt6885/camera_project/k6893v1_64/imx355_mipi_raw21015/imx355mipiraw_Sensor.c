...
static struct imgsensor_info_struct imgsensor_info = {
    .sensor_id = IMX355_SENSOR_ID_21015 - SENSOR_ID_OFFSET_21015,
...
static void read_module_data(void)
{
    ...
    gImgEepromInfo.camNormdata[IMGSENSOR_SENSOR_IDX_MAIN2][0] = Eeprom_1ByteDataRead(0x00, IMX355_EEPROM_SLAVE_ADDRESS);
    gImgEepromInfo.camNormdata[IMGSENSOR_SENSOR_IDX_MAIN2][1] = Eeprom_1ByteDataRead(0x01, IMX355_EEPROM_SLAVE_ADDRESS);
    gImgEepromInfo.camNormdata[IMGSENSOR_SENSOR_IDX_MAIN2][48] = 0xFF;
    gImgEepromInfo.camNormdata[IMGSENSOR_SENSOR_IDX_MAIN2][49] = 0xFF;
    imgsensor_info.module_id = Eeprom_1ByteDataRead(0x00, IMX355_EEPROM_SLAVE_ADDRESS);
    ...
    for(i = 2; i < 8; i++) {
       gImgEepromInfo.camNormdata[IMGSENSOR_SENSOR_IDX_MAIN2][i] = Eeprom_1ByteDataRead(0x04+i, IMX355_EEPROM_SLAVE_ADDRESS);
       gImgEepromInfo.camNormdata[IMGSENSOR_SENSOR_IDX_MAIN2][i+48] = 0xFF;
    }

    /*Read QR Code from eeprom: 8~27*/
    for (i = 0; i < OPLUS_CAMERASN_LENS; i ++) {
       gImgEepromInfo.camNormdata[IMGSENSOR_SENSOR_IDX_MAIN2][8+i] = Eeprom_1ByteDataRead(0xB0+i, IMX355_EEPROM_SLAVE_ADDRESS);
    }
    vcmID = (Eeprom_1ByteDataRead(0xb, IMX355_EEPROM_SLAVE_ADDRESS) << 8) | Eeprom_1ByteDataRead(0xa, IMX355_EEPROM_SLAVE_ADDRESS);
    lensID = (Eeprom_1ByteDataRead(0x9, IMX355_EEPROM_SLAVE_ADDRESS) << 8) | Eeprom_1ByteDataRead(0x8, IMX355_EEPROM_SLAVE_ADDRESS);
    memcpy(&gImgEepromInfo.camNormdata[IMGSENSOR_SENSOR_IDX_MAIN2][40], &vcmID, 4);
    memcpy(&gImgEepromInfo.camNormdata[IMGSENSOR_SENSOR_IDX_MAIN2][44], &lensID, 4);
    ...
    for (i = 0; i < CALI_DATA_SLAVE_LENGTH; i ++) {
        gImgEepromInfo.stereoMWdata[CALI_DATA_MASTER_LENGTH+i] =
                    Eeprom_1ByteDataRead(IMX355_STEREO_START_ADDR+i, IMX355_EEPROM_SLAVE_ADDRESS);
    }
    gImgEepromInfo.i4CurSensorIdx = IMGSENSOR_SENSOR_IDX_MAIN2;
    gImgEepromInfo.i4CurSensorId = imgsensor_info.sensor_id;
    Eeprom_DistortionParamsRead(IMGSENSOR_SENSOR_IDX_MAIN2 ,IMX355_EEPROM_SLAVE_ADDRESS);
}
