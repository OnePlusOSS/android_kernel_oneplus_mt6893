...
static void read_module_data(void)
{
    ...
    gImgEepromInfo.camNormdata[3][48] = 0xFF;
    gImgEepromInfo.camNormdata[3][49] = 0xFF;
    ...
    for(i = 2; i < 8; i++) {
       ...
       gImgEepromInfo.camNormdata[3][i+48] = 0xFF;
    }

    for (i = 0; i < OPLUS_CAMERASN_LENS; i ++) {
       gImgEepromInfo.camNormdata[3][8+i] = Eeprom_1ByteDataRead(0xB0+i, 0xA4);
    }
    vcmID = Eeprom_1ByteDataRead(0x7, 0xA4);
    lensID = Eeprom_1ByteDataRead(0x6, 0xA4);
    memcpy(&gImgEepromInfo.camNormdata[3][40], &vcmID, 4);
    memcpy(&gImgEepromInfo.camNormdata[4][44], &lensID, 4);
