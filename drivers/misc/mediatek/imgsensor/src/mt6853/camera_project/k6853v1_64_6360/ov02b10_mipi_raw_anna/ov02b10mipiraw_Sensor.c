...
static  imgsensor_info_struct imgsensor_info = {
    ...
    .sensor_id = OV02B10_SENSOR_ID_ANNA,
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            ...
            *sensor_id = ((read_cmos_sensor(0x0200) << 8) | read_cmos_sensor(0x0300));
            ...
            LOG_INF("Read sensor id fail,write_id:0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
            retry--;
        } while (retry > 0);
        i++;
        retry = 2;
...
//    if(gImgEepromInfo.camNormdata[3][2] != 0xca) {
//        LOG_INF("open sensor error\n");
...
//    }
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            ...
            LOG_INF("Read sensor id fail, write: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id, sensor_id);
            retry--;
        } while (retry > 0);
        i++;
        if (sensor_id == imgsensor_info.sensor_id) {
