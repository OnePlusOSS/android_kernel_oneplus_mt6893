...
#include "gc02m1bmipimono1_Sensor.h"
...
static bool project_20041 = false ;
#endif

static DEFINE_SPINLOCK(imgsensor_drv_lock);

static imgsensor_info_struct imgsensor_info = {
       .sensor_id = GC02M1B_SENSOR_ID1,        //record sensor id defined in Kd_imgsensor.h
       ...
       .hs_video = {
        ...
        .custom1 = {
             ...
             .framelength = 1290,            //record different mode's framelength
             .startx = 0,                    //record different mode's startx of grabwindow
             .starty = 0,                    //record different mode's starty of grabwindow
             .grabwindow_width = 1600,        //record different mode's width of grabwindow //1296
             .grabwindow_height = 1200,        //record different mode's height of grabwindow //972
...
static void preview_setting(void)
{
	...
	write_cmos_sensor(0x9d,0x18);//add by reece
	write_cmos_sensor(0xfe,0x00);
...
{
    LOG_INF("E!\n");
    write_cmos_sensor(0x41,0x05);
    write_cmos_sensor(0x42,0x0a);
    write_cmos_sensor(0x05,0x05);
    write_cmos_sensor(0x06,0x4c);
    write_cmos_sensor(0x07,0x00);
    write_cmos_sensor(0x08,0x2e);
    write_cmos_sensor(0x9d,0x2e);//add by reece
    write_cmos_sensor(0xfe,0x00);
    write_cmos_sensor(0x3e,0x90);
}    /*    preview_setting  */
...
#ifdef OPLUS_FEATURE_CAMERA_COMMON
static kal_uint8 gGc02m1b_SN[CAMERA_MODULE_SN_LENGTH];
static kal_uint8 gGc02m1b_CamInfo_CamInfo[CAMERA_MODULE_INFO_LENGTH];
static void read_eeprom_CamInfo(void)
{
	...
	for (idx = 0; idx <12; idx++) {
		...
		LOG_INF("gc02m0b_info[%d]: 0x%x\n", idx, get_byte[idx]);
	}

	gGc02m1b_CamInfo_CamInfo[0] = get_byte[0];
	gGc02m1b_CamInfo_CamInfo[1] = get_byte[1];
	gGc02m1b_CamInfo_CamInfo[2] = get_byte[6];
	gGc02m1b_CamInfo_CamInfo[3] = get_byte[7];
	gGc02m1b_CamInfo_CamInfo[4] = get_byte[8];
	gGc02m1b_CamInfo_CamInfo[5] = get_byte[9];
	gGc02m1b_CamInfo_CamInfo[6] = get_byte[10];
	gGc02m1b_CamInfo_CamInfo[7] = get_byte[11];
}

static void read_eeprom_SN(void)
{
	kal_uint16 idx = 0;
	kal_uint8 *get_byte= &gGc02m1b_SN[0];
	for (idx = 0; idx <CAMERA_MODULE_SN_LENGTH; idx++) {
		char pusendcmd[2] = {0x00 , (char)((0x0c + idx) & 0xFF) };
		iReadRegI2C(pusendcmd , 2, (u8*)&get_byte[idx],1, 0xA2);
		LOG_INF("gc02m1b_SN[%d]: 0x%x  0x%x\n", idx, get_byte[idx], gGc02m1b_SN[idx]);
	}
}
...
static kal_int32 write_Module_data(ACDK_SENSOR_ENGMODE_STEREO_STRUCT * pStereodata)
{
	...
	UINT32 i = 0;
	if(pStereodata != NULL) {
		pr_debug("gc02m1b SET_SENSOR_OTP: 0x%x %d 0x%x %d\n",
		...
		if ((pStereodata->uSensorId == GC02M1B_SENSOR_ID1) && (data_length == DUALCAM_CALI_DATA_LENGTH)
			&& (data_base == GC02M1B_STEREO_START_ADDR)) {
			pr_debug("gc02m1b Write: %x %x %x %x %x %x %x %x\n", pData[0], pData[39], pData[40], pData[1556],
			...
			write_eeprom_protect(1);
			msleep(6);
			pr_debug("com_0:0x%x\n", read_cmos_eeprom_8(GC02M1B_STEREO_START_ADDR));
			pr_debug("com_39:0x%x\n", read_cmos_eeprom_8(GC02M1B_STEREO_START_ADDR+39));
			pr_debug("innal_40:0x%x\n", read_cmos_eeprom_8(GC02M1B_STEREO_START_ADDR+40));
			pr_debug("innal_1556:0x%x\n", read_cmos_eeprom_8(GC02M1B_STEREO_START_ADDR+1556));
			pr_debug("tail1_1557:0x%x\n", read_cmos_eeprom_8(GC02M1B_STEREO_START_ADDR+1557));
			pr_debug("tail2_1558:0x%x\n", read_cmos_eeprom_8(GC02M1B_STEREO_START_ADDR+1558));
			pr_debug("tail3_1559:0x%x\n", read_cmos_eeprom_8(GC02M1B_STEREO_START_ADDR+1559));
			pr_debug("tail4_1560:0x%x\n", read_cmos_eeprom_8(GC02M1B_STEREO_START_ADDR+1560));
			pr_debug("gc02m1b write_Module_data Write end\n");
		} else {
			pr_err("Invalid Sensor id:0x%x write gc02m1b eeprom\n", pStereodata->uSensorId);
			return IMGSENSOR_RETURN_ERROR;
		}
	} else {
		pr_err("gc02m1b write_Module_data pStereodata is null\n");
		return IMGSENSOR_RETURN_ERROR;
	}
	return ret;
}
...
{
    int i = 0;
    float externStereo_20041[6] = {0.0194, 0.0040, 0.0035, -10.2240, 0.0220, -1.5220};
    ...
    for (i = 0; i < DUALCAM_CALI_INTERNALCOEF_DATA_LENGTH; i++) {
        gImgEepromInfo.stereoMWdata[CALI_DATA_MASTER_LENGTH + i] =
            Eeprom_1ByteDataRead(GC02M1B_STEREO_START_ADDR + i, 0xA2);
    }
    memcpy(&gImgEepromInfo.stereoMWdata[DUALCAM_CALI_DATA_LENGTH_TOTAL - 28], externStereo_20041, 24);
}
        ...
        do {
            *sensor_id = return_sensor_id() + 1;
            if (*sensor_id == imgsensor_info.sensor_id) {
                ...
                #endif
                pr_info("gc02m1b_mipi_mono1: i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
                return ERROR_NONE;
            }
            pr_info("gc02m1b_mipi_mono1: Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 2;
                             ...
                             UINT8 *feature_para,UINT32 *feature_para_len)
{
    ...
    switch (feature_id) {
        ...
        case SENSOR_FEATURE_GET_MODULE_INFO:
             if(project_20041){
                LOG_INF("gc02m1b GET_MODULE_CamInfo:%d %d\n", *feature_para_len, *feature_data_32);
                *(feature_data_32 + 1) = (gGc02m1b_CamInfo_CamInfo[1] << 24)
                            | (gGc02m1b_CamInfo_CamInfo[0] << 16)
                            | (gGc02m1b_CamInfo_CamInfo[3] << 8)
                            | (gGc02m1b_CamInfo_CamInfo[2] & 0xFF);
                *(feature_data_32 + 2) = (gGc02m1b_CamInfo_CamInfo[5] << 24)
                            | (gGc02m1b_CamInfo_CamInfo[4] << 16)
                            | (gGc02m1b_CamInfo_CamInfo[7] << 8)
                            | (gGc02m1b_CamInfo_CamInfo[6] & 0xFF);
            }
            break;
        case SENSOR_FEATURE_GET_MODULE_SN:
            if(project_20041){
               LOG_INF("gc02m1b GET_MODULE_SN:%d %d\n", *feature_para_len, *feature_data_32);
               if (*feature_data_32 < CAMERA_MODULE_SN_LENGTH/4)
                   *(feature_data_32 + 1) = (gGc02m1b_SN[4*(*feature_data_32) + 3] << 24)
                               | (gGc02m1b_SN[4*(*feature_data_32) + 2] << 16)
                               | (gGc02m1b_SN[4*(*feature_data_32) + 1] << 8)
                               | (gGc02m1b_SN[4*(*feature_data_32)] & 0xFF);
            }
            break;
