...
#define PFX "HI846_camera_sensor_chivas"
...
static bool project_20075 = false ;
#endif

static imgsensor_info_struct imgsensor_info = {
	.sensor_id = HI846_SENSOR_ID_CHIVAS,
...
static kal_int32 write_Module_data(ACDK_SENSOR_ENGMODE_STEREO_STRUCT * pStereodata)
{
	...
	if(pStereodata != NULL) {
		...
		if ((pStereodata->uSensorId == HI846_SENSOR_ID_CHIVAS)
...
static void set_mirror_flip(kal_uint8 image_mirror)
{
	...
	case IMAGE_V_MIRROR:
	write_cmos_sensor(0x000E, itemp | 0x0200);
	break;

	case IMAGE_H_MIRROR:
	write_cmos_sensor(0x000E, itemp | 0x0100);
	break;

	case IMAGE_HV_MIRROR:
	write_cmos_sensor(0x000E, itemp | 0x0300);
	break;
	}
}
...
static void write_shutter(kal_uint32 shutter)
{
	...
	printk("shutter =%d, framelength =%d", shutter,imgsensor.frame_length);
        ...
        do {
            *sensor_id =return_sensor_id();
            ...
            if (*sensor_id == imgsensor_info.sensor_id) {
                ...
                read_eeprom_SN();
                #endif
                printk("hi846_mipi_raw: i2c write id  : 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
                return ERROR_NONE;
            }
            printk("hi846_mipi_raw: get_imgsensor_id Read sensor id fail, i2c write id: 0x%x,sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 2;
...
static kal_uint32 open(void)
{
	...
	printk("[open]: PLATFORM:MT6735,MIPI 4LANE\n");
	printk("preview 1632*1224@30fps,360Mbps/lane; capture 3264*2448@30fps,720Mbps/lane\n");
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		spin_lock(&imgsensor_drv_lock);
		imgsensor.i2c_write_id = imgsensor_info.i2c_addr_table[i];
		spin_unlock(&imgsensor_drv_lock);
		do {
			...
			if (sensor_id == imgsensor_info.sensor_id) {
				printk("hi846_mipi_raw_chivas: i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id, sensor_id);
				break;
			}
			printk("hi846_mipi_raw: open:Read sensor id fail open i2c write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id, sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		if (sensor_id == imgsensor_info.sensor_id) {
	...
	imgsensor.current_fps = imgsensor_info.pre.max_framerate;
        #ifdef OPLUS_FEATURE_CAMERA_COMMON
	if(is_project(OPLUS_20075)||is_project(OPLUS_20076)){
	printk("wenjun 20075");
