...
static  struct imgsensor_info_struct imgsensor_info = {
	...
	.sensor_id = OV02B1B_SENSOR_ID_20611,
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			...
			*sensor_id = ((read_cmos_sensor(0x02) << 8) | read_cmos_sensor(0x03));
			pr_info("ov02b1b_mipi_raw_20611: get_imgsensor_id:0x%x  i2c:0x%x\n", *sensor_id, imgsensor.i2c_write_id);
			if (*sensor_id == OV02B10_SENSOR_ID) {
				*sensor_id = imgsensor_info.sensor_id;
				pr_info("ov02b1b_mipi_raw_20611: i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
				return ERROR_NONE;
			}
			pr_info("ov02b1b_mipi_raw_20611: Read sensor id fail,write_id:0x%x, id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
			retry--;
		} while (retry > 0);
		i++;
		retry = 2;
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			...
			pr_info("ov02b1b_mipi_raw_20611: Read sensor id fail, write: 0x%x, sensor: 0x%x\n", imgsensor.i2c_write_id, sensor_id);
			retry--;
		} while (retry > 0);
		i++;
		if (sensor_id == imgsensor_info.sensor_id) {
