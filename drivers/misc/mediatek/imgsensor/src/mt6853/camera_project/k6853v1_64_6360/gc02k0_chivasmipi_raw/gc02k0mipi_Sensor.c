...
#define PFX "gc02k0_camera_sensor_chivas"
#define LOG_1 LOG_INF("GC02K0CHIVAS, MIPI 1LANE\n")
/****************************   Modify end    *******************************************/

#define LOG_INF(format, args...)    pr_debug(PFX "[%s] " format, __func__, ##args)

static DEFINE_SPINLOCK(imgsensor_drv_lock);
static kal_bool GC02K0DuringTestPattern = KAL_FALSE;

static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = GC02K0_SENSOR_ID_CHIVAS,
...
static void set_shutter_frame_length(kal_uint16 shutter, kal_uint16 frame_length, kal_bool auto_extend_en)
{
	...
	printk("Exit! shutter = %d, framelength = %d\n", shutter, imgsensor.frame_length);
	printk("Exit! cal_shutter = %d, ", cal_shutter);
}
/*
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	if ((iReg >= ANALOG_GAIN_1) && (iReg < ANALOG_GAIN_2)) {
		...
		write_cmos_sensor(0xb1, temp >> 8);
		write_cmos_sensor(0xb2, temp & 0xff);
		printk("analogic gain 1.43x, add pregain = %d\n", temp);
		...
		write_cmos_sensor(0xb1, temp >> 8);
		write_cmos_sensor(0xb2, temp & 0xff);
		printk("analogic gain 1.99x, add pregain = %d\n", temp);
		...
		write_cmos_sensor(0xb1, temp >> 8);
		write_cmos_sensor(0xb2, temp & 0xff);
		printk("analogic gain 2.86x, add pregain = %d\n", temp);
		...
		write_cmos_sensor(0xb1, temp >> 8);
		write_cmos_sensor(0xb2, temp & 0xff);
		printk("analogic gain 4.01x, add pregain = %d\n", temp);
		...
		write_cmos_sensor(0xb1, temp >> 8);
		write_cmos_sensor(0xb2, temp & 0xff);
		printk("analogic gain 5.76x, add pregain = %d\n", temp);
		...
		write_cmos_sensor(0xb1, temp >> 8);
		write_cmos_sensor(0xb2, temp & 0xff);
		printk("analogic gain 8.30x, add pregain = %d\n", temp);
		...
		write_cmos_sensor(0xb1, temp >> 8);
		write_cmos_sensor(0xb2, temp & 0xff);
		printk("analogic gain 11.72x, add pregain = %d\n", temp);
	} else {
		write_cmos_sensor(0xfe, 0x00);
		...
		write_cmos_sensor(0xb1, temp >> 8);
		write_cmos_sensor(0xb2, temp & 0xff);
		printk("analogic gain 17.06x, add pregain = %d\n", temp);
	}
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            *sensor_id = return_sensor_id() & 0xffef;
            if (*sensor_id == GC02K0_SENSOR_ID) {
                *sensor_id = imgsensor_info.sensor_id;
            }
            if (*sensor_id == imgsensor_info.sensor_id) {
                printk("gc02k0_mipi_raw: i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
                #ifdef OPLUS_FEATURE_CAMERA_COMMON
            ...
            printk("gc02k0_mipi_raw: Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
            retry--;
        } while (retry > 0);
        i++;
        retry = 2;
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			...
			if (sensor_id == imgsensor_info.sensor_id) {
				printk("gc02k0_mipi_raw_chivas: i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id, sensor_id);
				break;
			}
			printk("gc02k0_mipi_raw: Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id, sensor_id);
			retry--;
