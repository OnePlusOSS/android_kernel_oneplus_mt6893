...
static kal_int32 write_Module_data(ACDK_SENSOR_ENGMODE_STEREO_STRUCT * pStereodata)
{
	...
	if(pStereodata != NULL) {
		...
		if ((pStereodata->uSensorId == GC02M1B_SENSOR_ID) && (data_length == DUALCAM_CALI_DATA_LENGTH)
    ...
    if(I2C_BUS != 2){
        *sensor_id = 0xFFFFFFFF;
    	...
    	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
			...
			#ifdef OPLUS_FEATURE_CAMERA_COMMON
			if(is_project(20041)||is_project(20042)){
			    read_eeprom_SN();
			    read_eeprom_CamInfo();
				/*project_20041 = true;
				if (deviceInfo_register_value == 0x00) {
					 imgsensor_info.module_id = read_module_id();
					 register_imgsensor_deviceinfo("Cam_r2",DEVICE_VERSION_GC02M1B,imgsensor_info.module_id);
					 deviceInfo_register_value=0x01;
				}
			}else{
			      if (deviceInfo_register_value == 0x00) {
				       imgsensor_info.module_id = 0x1B;
				       register_imgsensor_deviceinfo("Cam_r2",DEVICE_VERSION_GC02M1B,imgsensor_info.module_id);
				       deviceInfo_register_value=0x01;
			      }*/
			}
