...
#define SAR_REG_ADDRESS             (0x07)
#define SAR_REG_VALUE               (0x08)
...
static void sensor_devinfo_work(struct work_struct *dwork)
{
    ...
    /* send cfg to scp*/
    //static value store in cfg 3 4 5
#ifdef CONFIG_OPLUS_SENSOR_MTK68XX
...
#else
    for (index = 0; index < 6; index++) {
        cfg_data[index] = g_cali_data->acc_data[index];
    }
#endif
...
#else
    for (index = 0; index < 6; index++) {
        cfg_data[index] = g_cali_data->gyro_data[index];
    }
#endif
...
static ssize_t sensor_feature_write(struct file *filp, const char *ubuf, size_t cnt, loff_t *data)
{
    ...
    case SAR_REG_VALUE:
        ...
        DEVINFO_LOG("g_reg_add = 0x%x g_reg_val = 0x%x,g_sar_num = 0x%x\n", g_reg_address, g_reg_value, g_sar_num);
...
static int oplus_sensor_feature_init()
{
    ...
    p_entry = proc_create_data("sar_reg_address", S_IRUGO | S_IWUGO, oplus_sensor, &Sensor_info_fops,
            UINT2Ptr(SAR_REG_ADDRESS));
    if (!p_entry) {
        DEVINFO_LOG("sar_reg_address err\n ");
        return -ENOMEM;
    }

    p_entry = proc_create_data("sar_reg_value", S_IRUGO | S_IWUGO, oplus_sensor, &Sensor_info_fops,
            UINT2Ptr(SAR_REG_VALUE));
    if (!p_entry) {
        DEVINFO_LOG("sar_reg_value err\n ");
        return -ENOMEM;
    }
