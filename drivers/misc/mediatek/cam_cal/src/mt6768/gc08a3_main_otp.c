...
static kal_bool gc08a3_otp_read_module_info(void)
{
	...
	if (flag1 == GC08A3_FLAG_GROUP1) {
		addr = GC08A3_DATA_ADDR_MODULE1;
	} else if (flag2 == GC08A3_FLAG_GROUP2) {
...
static kal_bool gc08a3_otp_read_lsc(void)
{
	...
	gc08a3_otp_read_data_retry(addr,
		(kal_uint8 *)&(gc08a3_otp_data.lsc), GC08A3_DATA_LEN_LSC); //1869
	return KAL_TRUE;
}
...
unsigned int gc08a3_main_read_region(struct i2c_client *client,
		unsigned int addr,
		unsigned char *data, unsigned int size)
{
	...
	LOG_INF_IF(
		...
		sizeof(gc08a3_otp_data),
		GC08A3_OTP_SIZE, GC08A3_OTP_SIZE_VALID);
