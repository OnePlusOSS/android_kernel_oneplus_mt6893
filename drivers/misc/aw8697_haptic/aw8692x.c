...
static uint8_t aw8692x_get_chip_state(struct aw_haptic *aw_haptic)
{
	...
	i2c_r_bytes(aw_haptic, AW8692X_REG_SYSST, &chip_state_val,
		    AW_I2C_BYTE_ONE);
//	aw_dev_info("%s: chip state: 0x%02X!\n", __func__, chip_state_val);
...
//	aw_dev_info("%s: reg 0x79: 0x%02X\n", __func__, reg_val);
	if (flag && (reg_val < 0x40))
	...
	i2c_r_bytes(aw_haptic, AW8692X_REG_TESTR, &reg_val, AW_I2C_BYTE_ONE);
//	aw_dev_info("%s: reg 0x65: 0x%02X\n", __func__, reg_val);
	i2c_r_bytes(aw_haptic, AW8692X_REG_GLBRD5, &reg_val, AW_I2C_BYTE_ONE);
//	aw_dev_info("%s: reg 0x3F: 0x%02X\n", __func__, reg_val);
}
...
static void aw8692x_upload_lra(struct aw_haptic *aw_haptic, uint32_t flag)
{
	...
	i2c_w_bytes(aw_haptic, AW8692X_REG_ANACFG20, &cali_data,
				AW_I2C_BYTE_ONE);
	aw_dev_info("%s:final upload trim_lra: 0x%02X", __func__, cali_data);
	aw8692x_tm_config(aw_haptic, AW_LOCK);
}
...
static int aw8692x_get_irq_state(struct aw_haptic *aw_haptic)
{
	...
	if (reg_val & AW8692X_BIT_SYSINT_BST_SCPI) {
		...
		aw_dev_err("%s: chip scp int error\n", __func__);
	}
