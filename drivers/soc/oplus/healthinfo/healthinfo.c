...
/*
 * CONFIG_OPLUS_FEATURE_SCHED_ASSIST not defined in 21081
 */
...
static ssize_t alloc_wait_read(struct file *filp, char __user *buff, size_t count, loff_t *off)
{
        ...
        len = sprintf(page, "total_alloc_wait_h_cnt: %llu\n""total_alloc_wait_l_cnt: %llu\n"
...
static ssize_t ion_wait_read(struct file *filp, char __user *buff, size_t count, loff_t *off)
{
        ...
        len = sprintf(page, "total_ion_wait_h_cnt: %llu\n""total_ion_wait_l_cnt: %llu\n"
...
static ssize_t ohm_para_read(struct file *filp, char __user *buff, size_t count, loff_t *off)
{
        ...
        len = sprintf( page, "action: %s\n""ctrl: 0x%08x\n""logon: 0x%08x\n""trig: 0x%08x\n",
                (ohm_action_ctrl ? "true":"false"), (u32)ohm_ctrl_list, (u32)ohm_logon_list, (u32)ohm_trig_list);
...
static ssize_t ohm_para_write(struct file *file, const char __user *buff, size_t len, loff_t *ppos)
{
	...
	if(len > 31)
        ...
        ohm_debug("write: %s, set: %s, ctrl: 0x%08x, logon: 0x%08x, trig: 0x%08x\n",
                write_data, ctrl_list, (u32)ohm_ctrl_list, (u32)ohm_logon_list, (u32)ohm_trig_list);
	return len;
}
...
static ssize_t ss_binder_proc_read(struct file *filp, char __user *buff, size_t count, loff_t *off)
{
	char page[1024] = {0};
	int len = 0;

	len = sprintf(page, "async_mem_over_high: %llu\nasync_mem_over_low: %llu\nsync_mem_over_high: %llu\nsync_mem_over_low: %llu\nwarning_cg_bg: %llu\nwarning: %llu\n",
...
static ssize_t sched_assist_ss_binder_proc_read(struct file *filp, char __user *buff, size_t count, loff_t *off)
{
	char page[1024] = {0};
	int len = 0;

	len = sprintf(page, "in_all: %d\nin_tt: %d\nin_pt: %d\nfg_in_all: %d\nfg_in_tt: %d\nfg_in_pt: %d\nbg_in_all: %d\nbg_in_tt: %d\nbg_in_pt: %d\nwarning: %d\n",
		system_server_proc.trans2proc[TYPE_ALL][IN_ALL],
		system_server_proc.trans2proc[TYPE_ALL][IN_THREAD_TODO_LIST],
		system_server_proc.trans2proc[TYPE_ALL][IN_PROC_TODO_LIST],
		system_server_proc.trans2proc[TYPE_TOPAPP][IN_ALL],
		system_server_proc.trans2proc[TYPE_TOPAPP][IN_THREAD_TODO_LIST],
		system_server_proc.trans2proc[TYPE_TOPAPP][IN_PROC_TODO_LIST],
