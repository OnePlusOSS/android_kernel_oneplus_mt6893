...
static int32_t c_show_selftest(struct seq_file *m, void *v)
{
	...
	if ((TestResult_Short == 0)
			...
			&& (TestResult_Noise == 0)) {
		if(ts->oplus_baseline_test_flag == 1) {
...
int32_t nvt_selftest_open(struct inode *inode, struct file *file)
{
	...
	if(of_property_read_bool(np, "novatek,mp-support-dt")) {
		...
		if (ts->vendor_id == 0) {
			/* hlt */
			snprintf(mpcriteria, 32, "novatek-mp-criteria-hlt");
		} else if (ts->vendor_id == 2) {
			/* hlt b3 */
			snprintf(mpcriteria, 32, "novatek-mp-criteria-b3");
		} else {
			/* skyw */
			snprintf(mpcriteria, 32, "novatek-mp-criteria-skyw");
		}
...
#ifdef ODM_WT_EDIT
/*
	if (ts->oplus_baseline_test_flag == 0) {
		nvt_update_firmware(fw->firmware_name);
	} else {
...
struct file_operations oplus_nvt_selftest_fops = {
	.owner = THIS_MODULE,
...
int oplus_nvt_blackscreen_test(void)
{
...
*/
	...
	if ((TestResult_FWMutual == 0)
			&& (TestResult_FW_DiffMax == 0)) {
		sprintf(ts->nvt_oplus_proc_data->gesture_test.message, "0 error(s). All test passed");
	} else {
		if (ts->oplus_baseline_test_flag == 0) {
			sprintf(ts->nvt_oplus_proc_data->gesture_test.message, "can not do the test without baseline_test");
		} else {
			sprintf(ts->nvt_oplus_proc_data->gesture_test.message, "result=0");
		}
	}
...
{
	NVT_proc_selftest_entry = proc_create("nvt_selftest", 0444, NULL, &oplus_nvt_selftest_fops);
#if LPGW_TEST
