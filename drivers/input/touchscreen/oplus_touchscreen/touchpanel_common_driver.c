...
__attribute__((weak)) void switch_kalman_fun(struct touchpanel_data *ts,
		bool game_switch)
{
	return;
}

__attribute__((weak)) int touch_algorithm_handle(struct touchpanel_data *ts,
		int obj_attention, struct point_info *points)
{
	return obj_attention;
}
__attribute__((weak)) void oplus_touch_algorithm_init(struct touchpanel_data
		*ts)
{
	return;
}
...
void operate_mode_switch(struct touchpanel_data *ts)
{
	...
	if (ts->is_suspended) {
		...
		if (ts->sensitive_level_array_support && ts->ts_ops->sensitive_lv_set) {
			if (ts->sensitive_level_charging_array_support && ts->charger_pump_support
					&& ts->is_usb_checked) {
				ts->ts_ops->sensitive_lv_set(ts->chip_data,
							     ts->sensitive_level_charging_array[ts->sensitive_level_chosen]);
...
void notify_pen_state(int state)
{
	...
	if (g_tp->pen_support && (g_tp->is_pen_attracted != state)) {
		...
		TPD_INFO("%s: check pen state0 : %d %d, is_suspended: %d\n", __func__,
			 g_tp->is_pen_connected, g_tp->is_pen_attracted, g_tp->is_suspended);
		...
		if (!g_tp->is_suspended
			...
			g_tp->ts_ops->mode_switch(g_tp->chip_data, MODE_PEN_SCAN,
						  g_tp->is_pen_connected && !g_tp->is_pen_attracted);

		} else if (g_tp->is_suspended && ((g_tp->gesture_enable & 0x01) == 1
						  || g_tp->fp_enable)
				&& (!g_tp->hall_status)) {	/* For close pencil scan under gesture mode */
			g_tp->ts_ops->mode_switch(g_tp->chip_data, MODE_PEN_SCAN, \
						  (g_tp->gesture_enable_indep & (1 << PENDETECT)) && g_tp->is_pen_connected
	...
	if (ts->limit_edge) {
		if (points.x > ts->edge_limit.left_x2 && points.x < ts->edge_limit.right_x2) {
			...
			ts->edge_limit.in_which_area = AREA_NORMAL;
				...
				&& points.x < ts->edge_limit.left_x2) || (points.x > ts->edge_limit.right_x2
...
static bool corner_point_process(struct touchpanel_data *ts,
				 struct corner_info *corner, struct point_info *points, int i)
{
	int j;
			...
			if (ts->limit_edge) {
				if (points[i].x <= ts->monitor_data.eli_ver_range) {
					...
					left_edge_bit = left_edge_bit | (1 << i);
						...
						&& (points[i].x < ts->resolution_info.max_x - ts->monitor_data.eli_ver_range)) {
				...
				if (points[i].y <= ts->monitor_data.eli_hor_range) {
					...
					left_center_cnt++;
						...
						&& (points[i].y < ts->resolution_info.max_y - ts->monitor_data.eli_hor_range)) {
					cross_status[i] = 1;    //set cross status of i
					right_center_cnt++;
			...
			for (i = 0; i < ts->max_num; i++) {
				if ((((left_edge_bit & TOUCH_BIT_CHECK) >> i) & 0x01) && !cross_status[i]) {
					...
					if (!left_trigger_status && ts->monitor_data.eli_ver_pos) {
						if (points[i].x <= ts->monitor_data.eli_ver_range) {
							ts->monitor_data.eli_ver_pos[points[i].x / unit * max_array_y + points[i].y /
										     unit]++;

						} else {
							ts->monitor_data.eli_ver_pos[(ts->resolution_info.max_x - points[i].x) / unit *
										     max_array_y + points[i].y / unit]++;
						}
					}
				}
			}
			...
			for (i = 0; i < ts->max_num; i++) {
				if ((((left_edge_bit & TOUCH_BIT_CHECK) >> i) & 0x01) && !cross_status[i]) {
					...
					if (!left_trigger_status && ts->monitor_data.eli_hor_pos) {
						ts->monitor_data.eli_hor_pos[points[i].y / unit * max_array_x +
			...
			for (i = 0; i < ts->max_num; i++) {
				if ((((right_edge_bit & TOUCH_BIT_CHECK) >> i) & 0x01) && !cross_status[i]) {
					...
					if (!right_trigger_status && ts->monitor_data.eli_hor_pos) {
						ts->monitor_data.eli_hor_pos[(ts->resolution_info.max_y - points[i].y) / unit *
									     max_array_x + (ts->resolution_info.max_x - points[i].x) / unit]++;
					}
				}
			}
		...
		for (i = 0; i < ts->max_num; i++) {
			...
			if (((obj_attention & TOUCH_BIT_CHECK) >> i) & 0x01
				...
				if (ts->edge_limit_support) {
					if (corner_point_process(ts, corner, points, i) || (!ts->drlimit_remove_support
...
static void tp_btnkey_handle(struct touchpanel_data *ts)
{
	...
	if (CHK_BIT(ts->vk_bitmap, BIT_MENU)) {
		input_report_key_oplus(ts->kpd_input_dev, KEY_MENU, CHK_BIT(touch_state,
				       BIT_MENU));
	}

	if (CHK_BIT(ts->vk_bitmap, BIT_HOME)) {
		input_report_key_oplus(ts->kpd_input_dev, KEY_HOMEPAGE, CHK_BIT(touch_state,
				       BIT_HOME));
	}

	if (CHK_BIT(ts->vk_bitmap, BIT_BACK)) {
		input_report_key_oplus(ts->kpd_input_dev, KEY_BACK, CHK_BIT(touch_state,
				       BIT_BACK));
	}
...
static void tp_work_func(struct touchpanel_data *ts)
{
	...
	if (ts->ts_ops->u32_trigger_reason) {
		cur_event = ts->ts_ops->u32_trigger_reason(ts->chip_data, (ts->gesture_enable
...
void switch_headset_state(int headset_state)
{
	...
	if (g_tp->headset_pump_support && (g_tp->is_headset_checked != headset_state)) {
		...
		TPD_INFO("%s: check headset state : %d, is_suspended: %d\n", __func__,
			 headset_state, g_tp->is_suspended);
		...
		const char __user *buffer, size_t count, loff_t *ppos)
{
	...
	if (strstr(name, "smooth_charging_level") && (value_cnt == SMOOTH_LEVEL_NUM)
	...
	if (strstr(name, "sensitive_level") && (value_cnt == SENSITIVE_LEVEL_NUM)
				     ...
				     size_t count, loff_t *ppos)
{
	...
	if (!ts) {
		...
		snprintf(page, PAGESIZE - 100,
			 "%d, smooth_level:%d, sensitive_level:%d, high_frame_support:%d\n",
			 ts->noise_level, ts->smooth_level_chosen, ts->sensitive_level_chosen,
			 high_frame_support); /*support*/
	}
				     ...
				     const char __user *buffer, size_t count, loff_t *ppos)
{
	...
	ts->edge_limit.right_x1   = ts->resolution_info.LCD_WIDTH -
	...
	ts->edge_limit.right_y1   = ts->resolution_info.LCD_HEIGHT -
				      ...
				      const char __user *buffer, size_t count, loff_t *ppos)
{
	...
	if (ts->default_hor_area) {             //if configed
		if (!temp) {        //in vertical mode
			...
			ts->limit_corner = (1 == temp) ? (1 << CORNER_TOPLEFT) :
...
static int tp_test_limit_switch(struct touchpanel_data *ts)
{
	...
	memcpy(ts->panel_data.aging_test_limit_name, ts->panel_data.test_limit_name,
	       copy_len);
...
static int finger_protect_handler(void *data)
{
	...
	do {
		if (ts->spuri_fp_touch.lcd_trigger_fp_check) {
			...
			wait_event_interruptible(waiter, ts->spuri_fp_touch.fp_trigger
						 && ts->i2c_ready);
		}
		...
		const char __user *buffer, size_t count, loff_t *ppos)
{
	...
	if (value != ts->is_pen_connected) {
		...
		TPD_INFO("%s: check pen state1 : %d %d, is_suspended: %d\n", __func__,
			 ts->is_pen_connected, ts->is_pen_attracted, ts->is_suspended);
		...
		} else if (ts->is_suspended && ((ts->gesture_enable & 0x01) == 1
						|| ts->fp_enable)
				&& (!ts->hall_status)) {	/* For close pencil scan under gesture mode */
			ts->ts_ops->mode_switch(ts->chip_data, MODE_PEN_SCAN, \
						(ts->gesture_enable_indep & (1 << PENDETECT)) && ts->is_pen_connected
						&& !ts->is_pen_attracted);
		}
...
static int init_touchpanel_proc(struct touchpanel_data *ts)
{
	...
	if (ts->edge_limit_support || ts->fw_edge_limit_support) {
		...
		if (ts->edge_limit_support) {
			prEntry_tmp = proc_create_data("oplus_tp_limit_area", 0664, prEntry_tp,
	...
	if (ts->noise_modetest_support) {
		prEntry_tmp = proc_create_data("oplus_tp_noise_modetest", 0664, prEntry_tp,
	...
	if (ts->pen_support) {
		prEntry_tmp = proc_create_data("pencil_connected", 0666, prEntry_tp,
	...
	if (ts->auto_test_need_cal_support) {
		...
		prEntry_tmp = proc_create_data("calibration_status", 0666, prEntry_tp,
...
static int init_debug_info_proc(struct touchpanel_data *ts)
{
	...
	if (ts->smooth_level_array_support || ts->smooth_level_charging_array_support ||
			...
			||
			ts->stop_filter_set_support) {
		prEntry_tmp = proc_create_data("game_para_control", 0666, prEntry_debug_info,
	...
	if (ts->ear_sense_support) {
		...
		prEntry_tmp = proc_create_data("rawdata", 0666, prEntry_earsense,
	...
	if (ts->face_detect_support) {
		...
		prEntry_tmp = proc_create_data("fd_enable", 0666, ts->prEntry_tp,
		...
		prEntry_tmp = proc_create_data("event_num", 0666, ts->prEntry_tp,
		...
		prEntry_tmp = proc_create_data("fd_touch_count", 0666, ts->prEntry_tp,
	...
	if (ts->fingerprint_underscreen_support) {
		prEntry_tmp = proc_create_data("fp_enable", 0666, ts->prEntry_tp,
...
static int init_input_device(struct touchpanel_data *ts)
{
	...
	if (ts->pen_support) {
		input_set_abs_params(ts->pen_input_dev, ABS_X, 0, ts->pen_config.max_x - 1, 0,
				     0);
		input_set_abs_params(ts->pen_input_dev, ABS_Y, 0, ts->pen_config.max_y - 1, 0,
				     0);
		...
		input_set_abs_params(ts->pen_input_dev, ABS_TILT_X,
				     -(ts->pen_config.tilt_x_max), ts->pen_config.tilt_x_max, 0, 0);
		input_set_abs_params(ts->pen_input_dev, ABS_TILT_Y,
				     -(ts->pen_config.tilt_y_max), ts->pen_config.tilt_y_max, 0, 0);
			      ...
			      struct touchpanel_data *ts)
{
	...
	for_each_child_of_node(parent, np) {
		...
		rc = of_property_read_u32_array(np, "sensitive-level", temp8_array,
						SENSITIVE_LEVEL_NUM);
...
static int init_parse_dts(struct device *dev, struct touchpanel_data *ts)
{
	...
	ts->iovcc_shutdown_support = of_property_read_bool(np,
	...
	if (ts->panel_data.project_num > 0) {
		...
		for (i = 0; i < ts->panel_data.project_num; i++) {
			ts->panel_data.platform_support_commandline[i] = devm_kzalloc(dev, 100,
					GFP_KERNEL);
			ts->panel_data.platform_support_external_name[i] = devm_kzalloc(dev, 7,
					GFP_KERNEL);
			...
			rc = of_property_read_string_index(np, "platform_support_project_commandline",
							   i, (const char **)&ts->panel_data.platform_support_commandline[i]);
			...
			rc = of_property_read_string_index(np, "platform_support_project_external_name",
							   i, (const char **)&ts->panel_data.platform_support_external_name[i]);
	...
	ts->drlimit_remove_support  = of_property_read_bool(np,
	...
	ts->spurious_fp_support     = of_property_read_bool(np,
	...
	ts->smart_gesture_support   = of_property_read_bool(np,
				      "smart_gesture_support");
	ts->pressure_report_support = of_property_read_bool(np,
	...
	ts->external_touch_support  = of_property_read_bool(np,
				      "external_touch_support");
	ts->spuri_fp_touch.lcd_trigger_fp_check = of_property_read_bool(np,
	...
	ts->report_point_first_support = of_property_read_bool(ts->dev->of_node,
					 "report_point_first_support");
	ts->lcd_wait_tp_resume_finished_support = of_property_read_bool(np,
			"lcd_wait_tp_resume_finished_support");
	ts->spuri_fp_touch.lcd_resume_ok = true;
	ts->new_set_irq_wake_support = of_property_read_bool(np,
				       "new_set_irq_wake_support");
	ts->report_flow_unlock_support = of_property_read_bool(np,
					 "report_flow_unlock_support");
	ts->screenoff_fingerprint_info_support = of_property_read_bool(np,
			"screenoff_fingerprint_info_support");
	ts->irq_need_dev_resume_ok =  of_property_read_bool(np,
	...
	ts->thermal_detect_support = of_property_read_bool(np,
	...
	if (rc) {
		...
		if (ts->firmware_in_dts != NULL) {
			...
			if (ts->firmware_in_dts->data == NULL) {
				...
				rc = of_property_read_u8_array(np, "firmware-data",
	...
	if (ts->pen_support) {
		...
		TPD_INFO("pen_tx_num = %d, pen_rx_num = %d \n", ts->hw_res.pen_tx_num,
			 ts->hw_res.pen_rx_num);

		rc = of_property_read_u32_array(np, "touchpanel,pen-panel-coords", temp_array,
						2);
	...
	rc = of_property_read_u32_array(np, "touchpanel,sensitive-level-charging",
	...
	rc = of_property_read_u32_array(np, "touchpanel,stop-filter-set", temp_array,
					1);
...
int register_common_touch_device(struct touchpanel_data *pdata)
{
	...
	ts->panel_data.manufacture_info.version = kzalloc(MAX_DEVICE_VERSION_LENGTH,
			GFP_KERNEL);
	...
	ts->panel_data.manufacture_info.manufacture = kzalloc(MAX_DEVICE_MANU_LENGTH,
			GFP_KERNEL);
	...
	if (ts->spurious_fp_support) {
		ts->spuri_fp_touch.thread = kthread_run(finger_protect_handler, ts,
							"touchpanel_fp");
