...
struct panel_info {
	...
	const char
	*chip_name;                         /*chip name the panel is controlled by*/
...
struct touchpanel_data {
	...
	suspend_resume_state
	suspend_state;                 /*detail suspend/resume state*/
	...
	tp_interrupt_mode
	int_mode;                         /*whether interrupt and be disabled*/
	...
	struct mutex
		mutex;                       /*mutex for lock i2c related flow*/
	struct mutex
		report_mutex;                /*mutex for lock input report flow*/
	struct mutex           mutex_earsense;
	struct completion
		pm_complete;                 /*completion for control suspend and resume flow*/
	struct completion
		fw_complete;                 /*completion for control fw update*/
	struct completion
		resume_complete;                 /*completion for control fw update*/
	struct panel_info
		panel_data;                  /*GPIO control(id && pinctrl && tp_type)*/
	...
	char                  *earsense_delta;
	bool
	disable_gesture_ctrl;   /*when lcd_trigger_load_tp_fw start no need to control gesture*/
	bool
	report_point_first_support;             /*if it happened baseline error,reporting point first or reporting error first. */
	uint8_t
	report_point_first_enable;              /*reporting points first enable :1 ,disable 0;*/
...
struct oplus_touchpanel_operations {
	int (*get_chip_info)(void
			     *chip_data);                                           /*return 0:success;other:failed*/
	...
	u8(*get_keycode)(void
			 *chip_data);                                                        /*get touch-key code*/
	int (*esd_handle)(void *chip_data);
	int (*fw_handle)(void
			 *chip_data);                                                        /*return 0 normal; return -1:update failed;*/
	void (*resume_prepare)(void
			       *chip_data);                                           /*using for operation before resume flow,
                                                                                                eg:incell 3320 need to disable gesture to release inter pins for lcd resume*/
	fp_touch_state(*spurious_fp_check)(void
					   *chip_data);                                    /*spurious fingerprint check*/
	void (*finger_proctect_data_get)(void
					 *chip_data);                                    /*finger protect data get*/
	void (*exit_esd_mode)(void
			      *chip_data);                                           /*add for s4322 exit esd mode*/
	void (*register_info_read)(void *chip_data, uint16_t register_addr,
				   uint8_t *result, uint8_t length);    /*add for read registers*/
	void (*write_ps_status)(void *chip_data,
	...
	void (*specific_resume_operate)(void
					*chip_data);                                        /*some ic need specific opearation in resuming*/
	void (*resume_timedout_operate)(void
					*chip_data);                                        /*some ic need opearation if resume timed out*/
	int (*get_usb_state)(
		void);                                                      /*get current usb state*/
	int (*get_wireless_state)(
		void);                                                      /*get current sireless state*/
	void (*black_screen_test)(void *chip_data,
				  char *msg);                                /*message of black gesture test*/
	int (*irq_handle_unlock)(void
				 *chip_info);                                          /*irq handler without mutex*/
	int (*async_work)(void
			  *chip_info);                                                       /*async work*/
	int (*get_face_state)(void
			      *chip_info);                                          /*get face detect state*/
	void (*bootup_test)(void *chip_data, const struct firmware *fw,
			    struct monitor_data *mon_data, struct hw_resource *hw_res);   /*boot_up test*/
	...
	void (*freq_hop_trigger)(void
				 *chip_data);                       /*trigger frequency-hopping*/
...
__attribute__((weak)) int request_firmware_select(const struct firmware
		**firmware_p, const char *name, struct device *device)
{
	return 1;
}
__attribute__((weak)) int opticalfp_irq_handler(struct fp_underscreen_info
		*fp_tpinfo)
{
	return 0;
}
