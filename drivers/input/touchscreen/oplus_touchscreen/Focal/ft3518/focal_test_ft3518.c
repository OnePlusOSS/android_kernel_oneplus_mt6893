...
static int fts_test_bus_read(u8 *cmd, int cmdlen, u8 *data, int datalen)
{
	...
	ret = touch_i2c_read(fts_data->client, (char *)write_buf, cmdlen,
...
static int fts_test_bus_write(u8 *writebuf, int writelen)
{
	...
	ret = touch_i2c_write_block(fts_data->client, writebuf[0], writelen - 1,
				    &writebuf[1]);
...
static void fts_test_save_data(char *name, int *data, int datacnt, int line,
			       int fd)
{
	char *data_buf = NULL;
	u32 cnt = 0;
...
static bool compare_mc_sc(struct fts_ts_data *ts_data, bool tx_check,
...
static bool short_test_get_channel_num(struct fts_ts_data *ts_data,
...
static bool short_test_channel_to_gnd(struct fts_ts_data *ts_data,
...
static bool short_test_channel_to_channel(struct fts_ts_data *ts_data,
...
static int fts_uniformity_autotest(struct fts_ts_data *ts_data,
				   bool *test_result)
{
	int ret = 0;
	int row = 0;
...
static int fts_scap_rawdata_autotest(struct fts_ts_data *ts_data,
				     bool *test_result)
{
	int ret = 0;
	bool tmp_result = false;
...
static void fts_auto_write_result(struct fts_ts_data *ts_data, int failed_count)
{
	...
	if (failed_count) {
		snprintf(file_data_buf, 128,
			 "/sdcard/TpTestReport/screenOn/NG/tp_testlimit_%02d%02d%02d-%02d%02d%02d-fail-utc.csv",
		...
		snprintf(file_data_buf, 128,
			 "/sdcard/TpTestReport/screenOn/OK/tp_testlimit_%02d%02d%02d-%02d%02d%02d-pass-utc.csv",
			 (rtc_now_time.tm_year + 1900) % 100, rtc_now_time.tm_mon + 1,
			 rtc_now_time.tm_mday,
			 rtc_now_time.tm_hour, rtc_now_time.tm_min, rtc_now_time.tm_sec);
	...
	sys_write(ts_data->csv_fd, data_buf, buflen);
#endif
	fts_test_save_data("Noise Test", ts_data->noise_rawdata, node_num, rx_num,
			   ts_data->csv_fd);
	fts_test_save_data("Rawdata Test", ts_data->rawdata, node_num, rx_num,
			   ts_data->csv_fd);
	fts_test_save_data("Rawdata Uniformity Test", ts_data->rawdata_linearity,
			   ts_data->rl_cnt, rx_num, ts_data->csv_fd);
	//    fts_test_save_data("SCAP CB Test", ts_data->scap_cb, ts_data->scb_cnt, channel_num, ts_data->csv_fd);
	fts_test_save_data("SCAP CB Test", ts_data->scap_cb, channel_num, rx_num,
			   ts_data->csv_fd);
	fts_test_save_data("SCAP CB Test", ts_data->scap_cb + channel_num,
			   ts_data->scb_cnt - channel_num, rx_num, ts_data->csv_fd);
	//    fts_test_save_data("SCAP Rawdata Test", ts_data->scap_rawdata, ts_data->srawdata_cnt, channel_num, ts_data->csv_fd);
	fts_test_save_data("SCAP Rawdata Test", ts_data->scap_rawdata, channel_num,
			   rx_num, ts_data->csv_fd);
	fts_test_save_data("SCAP Rawdata Test", ts_data->scap_rawdata + channel_num,
			   ts_data->srawdata_cnt - channel_num, rx_num, ts_data->csv_fd);
...
static void fts_autotest_endoperation(struct fts_ts_data *ts_data,
				      const struct firmware *limit_fw)
{
	TPD_INFO("%s +\n", __func__);
...
static int fts_get_threshold_from_img(struct fts_ts_data *ts_data, char *data,
				      const struct firmware *limit_fw)
{
	...
	ret = request_real_test_limit(ts, &limit_fw, ts->panel_data.test_limit_name,
				      &ts_data->client->dev);
