...
static bool fts_fwupg_check_flash_status(struct fts_ts_data *ts_data,
...
static int fts_flash_write_buf(struct fts_ts_data *ts_data, u32 saddr, u8 *buf,
			       u32 len, u32 delay)
{
	int ret = 0;
	u32 i = 0;
...
void fts_auto_test(struct seq_file *s, void *chip_data,
			   ...
			   int data_type)
{
	...
	if (is_diff) {
		...
		ret = touch_i2c_write_byte(ts_data->client, FACTORY_REG_DATA_SELECT,
					   FTS_CAP_DATA_TYPE_DELTA);
	...
	if (data_type == FTS_MUTUAL_CAP_DATA_DELTA) {
		ret = touch_i2c_write_byte(ts_data->client, FACTORY_REG_LINE_ADDR,
					   FTS_MUTUAL_CAP_DATA_DELTA);

	} else if (data_type == FTS_SELF_CAP_DATA_DELTA) {
		ret = touch_i2c_write_byte(ts_data->client, FACTORY_REG_LINE_ADDR,
					   FTS_SELF_CAP_DATA_DELTA);

	} else if (data_type == FTS_SELF_CAP_DATA_DELTA_WP) {
		ret = touch_i2c_write_byte(ts_data->client, FACTORY_REG_LINE_ADDR,
					   FTS_SELF_CAP_DATA_DELTA_WP);
	}
...
static int fts_enable_game_mode(struct fts_ts_data *ts_data, bool enable)
{
	...
	if (enable) {
		ret = touch_i2c_write_byte(ts_data->client, FTS_REG_REPORT_RATE,
					   FTS_180HZ_REPORT_RATE);
...
static int ft3518_parse_dts(struct fts_ts_data *ts_data,
			    struct i2c_client *client)
{
	...
	ts_data->high_resolution_support_x8 = of_property_read_bool(np,
					      "high_resolution_support_x8");
	TPD_INFO("%s:high_resolution_support is:%d %d\n", __func__,
		 ts_data->high_resolution_support, ts_data->high_resolution_support_x8);
