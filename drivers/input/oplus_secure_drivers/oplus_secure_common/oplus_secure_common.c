...
extern char qsee_sn_bound[];
#endif

static bool  is_sn_bound_support(void)
{
#if IS_MODULE(CONFIG_OPLUS_SECURE_COMMON)
    return strstr(qsee_sn_bound, "true") ? true : false;
#else
    return strstr(saved_command_line, "qsee_support_sn_bound=true") ? true : false;
#endif
}
                ...
                size_t count, loff_t *off)
{
    ...
    if (oem_sec_override1_reg == 0x7860C4) {
        ...
        if (get_secureType() == SECURE_BOOT_ON_STAGE_2) {
                ...
                secureSNBound_state = is_sn_bound_support() ? SECURE_DEVICE_SN_BOUND_ON : SECURE_DEVICE_SN_BOUND_OFF;
        }
    }
