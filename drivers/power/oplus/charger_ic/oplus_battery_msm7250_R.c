...
int oplus_check_is_pd_swrap(void);
static void register_oplus_pdswrap_svid(struct work_struct *work) ;
				    ...
				    union power_supply_propval *val)
{
	...
	int rc = 0, buck_scale = 1, boost_scale = 1;
		...
		if (chip && chip->vbatt_num == 2 && chip->chg_ops->get_charger_current) {
			if(oplus_wrap_get_allow_reading()){
...
void smblib_usb_plugin_hard_reset_locked(struct smb_charger *chg)
{
	...
	if (vbus_rising) {
		...
		if (oplus_wrap_get_fastchg_started() == false && g_oplus_chip) {
...
#ifdef OPLUS_FEATURE_CHG_BASIC
	if(g_oplus_chip->vbatt_num == 2){
		chg->swrap_detect_time= cpu_clock(smp_processor_id()) / 1000000;
		if(((chg->swrap_detect_time - chg->swrap_detach_time) <= 1000) && oplus_wrap_get_detach_unexpectly() 
			&& g_oplus_chip->temperature <= 440 && g_oplus_chip->temperature >= 45){
			if(g_oplus_chip->swrap_disconnect_count < 3)
				g_oplus_chip->swrap_disconnect_count += 1;
			else
				oplus_wrap_set_disable_real_fast_chg(true);
			chg_err("g_oplus_chip->swrap_disconnect_count = %d\n",g_oplus_chip->swrap_disconnect_count);
		}else{
			g_oplus_chip->swrap_disconnect_count = 0;
			oplus_wrap_set_disable_real_fast_chg(false);
		}
		chg_err("chg->swrap_detect_time = %d, chg->swrap_detach_time = %d\n",chg->swrap_detect_time, chg->swrap_detach_time);
		chg->swrap_detect_time = 0;
		chg->swrap_detach_time = 0;
	}
#endif
...
#ifdef OPLUS_FEATURE_CHG_BASIC
		if(g_oplus_chip->vbatt_num == 2){
			chg->swrap_detach_time = cpu_clock(smp_processor_id()) / 1000000;
			chg_err("chg->swrap_detach_time = %d\n",chg->swrap_detach_time);
		}
#endif
						...
						ktime_t time)
{
	...
	if ((oplus_get_usb_status() & USB_WATER_DETECT) != 0 && g_oplus_chip) {
		...
		oplus_wrap_set_disable_adapter_output(false);
...
static bool smblib_src_lpd(struct smb_charger *chg)
{
	...
	if (lpd_flag) {
		...
		if ((oplus_get_usb_status() & USB_WATER_DETECT) == 0 && g_oplus_chip) {
                    oplus_wrap_set_disable_adapter_output(true);
...
irqreturn_t typec_state_change_irq_handler(int irq, void *data)
{
	...
	if (chg->typec_mode == POWER_SUPPLY_TYPEC_SINK
		...
		//vote(chg->apsd_disable_votable, SWRAP_OTG_VOTER, true, 0);
...
//		if (oplus_usbtemp_check_is_support() == true)
	//		wake_up_interruptible(&oplus_usbtemp_wq);
		...
		if (oplus_ccdetect_get_power_role() != POWER_SUPPLY_TYPEC_PR_SINK
			...
			oplus_ccdetect_disable();
#ifdef OPLUS_FEATURE_CHG_BASIC
			if(chg->swrap_detach_time > 0 && g_oplus_chip->vbatt_num == 2){
				chg->swrap_detach_time = 0;
				oplus_wrap_set_detach_unexpectly(false);
				oplus_wrap_set_disable_real_fast_chg(false);
				chg_err("chg->swrap_detach_time =  %d, detach_unexpectly = %d\n",chg->swrap_detach_time, oplus_wrap_get_detach_unexpectly());
			}
#endif
...
#endif
	if (oplus_wrap_get_fastchg_started() == true || chip->charger_volt < 4400) {
		counts = 0;
		goto rerun_work;
	}
...
int smblib_init(struct smb_charger *chg)
{
	...
	INIT_DELAYED_WORK(&chg->regist_pd, register_oplus_pdswrap_svid);
...
void smbchg_set_chargerid_switch_val(int value)
{
	...
	if (oplus_wrap_get_adapter_update_real_status() == ADAPTER_FW_NEED_UPDATE
...
bool oplus_chg_is_usb_present(void)
{
	...
	if (vbus_rising == false && oplus_wrap_get_fastchg_started() == true) {
		if (qpnp_get_prop_charger_voltage_now() > 2000) {
			chg_err("USBIN_PLUGIN_RT_STS_BIT low but fastchg started true and chg vol > 2V\n");
			vbus_rising = true;
		}
	}
	if (vbus_rising == false && (oplus_wrap_get_fastchg_started() == true && (chip->vbatt_num == 2))) {
			chg_err("USBIN_PLUGIN_RT_STS_BIT low but fastchg started true and SWRAP\n");
			vbus_rising = true;
	}
#endif/*OPLUS_FEATURE_CHG_BASIC*/
...
static int smb5_probe(struct platform_device *pdev)
{
	...
	rc = oplus_chg_parse_swrap_dt(oplus_chip);
