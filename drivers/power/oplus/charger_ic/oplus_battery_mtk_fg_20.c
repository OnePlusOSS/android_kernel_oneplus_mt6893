...
{
	if(oplus_gauge_check_chip_is_null() /*|| oplus_wrap_check_chip_is_null()*/ || oplus_chg_check_chip_is_null())
	{
		battery_xlog_printk(BAT_LOG_CRTI, "[do_chrdet_int_task] wrap || gauge || charger not ready, will do after bettery init.\n");
		return;
	}

	if(oplus_wrap_get_fastchg_started() == true && oplus_wrap_get_adapter_update_status() != ADAPTER_FW_NEED_UPDATE){
...
static long adc_cali_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case Get_FakeOff_Param: /* For Factory Mode*/
		...
		fakeoff_out_data[3] = oplus_wrap_get_fastchg_started();
