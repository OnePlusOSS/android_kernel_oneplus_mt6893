        ...
        union power_supply_propval *val)
{
        ...
        if (g_oplus_chip->charger_exist) {
                if ((g_oplus_chip->charger_type == POWER_SUPPLY_TYPE_USB_DCP) || (oplus_wrap_get_fastchg_started() == true)
                        || (oplus_wrap_get_fastchg_to_normal() == true) || (oplus_wrap_get_fastchg_to_warm() == true)
                        || (oplus_wrap_get_adapter_update_status() == ADAPTER_FW_NEED_UPDATE) || (oplus_wrap_get_btb_temp_over() == true)) {
                        g_oplus_chip->ac_online = true;
                } else {
                        g_oplus_chip->ac_online = false;
                }
        } else {
                if ((oplus_wrap_get_fastchg_started() == true) || (oplus_wrap_get_fastchg_to_normal() == true)
                        || (oplus_wrap_get_fastchg_to_warm() == true) || (oplus_wrap_get_adapter_update_status() == ADAPTER_FW_NEED_UPDATE)
                        || (oplus_wrap_get_btb_temp_over() == true) || g_oplus_chip->mmi_fastchg == 0) {
                        g_oplus_chip->ac_online = true;
                } else {
                        g_oplus_chip->ac_online = false;
                }
		...
		union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_SET_SHIP_MODE:
        ...
        case POWER_SUPPLY_PROP_CHARGE_TECHNOLOGY:
                if (g_oplus_chip) {
                        val->intval = g_oplus_chip->wrap_project;
                } else {
                        val->intval = true;
                }
                break;
        ...
        case POWER_SUPPLY_PROP_FAST_CHARGE:
                if (g_oplus_chip) {
                        val->intval = oplus_chg_show_wrap_logo_ornot();
                } else {
                        val->intval = 0;
                }
                break;
        ...
        case POWER_SUPPLY_PROP_ADAPTER_FW_UPDATE:
                val->intval = oplus_wrap_get_adapter_update_status();
                break;

        case POWER_SUPPLY_PROP_WRAPCHG_ING:
                val->intval = oplus_wrap_get_fastchg_ing();
                break;
