...
int mp2650_input_current_limit_ctrl_by_wrap_write(int current_ma)
{
	int rc = 0;
	int tmp = 0;
	int count =0;
	...
	chg_err("set charge ctrl_by_wrap current limit = %d\n", current_ma);
...
void mp2650_dump_registers(void)
{
    ...
    if(dump_count == DUMP_REG_LOG_CNT_30S) {
		...
		"[0x%02x, 0x%02x, 0x%02x, 0x%02x], [0x%02x, 0x%02x, 0x%02x, 0x%02x], "
		"[0x%02x, 0x%02x, 0x%02x, 0x%02x], [0x%02x, 0x%02x, 0x%02x, 0x%02x], "
		"[0x%02x, 0x%02x, 0x%02x], [reg48=0x%02x, reg49=0x%02x, reg2D=0x%02x]\n",
...
struct oplus_chg_operations  mp2650_chg_ops = {
    ...
    .input_current_ctrl_by_wrap_write = mp2650_input_current_limit_ctrl_by_wrap_write,
