...
/*Shouli.Wang@BSP.CHG.Basic 2019/11/11, add for typec_cc_orientation node*/
...
/*Sidong.Zhao@BSP.CHG.Basic 2020/8/4, add for usb port over temp protection*/
...
/*Shouli.Wang@BSP.CHG.Basic 2019/12/07, add for ship mode function*/
	...
	while (!kthread_should_stop()) {
		...
		if ((chip->usb_temp_r >= USB_57C)
			...
			for (i = 0; i < RETRY_COUNT; i++) {
				...
				oplus_get_usbtemp_volt(chip);
				get_usb_temp(chip);
				/*Junbo.Guo@BSP.CHG.Basic 2020/1/7, add for usbtemp*/
...
/*Sidong.Zhao@BSP.CHG.Basic 2020/8/4, add for usb port over temp protection*/
					...
					if (oplus_chg_get_otg_online() == true) {
						...
						/*Shouli.Wang@BSP.CHG.Basic 2019/12/23, wait for vbus fall to make sure otg disable*/
						for(i = 0; i < 100 ; i++){
					...
					/*Shouli.Wang@BSP.CHG.Basic 2019/12/23, disable sub charger*/
					if(chip->is_double_charger_support)
...
#ifdef OPLUS_FEATURE_CHG_BASIC
/*Shouli.Wang@BSP.CHG.Basic 2019/12/05, add for wake usbtemp thread*/
...
/*====================================================================*/
/*Shouli.Wang@BSP.CHG.Basic 2019/10/07, add custom battery node*/
	...
	if (oplus_chip->vbatt_num == 1) {
		...
		if(oplus_chip->is_double_charger_support) {
			...
			case ((1 << BQ2591X) | (1 << BQ2589X)):
				...
				break;
			/*Shouli.Wang@BSP.CHG.Basic 2021/01/07, add for charger bring up*/
...
#ifdef OPLUS_FEATURE_CHG_BASIC
/*Shouli.Wang@BSP.CHG.Basic 2019/10/28, add for hvdcp charge*/
	...
	/*Sidong.Zhao@BSP.CHG.Basic 2019/11/15,add sysfs node for charge control*/
