...
extern bool oplus_is_power_off_charging(struct oplus_wrap_chip *chip);
		...
		union power_supply_propval *val)
{
	...
	if (chip->charger_exist) {
		if ((chip->charger_type == POWER_SUPPLY_TYPE_USB_DCP || suspend_charger)
				|| (oplus_wrap_get_fastchg_started() == true)
				|| (oplus_wrap_get_fastchg_to_normal() == true)
				|| (oplus_wrap_get_fastchg_to_warm() == true)
				|| (oplus_wrap_get_fastchg_dummy_started() == true)
				|| (oplus_wrap_get_adapter_update_status() == ADAPTER_FW_NEED_UPDATE)
				|| (oplus_wrap_get_btb_temp_over() == true)) {
			chip->ac_online = true;
		} else {
...
static void oplus_midas_chg_data_init(void)
{
	...
	if(oplus_wrap_get_allow_reading() == true) {
		...
		union power_supply_propval *val)
{
	...
	switch (psp) {
		case POWER_SUPPLY_PROP_STATUS:
			if (oplus_chg_show_wrap_logo_ornot() == 1) {
		...
		case POWER_SUPPLY_PROP_CAPACITY:
			if(chip->wrap_show_ui_soc_decimal == true && chip->decimal_control) {
		...
		case POWER_SUPPLY_PROP_CHARGE_NOW:
			if (oplus_wrap_get_fastchg_started() == true && (chip->vbatt_num == 2)
			&& oplus_wrap_get_fast_chg_type() != CHARGER_SUBTYPE_FASTCHG_WRAP) {
				val->intval = 10000;
			} else {
		...
		char __user *buff, size_t count, loff_t *off)
{
	...
	if (chip->wrap_show_ui_soc_decimal) {
		swrap_is_control_by_wrap = (chip->vbatt_num == 2 && oplus_wrap_get_fast_chg_type() == CHARGER_SUBTYPE_FASTCHG_WRAP);
		if(swrap_is_control_by_wrap != true
				&&  chip->boot_completed != false && chip->calculate_decimal_time == 0 && oplus_chg_show_wrap_logo_ornot() == true) {
...
/*--------------------------------------------------------*/
int oplus_chg_parse_swrap_dt(struct oplus_chg_chip *chip)
{
	int rc;
	...
	chg_err("oplus_parse_swrap_dt, chip->vbatt_num = %d,chip->wrap_project = %d.\n",
			chip->vbatt_num,chip->wrap_project);
	return 0;
}
...
int oplus_chg_parse_charger_dt(struct oplus_chg_chip *chip)
{
	...
	chip->limits.default_input_current_wrap_ma_high
			= chip->limits.input_current_wrap_ma_high;
	chip->limits.default_input_current_wrap_ma_warm
			= chip->limits.input_current_wrap_ma_warm;
	chip->limits.default_input_current_wrap_ma_normal
			= chip->limits.input_current_wrap_ma_normal;
	...
	charger_xlog_printk(CHG_LOG_CRTI,
			...
			chip->limits.charger_current_wrap_ma_normal = %d, \
			chip->ffc_support = %d\
			chip->dual_ffc = %d\
...
void oplus_chg_set_input_current_limit(struct oplus_chg_chip *chip)
{
	...
	is_mcu_fastchg = (oplus_wrap_get_fastchg_started()
					&&(chip->vbatt_num != 2 || oplus_wrap_get_fast_chg_type() != CHARGER_SUBTYPE_FASTCHG_WRAP));
	...
	if (chip->chg_ctrl_by_wrap && chip->vbatt_num == 2
				&& oplus_wrap_get_fast_chg_type() == CHARGER_SUBTYPE_FASTCHG_WRAP
				&& oplus_wrap_get_fastchg_started() == true) {
		if(chip->led_on) {
			if(chip->wrap_temp_status == WRAP_TEMP_STATUS__HIGH) {
				current_limit = chip->limits.input_current_wrap_led_ma_high;
			} else if(chip->wrap_temp_status == WRAP_TEMP_STATUS__WARM) {
				current_limit = chip->limits.input_current_wrap_led_ma_warm;
			} else if(chip->wrap_temp_status == WRAP_TEMP_STATUS__NORMAL) {
				current_limit = chip->limits.input_current_wrap_led_ma_normal;
			}
		} else if (!(chip->chg_ctrl_by_calling && chip->calling_on)) {
			if(chip->wrap_temp_status == WRAP_TEMP_STATUS__HIGH) {
				current_limit = chip->limits.input_current_wrap_ma_high;
			} else if(chip->wrap_temp_status == WRAP_TEMP_STATUS__WARM) {
				current_limit = chip->limits.input_current_wrap_ma_warm;
			} else if(chip->wrap_temp_status == WRAP_TEMP_STATUS__NORMAL) {
				current_limit = chip->limits.input_current_wrap_ma_normal;
			}
		}
		chg_err("chg_ctrl_by_wrap,  \
				led_on = %d,\
				calling_on = %d,\
				current_limit[%d], \
				chip->wrap_temp_status[%d]\n",
				chip->led_on,
				chip->calling_on,
				current_limit,
				chip->wrap_temp_status);
		if ( chip->chg_ops->input_current_ctrl_by_wrap_write) {
			chip->chg_ops->input_current_ctrl_by_wrap_write(current_limit);
			return;
		}
	}
...
static void oplus_chg_vfloat_over_check(struct oplus_chg_chip *chip)
{
	...
	if (oplus_wrap_get_fastchg_ing() == true) {
		return;
	}
#ifndef CONFIG_OPLUS_CHARGER_MTK
...
#endif
	//if (!((oplus_wrap_get_fastchg_to_normal()== true) || (oplus_wrap_get_fastchg_to_warm() == true))) {
...
#endif
#endif
	if (oplus_wrap_get_adapter_update_status() == ADAPTER_FW_NEED_UPDATE) {
		return true;
	}
	if (oplus_wrap_get_fastchg_started() == true) {
		return true;
...
static void oplus_chg_check_led_on_ichging(struct oplus_chg_chip *chip)
{
	if (chip->led_on_change || (chip->led_on && chip->led_temp_change)) {
		...
		if (chip->chg_ctrl_by_wrap && chip->vbatt_num == 2) {
			if (oplus_wrap_get_fastchg_started() == true
					&& oplus_wrap_get_fast_chg_type()
					!= CHARGER_SUBTYPE_FASTCHG_WRAP) {
				return;
			}
			if (oplus_wrap_get_allow_reading() == true
					|| oplus_wrap_get_fast_chg_type()
...
#define TWRAP_COUNTS	2
#define TWRAP_HYSTERISIS_DECIDEGC	10

static void oplus_chg_check_wrap_temp_status(struct oplus_chg_chip *chip)
{
	int batt_temp = chip->temperature;
	OPLUS_CHG_TBAT_WRAP_STATUS tbat_wrap_status = chip->wrap_temp_status;
	static int high_counts = 0, warm_counts = 0, normal_counts = 0;
	static bool wrap_first_set_input_current_flag = false;

	if (chip->vbatt_num != 2
			|| oplus_wrap_get_fast_chg_type() != CHARGER_SUBTYPE_FASTCHG_WRAP
			|| oplus_wrap_get_fastchg_started() == false) {
		wrap_first_set_input_current_flag = false;
		return;
	}
	if (batt_temp > chip->limits.wrap_high_bat_decidegc) {			/*>45C*/
		if (oplus_wrap_get_fastchg_started() == true) {
				chg_err("tbatt > 45, quick out wrap");
				//oplus_chg_set_chargerid_switch_val(0);
				//oplus_wrap_switch_mode(NORMAL_CHARGER_MODE);
		}
	} else if (batt_temp > chip->limits.wrap_warm_bat_decidegc_antishake) {	/*>38C && <= 45*/
		high_counts ++;
		if(high_counts >= TWRAP_COUNTS)
		{
			high_counts = 0;
			warm_counts = 0;
			normal_counts = 0;
			tbat_wrap_status = WRAP_TEMP_STATUS__HIGH;
		}
	 } else if (batt_temp >= chip->limits.wrap_normal_bat_decidegc_antishake) { /*>34C && <= 38*/
		warm_counts ++;
		if (warm_counts >= TWRAP_COUNTS)
		{
			high_counts = 0;
			warm_counts = 0;
			normal_counts = 0;
			tbat_wrap_status = WRAP_TEMP_STATUS__WARM;
		}
	} else {								// < 34
		normal_counts ++;
		if (normal_counts >= TWRAP_COUNTS)
		{
			high_counts = 0;
			warm_counts = 0;
			normal_counts = 0;
			tbat_wrap_status = WRAP_TEMP_STATUS__NORMAL;
		}
	}
	chg_err("tbat_wrap_status[%d],chip->wrap_temp_status[%d] ",
		tbat_wrap_status, chip->wrap_temp_status);
	if (wrap_first_set_input_current_flag == false) {
		chip->limits.temp_little_cool_fastchg_current_ma
			= chip->limits.charger_current_wrap_ma_normal;
		chip->limits.temp_normal_fastchg_current_ma
			= chip->limits.charger_current_wrap_ma_normal;
		oplus_chg_set_charging_current(chip);
		chg_err("set charger current ctrl by wrap[%d]\n",
			chip->limits.temp_little_cool_fastchg_current_ma);
	}
	if (tbat_wrap_status != chip->wrap_temp_status
			|| wrap_first_set_input_current_flag == false) {
		chip->limits.wrap_warm_bat_decidegc_antishake
			= chip->limits.wrap_warm_bat_decidegc;
		chip->limits.wrap_normal_bat_decidegc_antishake
			= chip->limits.wrap_normal_bat_decidegc;
		if (tbat_wrap_status > chip->wrap_temp_status
				&& tbat_wrap_status == WRAP_TEMP_STATUS__WARM) {
			chip->limits.wrap_normal_bat_decidegc_antishake
				= chip->limits.wrap_normal_bat_decidegc - TWRAP_HYSTERISIS_DECIDEGC;
		} else if (tbat_wrap_status > chip->wrap_temp_status
				&& tbat_wrap_status == WRAP_TEMP_STATUS__HIGH) {
			chip->limits.wrap_warm_bat_decidegc_antishake
				= chip->limits.wrap_warm_bat_decidegc - TWRAP_HYSTERISIS_DECIDEGC;
		} else if (tbat_wrap_status < chip->wrap_temp_status
				&& tbat_wrap_status == WRAP_TEMP_STATUS__NORMAL) {
			chip->limits.wrap_normal_bat_decidegc_antishake
				= chip->limits.wrap_normal_bat_decidegc + TWRAP_HYSTERISIS_DECIDEGC;
		...
		chip->limits.wrap_warm_bat_decidegc_antishake,
			chip->limits.wrap_normal_bat_decidegc_antishake);
		wrap_first_set_input_current_flag = true;
		chip->wrap_temp_change = true;
		chip->wrap_temp_status = tbat_wrap_status;
	}

}

static void oplus_chg_check_wrap_ichging(struct oplus_chg_chip *chip)
{
	if (chip->vbatt_num == 2 && chip->wrap_temp_change) {
		chip->wrap_temp_change = false;
		if (oplus_wrap_get_fast_chg_type() == CHARGER_SUBTYPE_FASTCHG_WRAP) {
			oplus_chg_set_input_current_limit(chip);
		}
	}
}
...
void oplus_chg_variables_reset(struct oplus_chg_chip *chip, bool in)
{
	if (in) {
		...
		if(!oplus_chg_show_wrap_logo_ornot()) {
	...
	chip->chg_ctrl_by_wrap = chip->chg_ctrl_by_wrap_default;
	chip->ffc_temp_status = FFC_TEMP_STATUS__NORMAL;
	chip->wrap_temp_status = WRAP_TEMP_STATUS__NORMAL;
...
//	chip->anti_shake_bound.overtemp_bound = chip->limits.overtemp_bat_decidegc;
	...
	chip->em_mode = false;
	chip->swrap_detach_time = 0;
	chip->swrap_detect_time = 0;
	...
	if (chip->chg_ops->check_chrdet_status()) {
		...
		if (chip->charger_type == POWER_SUPPLY_TYPE_UNKNOWN) {
			...
			if (oplus_wrap_get_fastchg_to_normal() == true
				...
				if (oplus_wrap_get_reset_adapter_st()) {
			...
			} else if (oplus_wrap_get_fastchg_started() == false
...
static void oplus_chg_get_battery_data(struct oplus_chg_chip *chip)
{
	...
	if ((chip->chg_ops->get_charger_current && oplus_wrap_get_allow_reading() == true)
...
static void oplus_chg_check_aicl_input_limit(struct oplus_chg_chip *chip)
{
	...
	if (chip->charging_state == CHARGING_STATUS_FAIL || chip->batt_full == true
			|| ((chip->tbatt_status != BATTERY_STATUS__NORMAL)
			&& (chip->tbatt_status != BATTERY_STATUS__LITTLE_COOL_TEMP))
			|| chip->ui_soc > 85 || oplus_wrap_get_fastchg_started() == true) {
		aicl_delay_count = 0;
		return;
	}
...
static void oplus_chg_protection_check(struct oplus_chg_chip *chip)
{
	...
	if (chip->chg_ctrl_by_wrap) {
		oplus_chg_check_wrap_temp_status(chip);
		oplus_chg_check_wrap_ichging(chip);
	}
	...
	if (chip->charger_exist && chip->batt_exist && chip->batt_full && chip->mmi_chg && (chip->stop_chg == 1 || chip->charger_type == 5)) {
		...
		if (chip->smooth_soc == chip->ui_soc) {
			...
			if (soc_down_count >= soc_down_limit) {
				soc_down_count = 0;
				if(oplus_chg_show_wrap_logo_ornot() == false
					|| (chip->wrap_show_ui_soc_decimal == false || !chip->decimal_control)) {
					chip->ui_soc--;
				}
			}
		}
		...
		oplus_wrap_get_fastchg_started(), oplus_wrap_get_fast_chg_type());
		if (chip->ui_soc == 100
			&& chip->fastchg_to_ffc == false
			&& (oplus_wrap_get_fastchg_started() == false
			|| oplus_wrap_get_fast_chg_type() == CHARGER_SUBTYPE_FASTCHG_WRAP)) {
...
static void oplus_chg_get_chargerid_voltage(struct oplus_chg_chip *chip)
{
	...
	if (oplus_wrap_get_wrap_switch_val() == 1) {
	...
	} else if (oplus_wrap_get_wrap_switch_val() == 0) {
		if (chip->chargerid_volt_got == false) {
			chip->chg_ops->set_chargerid_switch_val(1);
			oplus_wrap_set_wrap_chargerid_switch_val(1);
#ifdef CONFIG_OPLUS_CHARGER_MTK
			usleep_range(100000, 110000);
#else
			usleep_range(20000, 22000);
			...
			chip->chargerid_volt = chip->chg_ops->get_chargerid_volt();
			chip->chargerid_volt_got = true;
			oplus_wrap_set_wrap_chargerid_switch_val(0);
			if (chip->wrap_project == false) {
...
/*10 seconds after typec init, PD starts to communicate, so set true force reset adapter.only for MTK*/
	...
	if (oplus_wrap_get_abnormal_adapter_current_cnt() > 0
			&& chip->abnormal_adapter_dis_cnt >= oplus_wrap_get_abnormal_adapter_current_cnt()) {
		charger_xlog_printk(CHG_LOG_CRTI, " abnormal adapter dis cnt >= wrap_max,return\n");
		return;
	}
	...
	if (chip->charger_type == POWER_SUPPLY_TYPE_USB_DCP) {
		...
		if (oplus_wrap_get_fastchg_started() == false
			...
			oplus_wrap_switch_fast_chg();
		}
		if (!oplus_wrap_get_fastchg_started()
				&& !oplus_wrap_get_fastchg_dummy_started()
				&& !oplus_wrap_get_fastchg_to_normal()
				&& !oplus_wrap_get_fastchg_to_warm()) {
			if (suspend_charger) {
				reset_mcu_delay = RESET_MCU_DELAY_15S;
				suspend_charger = false;
				oplus_wrap_set_ap_clk_high();
	...
	if (chip->boot_mode == KERNEL_POWER_OFF_CHARGING_BOOT
		...
		if ((chip->chg_ops->check_chrdet_status() == false)
			...
			if ((chip->chg_ops->check_chrdet_status() == false)
				...
				if ((oplus_wrap_get_fastchg_to_normal() == false)
						&& (oplus_wrap_get_fastchg_to_warm() == false)
						&& (oplus_wrap_get_adapter_update_status() != ADAPTER_FW_NEED_UPDATE)
						&& (oplus_wrap_get_btb_temp_over() == false)) {
					charger_xlog_printk(CHG_LOG_CRTI,
...
static void oplus_chg_critical_log(struct oplus_chg_chip *chip)
{
	...
	} else if (oplus_wrap_get_btb_temp_over() == true
			|| oplus_wrap_get_fastchg_to_normal() == true) {
...
static void oplus_chg_ibatt_check_and_set(struct oplus_chg_chip *chip)
{
	...
	if (!chip->charger_exist || (oplus_wrap_get_fastchg_started() == true)) {
		current_adapt = 0;
		ibatt_count = 0;
		average_current = 0;
		fail_count = 0;
...
static void oplus_chg_pd_config(struct oplus_chg_chip *chip)
{
	...
	if (chip->pd_chging == false && chip->chg_ops->oplus_chg_get_pd_type() == true
			&& oplus_chg_show_wrap_logo_ornot() == false) {
...
static void oplus_chg_pdqc_to_normal(struct oplus_chg_chip *chip)
{
	...
	if(oplus_chg_show_wrap_logo_ornot() == true){
		return;
	}
	if (chip->charger_volt > 7500) {
		pdqc_9v = true;
...
int oplus_is_wrap_project(void)
{
	if (!g_charger_chip) {
		return 0;
	} else {
		return g_charger_chip->wrap_project;
	}
}

int oplus_chg_show_wrap_logo_ornot(void)
{
	if (!g_charger_chip) {
		return 0;
	}
	...
	if (g_charger_chip->wireless_support
			&& oplus_wpc_get_adapter_type() == CHARGER_SUBTYPE_FASTCHG_SWRAP
		...
		return 1;
#endif
	if (g_charger_chip->chg_ctrl_by_wrap) {
		if (oplus_wrap_get_fastchg_started() == true
				&& oplus_wrap_get_fast_chg_type() == CHARGER_SUBTYPE_FASTCHG_WRAP) {
			//chg_err("show_wrap_logo_ornot by wrap\n");
	...
	if (oplus_wrap_get_fastchg_started()) {
		return 1;
	} else if (oplus_wrap_get_fastchg_to_normal() == true
			|| oplus_wrap_get_fastchg_to_warm() == true
			|| oplus_wrap_get_fastchg_dummy_started() == true
			|| oplus_wrap_get_adapter_update_status() == ADAPTER_FW_NEED_UPDATE) {
		if (g_charger_chip->prop_status != POWER_SUPPLY_STATUS_FULL
...
struct current_level SWRAP_2_0[] = {
...
struct current_level SWRAP_1_0[] = {
...
int oplus_convert_current_to_level(struct oplus_chg_chip *chip, int val)
{
	...
	reply_bits = oplus_wrap_get_reply_bits();
	if(!val || !reply_bits) {
		return level;
	}
	if(reply_bits == 7) {
		if(chip->vbatt_num == 2) {
			level = find_level_to_current(val, SWRAP_2_0, ARRAY_SIZE(SWRAP_2_0));
		} else {
			level = find_level_to_current(val, SWRAP_1_0, ARRAY_SIZE(SWRAP_1_0));
		}
	} else {
		level = val / 1000;
	}
...
void oplus_smart_charge_by_shell_temp(struct oplus_chg_chip *chip, int val) {
	...
	if (!val) {
		if(chip->cool_down != 0){
			...
			chip->limits.input_current_wrap_ma_high = chip->limits.default_input_current_wrap_ma_high;
			chip->limits.input_current_wrap_ma_warm = chip->limits.default_input_current_wrap_ma_warm;
			chip->limits.input_current_wrap_ma_normal = chip->limits.default_input_current_wrap_ma_normal;
	...
	case CHARGER_SUBTYPE_DEFAULT:
		break;
	case CHARGER_SUBTYPE_FASTCHG_WRAP:
		chip->limits.input_current_wrap_ma_high = val;
		chip->limits.input_current_wrap_ma_warm = val;
		chip->limits.input_current_wrap_ma_normal = val;
		chip->limits.input_current_cool_down_ma = val;
		chip->cool_down_done = true;
		chip->cool_down_force_5v = true;
		chip->chg_ctrl_by_cool_down = true;
		break;
	case CHARGER_SUBTYPE_FASTCHG_SWRAP:
...
static const int cool_down_current_limit_wrap[6] = {1000, 1500, 2000, 2500, 3000, 3500};
void oplus_smart_charge_by_cool_down(struct oplus_chg_chip *chip, int val)
{
	...
	if (val > 6) {
		...
		chip->limits.input_current_wrap_ma_high = cool_down_current_limit_wrap[6 - 1];
		chip->limits.input_current_wrap_ma_warm = cool_down_current_limit_wrap[6 - 1];
		chip->limits.input_current_wrap_ma_normal = cool_down_current_limit_wrap[6 - 1];
		chip->cool_down_done = true;
	} else {
		...
		chip->limits.input_current_wrap_ma_high = cool_down_current_limit_wrap[val - 1];
		chip->limits.input_current_wrap_ma_warm = cool_down_current_limit_wrap[val - 1];
		chip->limits.input_current_wrap_ma_normal = cool_down_current_limit_wrap[val - 1];
		chip->cool_down_done = true;
	}
...
void oplus_chg_set_input_current_without_aicl(int current_ma)
{
    if (!g_charger_chip) {
        ...
        if(g_charger_chip->chg_ops->input_current_write_without_aicl && oplus_wrap_get_allow_reading() == true) {
...
void oplus_chg_config_charger_vsys_threshold(int val)
{
    if (!g_charger_chip) {
        ...
        if(g_charger_chip->chg_ops->set_charger_vsys_threshold && oplus_wrap_get_allow_reading() == true) {
...
void oplus_chg_enable_burst_mode(bool enable)
{
    if (!g_charger_chip) {
        ...
        if(g_charger_chip->chg_ops->enable_burst_mode && oplus_wrap_get_allow_reading() == true) {
...
int oplus_chg_get_fast_chg_type(void)
{
	...
	type = oplus_wrap_get_fast_chg_type();
...
#ifndef CONFIG_OPLUS_CHARGER_MTK
	if (type == 0) {
		if (oplus_wpc_get_adapter_type() == CHARGER_SUBTYPE_FASTCHG_WRAP
				|| oplus_wpc_get_adapter_type() == CHARGER_SUBTYPE_FASTCHG_SWRAP)
...
void oplus_chg_check_break(int vbus_rising)
{
	...
	if(vbus_rising == 1) {
		chip->swrap_detect_time = local_clock() / 1000000;
		oplus_chg_check_abnormal_adapter(vbus_rising);
		if(((chip->swrap_detect_time - chip->swrap_detach_time) <= 1000) && oplus_wrap_get_detach_unexpectly()
			&& chip->temperature <= 440 && chip->temperature >= 45) {
			chip->detect_detach_unexpeactly = 1;
			if(chip->swrap_disconnect_count < 3) {
				chip->swrap_disconnect_count += 1;
			}
			chg_err("swrap_disconnect_count = %d\n", chip->swrap_disconnect_count);
		} else if (((chip->swrap_detect_time - chip->swrap_detach_time) <= 100)) {
			chip->detect_detach_unexpeactly = 2;
		} else {
			chip->swrap_disconnect_count = 0;
		}
		chg_err("swrap_detect_time = %d, chg->swrap_detach_time = %d\n",
			chip->swrap_detect_time, chip->swrap_detach_time);
		chip->swrap_detect_time = 0;
		chip->swrap_detach_time = 0;
	} else {
		chip->swrap_detach_time = local_clock() / 1000000;
		oplus_chg_check_abnormal_adapter(vbus_rising);
		chg_err("swrap_detach_time = %d\n", chip->swrap_detach_time);
	}
	return;
}
...
static void oplus_chg_check_abnormal_adapter(int vbus_rising)
{
	...
	static bool pre_pd_swrap = false;
	...
	if (!vbus_rising) {
		...
		if (!chip->mmi_chg
				...
				|| oplus_wrap_get_fastchg_dummy_started()
				|| (oplus_wrap_get_adapter_update_status() == ADAPTER_FW_NEED_UPDATE)
				|| oplus_wrap_get_btb_temp_over()
				|| !oplus_wrap_get_fastchg_ing()
				|| oplus_wrap_get_detach_unexpectly()) {
			pre_is_abnormal_adapter = false;
		}

		if (pre_pd_swrap && pre_is_abnormal_adapter) {
			chip->icon_debounce = true;
		} else {
			chip->icon_debounce = false;
		}
		chg_err("vbus_down [%d %d %d %d %d]\n",
			pre_pd_swrap, pre_is_abnormal_adapter,
	...
	if (chip->mmi_chg
			...
			&& (oplus_wrap_get_detach_unexpectly() || oplus_wrap_get_fastchg_ing())
			&& (chip->swrap_detect_time - chip->swrap_detach_time) <= 1500) {
		chip->abnormal_adapter_dis_cnt++;
	} else {
		chip->abnormal_adapter_dis_cnt = 0;
		chip->icon_debounce = false;
