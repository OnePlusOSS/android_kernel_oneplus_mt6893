		...
		char *buf)
{
	...
	val = oplus_chg_show_wrap_logo_ornot();
		...
		char *buf)
{
	...
	return sprintf(buf, "%d\n", oplus_wrap_get_adapter_update_status());
}
...
static ssize_t wrapchg_ing_show(struct device *dev, struct device_attribute *attr,
		char *buf)
{
	int val = 0;
	struct oplus_chg_chip *chip = NULL;
	...
	val = oplus_wrap_get_fastchg_ing();
...
static DEVICE_ATTR_RO(wrapchg_ing);
