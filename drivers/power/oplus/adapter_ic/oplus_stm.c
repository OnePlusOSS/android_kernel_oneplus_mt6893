...
static void wrap_uart_gpio_set_value(unsigned long pin, bool value)
{
	gpio_set_value(pin, value);
}

static int wrap_uart_gpio_get_value(unsigned long pin)
{
        return gpio_get_value(pin);
}

static void wrap_uart_tx_bit(unsigned char tx_data)
{
        ...
        case BIT_START:
                ...
                wrap_uart_gpio_set_value(chip->uart_tx_gpio, 0);
                tx_bit = BIT_0;
                break;
        case BIT_0:
        case BIT_1:
        ...
        case BIT_7:
                if (tx_data & (1 << tx_bit)) {
                        wrap_uart_gpio_set_value(chip->uart_tx_gpio, 1);
                } else {
                        wrap_uart_gpio_set_value(chip->uart_tx_gpio, 0);
                }
                tx_bit++;
...
static int wrap_uart_rx_bit(void)
{
        ...
        case BIT_IDLE:
                ...
                if (!wrap_uart_gpio_get_value(chip->uart_rx_gpio)) {
                        rx_bit = BIT_0;
...
static void wrap_uart_tx_byte(unsigned char tx_data)
{
		struct chip_stm *chip = the_chip;

		chip->timer_delay = 51;
        while (1) {
                wrap_uart_tx_bit(tx_data);
...
static unsigned char wrap_uart_rx_byte(unsigned int cmd)
{
...
static void wrap_uart_irq_fiq_enable(bool enable)
{
#if 0
        if (enable) {
                preempt_enable();
...
static int wrap_uart_write_some_addr(u8 *fw_buf, int length)
{
        ...
        while (1) {
                ...
                /*tx: 0xF5*/
                wrap_uart_irq_fiq_enable(false);
                wrap_uart_tx_byte((Write_Addr_Line_Cmd >> 8) & 0xff);
                wrap_uart_irq_fiq_enable(true);

                /*tx: 0x02*/
                wrap_uart_irq_fiq_enable(false);
                wrap_uart_tx_byte(Write_Addr_Line_Cmd & 0xff);
                wrap_uart_irq_fiq_enable(true);

                /*count:16 bytes*/
                wrap_uart_irq_fiq_enable(false);
                wrap_uart_tx_byte(16);
                wrap_uart_irq_fiq_enable(true);
                ...
                wrap_uart_irq_fiq_enable(false);
                wrap_uart_tx_byte((write_addr >> 8) & 0xff);
                wrap_uart_irq_fiq_enable(true);

                wrap_uart_irq_fiq_enable(false);
                wrap_uart_tx_byte(write_addr & 0xff);
                wrap_uart_irq_fiq_enable(true);
                ...
                for (i = 0;i < 16;i++) {
                        wrap_uart_irq_fiq_enable(false);
                        wrap_uart_tx_byte(fw_buf[fw_count]);
                        fw_count++;
                        if (i == 15) {
                                rx_val = wrap_uart_rx_byte(Write_Addr_Line_Cmd);
                        }
                        wrap_uart_irq_fiq_enable(true);
                }
                write_addr += 16;
...
static bool wrap_uart_read_addr_line_and_check(unsigned int addr)
{
        ...
        /*tx:0xF5*/
        wrap_uart_irq_fiq_enable(false);
        wrap_uart_tx_byte((Read_Addr_Line_Cmd >> 8) & 0xff);
        wrap_uart_irq_fiq_enable(true);

        /*tx:0x01*/
        wrap_uart_irq_fiq_enable(false);
        wrap_uart_tx_byte(Read_Addr_Line_Cmd & 0xff);
        wrap_uart_irq_fiq_enable(true);

        /*tx:0x9F*/
        wrap_uart_irq_fiq_enable(false);
        wrap_uart_tx_byte((addr >> 8) & 0xff);
        wrap_uart_irq_fiq_enable(true);

        /*tx:0xF0*/
        wrap_uart_irq_fiq_enable(false);
        wrap_uart_tx_byte(addr & 0xff);

        /*addr(2 bytes) + data(16 bytes)*/
        fw_check_buf[0] = wrap_uart_rx_byte(Read_Addr_Line_Cmd);
        if (chip->rx_timeout) {
                goto  read_addr_line_err;
        }
        fw_check_buf[1] = wrap_uart_rx_byte(Read_Addr_Line_Cmd);
        if (chip->rx_timeout) {
                goto  read_addr_line_err;
        }
        ...
        for (i = 0; i < 16;i++) {
                fw_check_buf[i + 2] = wrap_uart_rx_byte(Read_Addr_Line_Cmd);
                if (chip->rx_timeout) {
                        goto  read_addr_line_err;
                }
        }
...
static int wrap_uart_read_front_addr_and_check(void)
{
        ...
        while (read_addr < STM8S_ADAPTER_LAST_ADDR) {
                result = wrap_uart_read_addr_line_and_check(read_addr);
...
static bool wrap_adapter_update_handle(unsigned long tx_pin, unsigned long rx_pin)
{
        unsigned char rx_val = 0;
...
/*step1: Tx_Erase_Addr_Line*/
        /*tx:0xF5*/
        wrap_uart_irq_fiq_enable(false);
        wrap_uart_tx_byte((Erase_Addr_Line_Cmd >> 8) & 0xff);
        wrap_uart_irq_fiq_enable(true);

        /*tx:0x03*/
        wrap_uart_irq_fiq_enable(false);
        wrap_uart_tx_byte(Erase_Addr_Line_Cmd & 0xff);
        wrap_uart_irq_fiq_enable(true);

        /*tx:0x9F*/
        wrap_uart_irq_fiq_enable(false);
        wrap_uart_tx_byte((Last_Line_Addr >> 8) & 0xff);
        wrap_uart_irq_fiq_enable(true);

        /*tx:0xF0*/
        wrap_uart_irq_fiq_enable(false);
        wrap_uart_tx_byte(Last_Line_Addr & 0xff);
        rx_val = wrap_uart_rx_byte(Erase_Addr_Line_Cmd);
        wrap_uart_irq_fiq_enable(true);
        if (rx_val != UART_ACK || chip->rx_timeout) {
...
/*Step2: Tx_Read_Addr_Line */
        ...
        for (rx_last_line_count = 0; rx_last_line_count < 18;rx_last_line_count++) {
                rx_last_line[rx_last_line_count] = wrap_uart_rx_byte(Read_Addr_Line_Cmd);
                if (chip->rx_timeout) {
                        break;
                }
        }
...
/*Step3: Tx_Erase_All */
        /*tx:0xF5*/
        wrap_uart_irq_fiq_enable(false);
        wrap_uart_tx_byte((Erase_All_Cmd >> 8) & 0xff);
        wrap_uart_irq_fiq_enable(true);

        /*tx:0x05*/
        wrap_uart_irq_fiq_enable(false);
        wrap_uart_tx_byte(Erase_All_Cmd & 0xff);
        rx_val = wrap_uart_rx_byte(Erase_All_Cmd);
        wrap_uart_irq_fiq_enable(true);
        if (rx_val != UART_ACK || chip->rx_timeout) {
...
/* Step4: Tx_Write_Addr_Line */
        rc = wrap_uart_write_some_addr(&adapter_stm8s_firmware_data[0],
...
/* Step5: Tx_Read_All */
        rc = wrap_uart_read_front_addr_and_check();
        if (rc) {
...
/* Step6: write the last line */
        rc = wrap_uart_write_some_addr(&adapter_stm8s_firmware_data[sizeof(adapter_stm8s_firmware_data) - 34], 34);
        if (rc) {
...
/* Step7: Tx_Boot_Over */
        /*tx:0xF5*/
        wrap_uart_irq_fiq_enable(false);
        wrap_uart_tx_byte((Boot_Over_Cmd >> 8) & 0xff);
        wrap_uart_irq_fiq_enable(true);

        /*tx:0x06 */
        wrap_uart_irq_fiq_enable(false);
        wrap_uart_tx_byte(Boot_Over_Cmd & 0xff);
        rx_val = wrap_uart_rx_byte(Boot_Over_Cmd);
        wrap_uart_irq_fiq_enable(true);
        if (rx_val != UART_ACK || chip->rx_timeout) {
...
bool oplus_wrap_adapter_update_is_tx_gpio(unsigned long gpio_num)
{
        if (!the_chip) {
                return false;
        }
...
bool oplus_wrap_adapter_update_is_rx_gpio(unsigned long gpio_num)
{
        if (!the_chip) {
                return false;
        }
...
struct oplus_adapter_operations oplus_adapter_ops = {
        .adapter_update = wrap_adapter_update_handle,
};
...
static int __init adapter_ic_init(void)
{
        ...
        if (!chip) {
                chg_err(" wrap_adapter alloc fail\n");
                return -1;
        }
