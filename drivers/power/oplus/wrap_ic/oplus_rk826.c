...
static int oplus_i2c_dma_read(struct i2c_client *client, u16 addr, s32 len, u8 *rxbuf)
{
	...
	//chg_debug("wrap dma i2c read: 0x%x, %d bytes(s)\n", addr, len);
	for (retry = 0; retry < 5; ++retry) {
		ret = i2c_transfer(client->adapter, &msg[0], 2);
		if (ret < 0) {
...
static int oplus_i2c_dma_write(struct i2c_client *client, u16 addr, s32 len, u8 const *txbuf)
{
	...
	//chg_debug("wrap dma i2c write: 0x%x, %d bytes(s)\n", addr, len);
	for (retry = 0; retry < 5; ++retry) {
		ret = i2c_transfer(client->adapter, &msg, 1);
		if (ret < 0) {
			continue;
...
static bool rk826_fw_update_check(struct oplus_wrap_chip *chip)
{
	int ret = 0;
	u8 data_buf[4]= {0};
...
int WriteSram(struct oplus_wrap_chip *chip, const u8 *buf, u32 size)
{
	u8 offset = 0;
	u16 reg_addr;
	int ret = 0;
...
int Download_00_code(struct oplus_wrap_chip *chip)
{
...
int Download_ff_code(struct oplus_wrap_chip *chip)
{
	u8 transfer_buf[TRANSFER_LIMIT];
	u32 onetime_size = TRANSFER_LIMIT - 8;
	u32 index = 0;
...
static int rk826_fw_write_00_code(struct oplus_wrap_chip *chip)
{
	int ret = 0;
	int iTryCount = 3;
	struct_req req = {0};
...
update_fw_err:
	charger_abnormal_log = CRITICAL_LOG_WRAP_FW_UPDATE_ERR;
	chg_err("fail\n");
	return 1;
}
static int rk826_fw_write_ff_code(struct oplus_wrap_chip *chip)
{
	int ret = 0;
	int iTryCount = 3;
	struct_req req = {0};
...
int DownloadFirmware(struct oplus_wrap_chip *chip, const u8 *buf, u32 size)
{
	u8 transfer_buf[TRANSFER_LIMIT];
	u32 onetime_size = TRANSFER_LIMIT - 8;
	u32 index = 0;
...
static int rk826_fw_update(struct oplus_wrap_chip *chip)
{
	int ret = 0;
	int iTryCount = 3;
	struct_req req = {0};
...
static int rk826_get_fw_verion_from_ic(struct oplus_wrap_chip *chip)
{
...
static int rk826_fw_check_then_recover(struct oplus_wrap_chip *chip)
{
	int update_result = 0;
	int try_count = 5;
	int ret = 0;
...
static int rk826_fw_check_then_recover_fix(struct oplus_wrap_chip *chip)
{
	int update_result = 0;
	int try_count = 5;
	int ret = 0;
...
int rk826_asic_fw_status(struct oplus_wrap_chip *chip)
{
	u32 force_dis_update_flag = 0x00000000;
	u32 sw_reset_flag = SW_RESET_FLAG;
	u8 value_buf[4] = {0};
...
void rk826_update_temperature_soc(void)
{
	...
	chg_err("kilody in! soc = %d,temp = %d,chging = %d\n", soc, temp, oplus_wrap_get_fastchg_ing());
}

struct oplus_wrap_operations oplus_rk826_ops = {
	...
	.set_wrap_chargerid_switch_val = opchg_set_wrap_chargerid_switch_val,
...
/* Jiaoyang@CM.BSP.CHG 2020/10/17 add for sala_A SWRAP charging */
...
static int rk826_parse_fw_from_dt(struct oplus_wrap_chip *chip)
{
...
static int rk826_parse_fw_from_array(struct oplus_wrap_chip *chip)
{
	if (chip->batt_type_4400mv) {
		chip->firmware_data = rk826_fw_data_4400_wrap_ffc_15c;
		chip->fw_data_count = sizeof(rk826_fw_data_4400_wrap_ffc_15c);
		chip->fw_data_version = rk826_fw_data_4400_wrap_ffc_15c[chip->fw_data_count - 4];
	} else {
		chip->firmware_data = rk826_fw_data_4400_wrap_ffc_15c;
		chip->fw_data_count = sizeof(rk826_fw_data_4400_wrap_ffc_15c);
		chip->fw_data_version = rk826_fw_data_4400_wrap_ffc_15c[chip->fw_data_count - 4];
	...
	if (chip->wrap_fw_type == WRAP_FW_TYPE_RK826_4400_WRAP_FFC_15C) {
		chip->firmware_data = rk826_fw_data_4400_wrap_ffc_15c;
		chip->fw_data_count = sizeof(rk826_fw_data_4400_wrap_ffc_15c);
		chip->fw_data_version = rk826_fw_data_4400_wrap_ffc_15c[chip->fw_data_count - 4];
	} else if (chip->wrap_fw_type == WRAP_FW_TYPE_RK826_4450_WRAP_FFC_5V6A_4BIT) {
		chip->firmware_data = rk826_fw_data_4450_wrap_ffc_5v6a_4bit;
		chip->fw_data_count = sizeof(rk826_fw_data_4450_wrap_ffc_5v6a_4bit);
		chip->fw_data_version = rk826_fw_data_4450_wrap_ffc_5v6a_4bit[chip->fw_data_count - 4];
	} else if (chip->wrap_fw_type == WRAP_FW_TYPE_RK826_4450_SWRAP_FFC_5V6A_CHAKA) {
		chip->firmware_data = rk826_fw_data_4450_swrap_ffc_5v6a_chaka;
		chip->fw_data_count = sizeof(rk826_fw_data_4450_swrap_ffc_5v6a_chaka);
		chip->fw_data_version = rk826_fw_data_4450_swrap_ffc_5v6a_chaka[chip->fw_data_count - 4];
	} else if (chip->wrap_fw_type == WRAP_FW_TYPE_RK826_4450_SWRAP_FFC_6300MA_HIMA) {
		chip->firmware_data = rk826_fw_data_4450_swrap_ffc_6300mA_hima;
		chip->fw_data_count = sizeof(rk826_fw_data_4450_swrap_ffc_6300mA_hima);
		chip->fw_data_version = rk826_fw_data_4450_swrap_ffc_6300mA_hima[chip->fw_data_count - 4];
	} else if (chip->wrap_fw_type == WRAP_FW_TYPE_RK826_4450_SWRAP_FFC_6300MA_HOREE) {
		chip->firmware_data = rk826_fw_data_4450_swrap_ffc_6300mA_horee;
		chip->fw_data_count = sizeof(rk826_fw_data_4450_swrap_ffc_6300mA_horee);
		chip->fw_data_version = rk826_fw_data_4450_swrap_ffc_6300mA_horee[chip->fw_data_count - 4];
	} else if (chip->wrap_fw_type == WRAP_FW_TYPE_RK826_4450_SWRAP_6300MA && !get_hwpcb_version()) {
		chip->firmware_data = rk826_fw_data_4450_swrap_6300ma;
		chip->fw_data_count = sizeof(rk826_fw_data_4450_swrap_6300ma);
		chip->fw_data_version = rk826_fw_data_4450_swrap_6300ma[chip->fw_data_count - 4];
	} else if (chip->wrap_fw_type == WRAP_FW_TYPE_RK826_4450_SWRAP_6300MA && get_hwpcb_version()) {
		chip->firmware_data = rk826_fw_data_4450_swrap_evt2_4500ma;
		chip->fw_data_count = sizeof(rk826_fw_data_4450_swrap_evt2_4500ma);
		chip->fw_data_version = rk826_fw_data_4450_swrap_evt2_4500ma[chip->fw_data_count - 4];
	} else if (chip->wrap_fw_type == WRAP_FW_TYPE_RK826_4450_SWRAP_FFC_5V6A_DALI) {
		chip->firmware_data = rk826_fw_data_4450_swrap_ffc_5v6a_dali;
		chip->fw_data_count = sizeof(rk826_fw_data_4450_swrap_ffc_5v6a_dali);
		chip->fw_data_version = rk826_fw_data_4450_swrap_ffc_5v6a_dali[chip->fw_data_count - 4];
	}
...
static ssize_t wrap_fw_check_read(struct file *filp, char __user *buff, size_t count, loff_t *off)
{
	char page[256] = {0};
	char read_data[32] = {0};
	int len = 0;
...
#endif
#endif
	if (get_wrap_mcu_type(chip) != OPLUS_WRAP_ASIC_HWID_RK826) {
...
MODULE_DESCRIPTION("Driver for oplus wrap rk826 fast mcu");
