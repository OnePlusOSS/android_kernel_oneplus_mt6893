...
static int oplus_wrap_i2c_write(struct i2c_client *client, u8 addr, s32 len, u8 *txbuf)
{
#ifdef CONFIG_OPLUS_CHARGER_MTK
...
static bool pic16f_fw_check(struct oplus_wrap_chip *chip)
{
...
static void pic16f_fw_data_recover(struct oplus_wrap_chip *chip,unsigned char *data_buf,
...
static int pic16f_fw_write(struct oplus_wrap_chip *chip,unsigned char *data_buf,unsigned int offset,unsigned int length)
{
	unsigned int count = 0;
...
static int pic16f_fw_update(struct oplus_wrap_chip *chip)
{
	unsigned char zero_buf[1] = {0};
	unsigned char addr_buf[2] = {0x02,0x00};
	unsigned char temp_buf[1]={0};
...
static int pic16f_fw_check_then_recover(struct oplus_wrap_chip *chip)
{
	int update_result = 0;
	int ret = 0;
...
struct oplus_wrap_operations oplus_pic16f_ops = {
...
MODULE_DESCRIPTION("Driver for oplus wrap pic16f fast mcu");
