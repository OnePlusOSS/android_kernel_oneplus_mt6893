...
#include "oplus_wrap_fw.h"

int g_hw_version = 0;
void oplus_wrap_data_irq_init(struct oplus_wrap_chip *chip);
...
int get_wrap_mcu_type(struct oplus_wrap_chip *chip)
{
	...
	return oplus_wrap_mcu_hwid_check(chip);
}

static int opchg_bq27541_gpio_pinctrl_init(struct oplus_wrap_chip *chip)
{
	chip->wrap_gpio.pinctrl = devm_pinctrl_get(chip->dev);
	if (IS_ERR_OR_NULL(chip->wrap_gpio.pinctrl)) {
		chg_err(": %d Getting pinctrl handle failed\n", __LINE__);
		return -EINVAL;
	}
	/* set switch1 is active and switch2 is active*/
	if (1) {
		chip->wrap_gpio.gpio_switch1_act_switch2_act =
			pinctrl_lookup_state(chip->wrap_gpio.pinctrl, "switch1_act_switch3_act");
		if (IS_ERR_OR_NULL(chip->wrap_gpio.gpio_switch1_act_switch2_act)) {
			chg_err(": %d Failed to get the active state pinctrl handle\n", __LINE__);
			return -EINVAL;
		}
		/* set switch1 is sleep and switch2 is sleep*/
		chip->wrap_gpio.gpio_switch1_sleep_switch2_sleep =
			pinctrl_lookup_state(chip->wrap_gpio.pinctrl, "switch1_sleep_switch3_sleep");
		if (IS_ERR_OR_NULL(chip->wrap_gpio.gpio_switch1_sleep_switch2_sleep)) {
			chg_err(": %d Failed to get the suspend state pinctrl handle\n", __LINE__);
			return -EINVAL;
		}

		chip->wrap_gpio.gpio_switch1_ctr1_act =
			pinctrl_lookup_state(chip->wrap_gpio.pinctrl, "charging_switch1_ctr1_active");
		if (IS_ERR_OR_NULL(chip->wrap_gpio.gpio_switch1_ctr1_act)) {
			chg_err(": %d Failed to get the charging_switch1_ctr1_active handle\n", __LINE__);
			//return -EINVAL;
		}

		chip->wrap_gpio.gpio_switch1_ctr1_sleep =
			pinctrl_lookup_state(chip->wrap_gpio.pinctrl, "charging_switch1_ctr1_sleep");
		if (IS_ERR_OR_NULL(chip->wrap_gpio.gpio_switch1_ctr1_sleep)) {
			chg_err(": %d Failed to get the charging_switch1_ctr1_sleep handle\n", __LINE__);
			//return -EINVAL;
		}
	} else {
		chip->wrap_gpio.gpio_switch1_act_switch2_act =
			pinctrl_lookup_state(chip->wrap_gpio.pinctrl, "switch1_act_switch2_act");
		if (IS_ERR_OR_NULL(chip->wrap_gpio.gpio_switch1_act_switch2_act)) {
			chg_err(": %d Failed to get the active state pinctrl handle\n", __LINE__);
			return -EINVAL;
		}
		/* set switch1 is sleep and switch2 is sleep*/
		chip->wrap_gpio.gpio_switch1_sleep_switch2_sleep =
			pinctrl_lookup_state(chip->wrap_gpio.pinctrl, "switch1_sleep_switch2_sleep");
		if (IS_ERR_OR_NULL(chip->wrap_gpio.gpio_switch1_sleep_switch2_sleep)) {
			chg_err(": %d Failed to get the suspend state pinctrl handle\n", __LINE__);
			return -EINVAL;
		}
	...
	chip->wrap_gpio.gpio_switch1_act_switch2_sleep =
		pinctrl_lookup_state(chip->wrap_gpio.pinctrl, "switch1_act_switch2_sleep");
	if (IS_ERR_OR_NULL(chip->wrap_gpio.gpio_switch1_act_switch2_sleep)) {
		chg_err(": %d Failed to get the state 2 pinctrl handle\n", __LINE__);
		return -EINVAL;
	}
	/* set switch1 is sleep and switch2 is active*/
	chip->wrap_gpio.gpio_switch1_sleep_switch2_act =
		pinctrl_lookup_state(chip->wrap_gpio.pinctrl, "switch1_sleep_switch2_act");
	if (IS_ERR_OR_NULL(chip->wrap_gpio.gpio_switch1_sleep_switch2_act)) {
			chg_err(": %d Failed to get the state 3 pinctrl handle\n", __LINE__);
		return -EINVAL;
	}
	/* set clock is active*/
	chip->wrap_gpio.gpio_clock_active =
		pinctrl_lookup_state(chip->wrap_gpio.pinctrl, "clock_active");
	if (IS_ERR_OR_NULL(chip->wrap_gpio.gpio_clock_active)) {
			chg_err(": %d Failed to get the state 3 pinctrl handle\n", __LINE__);
		return -EINVAL;
	}
	/* set clock is sleep*/
	chip->wrap_gpio.gpio_clock_sleep =
		pinctrl_lookup_state(chip->wrap_gpio.pinctrl, "clock_sleep");
	if (IS_ERR_OR_NULL(chip->wrap_gpio.gpio_clock_sleep)) {
		chg_err(": %d Failed to get the state 3 pinctrl handle\n", __LINE__);
		return -EINVAL;
	}
	/* set clock is active*/
	chip->wrap_gpio.gpio_data_active =
		pinctrl_lookup_state(chip->wrap_gpio.pinctrl, "data_active");
	if (IS_ERR_OR_NULL(chip->wrap_gpio.gpio_data_active)) {
		chg_err(": %d Failed to get the state 3 pinctrl handle\n", __LINE__);
		return -EINVAL;
	}
	/* set clock is sleep*/
	chip->wrap_gpio.gpio_data_sleep =
		pinctrl_lookup_state(chip->wrap_gpio.pinctrl, "data_sleep");
	if (IS_ERR_OR_NULL(chip->wrap_gpio.gpio_data_sleep)) {
		chg_err(": %d Failed to get the state 3 pinctrl handle\n", __LINE__);
		return -EINVAL;
	}
	/* set reset is atcive*/
	chip->wrap_gpio.gpio_reset_active =
		pinctrl_lookup_state(chip->wrap_gpio.pinctrl, "reset_active");
	if (IS_ERR_OR_NULL(chip->wrap_gpio.gpio_reset_active)) {
		chg_err(": %d Failed to get the state 3 pinctrl handle\n", __LINE__);
		return -EINVAL;
	}
	/* set reset is sleep*/
	chip->wrap_gpio.gpio_reset_sleep =
		pinctrl_lookup_state(chip->wrap_gpio.pinctrl, "reset_sleep");
	if (IS_ERR_OR_NULL(chip->wrap_gpio.gpio_reset_sleep)) {
		chg_err(": %d Failed to get the state 3 pinctrl handle\n", __LINE__);
		return -EINVAL;
	}
	return 0;
...
void oplus_wrap_fw_type_dt(struct oplus_wrap_chip *chip)
{
	...
	if (chip->wrap_current_lvl_cnt > 0) {
		...
		chip->wrap_current_lvl = devm_kcalloc(chip->dev, chip->wrap_current_lvl_cnt,
			sizeof(*chip->wrap_current_lvl), GFP_KERNEL);
		if (!chip->wrap_current_lvl){
			chg_err("devm_kcalloc wrap_current_lvl error\n");
			return;
		}
		...
		for(loop = 0; loop < chip->wrap_current_lvl_cnt; loop++) {
			chg_err("wrap_current_lvl[%d]\n", chip->wrap_current_lvl[loop]);
		}
	}
	...
	if (rc) {
		...
		chg_debug("qcom,wrap-low-temp is %d\n", chip->wrap_low_temp);
	}
	...
	if (rc) {
		...
		chg_debug("qcom,wrap-little-cool-temp is %d\n", chip->wrap_little_cool_temp);
	}
	...
	if (rc) {
		...
		chg_debug("qcom,wrap-cool-temp is %d\n", chip->wrap_cool_temp);
	}
	...
	if (rc) {
		...
		chg_debug("qcom,wrap-little-cold-temp is %d\n", chip->wrap_little_cold_temp);
	}
	...
	if (rc) {
		...
		chg_debug("qcom,wrap-normal-low-temp is %d\n", chip->wrap_normal_low_temp);
	}
	...
	if (rc) {
		...
		chg_debug("qcom,wrap_warm_bat_suspend_volt is %d\n", chip->wrap_warm_bat_suspend_volt);
	}
	...
	if (rc) {
		...
		chg_debug("qcom,wrap_reply_mcu_bits is %d\n",
			chip->wrap_reply_mcu_bits);
	}
	...
	if (rc) {
		...
		chg_debug("qcom,wrap_multistep_initial_batt_temp is %d\n",
			chip->wrap_multistep_initial_batt_temp);
	}
	...
	if (rc) {
		...
		chg_debug("qcom,wrap_strategy_normal_current is %d\n",
			chip->wrap_strategy_normal_current);
	}
	...
	if (rc) {
		...
		chg_debug("qcom,wrap_strategy1_batt_low_temp1 is %d\n",
			chip->wrap_strategy1_batt_low_temp1);
	}
	...
	if (rc) {
		...
		chg_debug("qcom,wrap_strategy1_batt_low_temp2 is %d\n",
			chip->wrap_strategy1_batt_low_temp2);
	}
	...
	if (rc) {
		...
		chg_debug("qcom,wrap_strategy1_batt_low_temp0 is %d\n",
			chip->wrap_strategy1_batt_low_temp0);
	}
	...
	if (rc) {
		...
		chg_debug("qcom,wrap_strategy1_batt_high_temp0 is %d\n",
			chip->wrap_strategy1_batt_high_temp0);
	}
	...
	if (rc) {
		...
		chg_debug("qcom,wrap_strategy1_batt_high_temp1 is %d\n",
			chip->wrap_strategy1_batt_high_temp1);
	}
	...
	if (rc) {
		...
		chg_debug("qcom,wrap_strategy1_batt_high_temp2 is %d\n",
			chip->wrap_strategy1_batt_high_temp2);
	}
	...
	if (rc) {
		...
		chg_debug("qcom,wrap_strategy1_high_current0 is %d\n",
			chip->wrap_strategy1_high_current0);
	}
	...
	if (rc) {
		...
		chg_debug("qcom,wrap_strategy1_high_current1 is %d\n",
			chip->wrap_strategy1_high_current1);
	}
	...
	if (rc) {
		...
		chg_debug("qcom,wrap_strategy1_high_current2 is %d\n",
			chip->wrap_strategy1_high_current2);
	}
	...
	if (rc) {
		...
		chg_debug("qcom,wrap_strategy1_low_current2 is %d\n",
			chip->wrap_strategy1_low_current2);
	}
	...
	if (rc) {
		...
		chg_debug("qcom,wrap_strategy1_low_current1 is %d\n",
			chip->wrap_strategy1_low_current1);
	}
	...
	if (rc) {
		...
		chg_debug("qcom,wrap_strategy1_low_current0 is %d\n",
			chip->wrap_strategy1_low_current0);
	}
	...
	if (rc) {
		...
		chg_debug("qcom,wrap_strategy2_batt_up_temp1 is %d\n",
			chip->wrap_strategy2_batt_up_temp1);
	}
	...
	if (rc) {
		...
		chg_debug("qcom,wrap_strategy2_batt_up_down_temp2 is %d\n",
			chip->wrap_strategy2_batt_up_down_temp2);
	}
	...
	if (rc) {
		...
		chg_debug("qcom,wrap_strategy2_batt_up_temp3 is %d\n",
			chip->wrap_strategy2_batt_up_temp3);
	}
	...
	if (rc) {
		...
		chg_debug("qcom,wrap_strategy2_batt_up_down_temp4 is %d\n",
			chip->wrap_strategy2_batt_up_down_temp4);
	}
	...
	if (rc) {
		...
		chg_debug("qcom,wrap_strategy2_batt_up_temp5 is %d\n",
			chip->wrap_strategy2_batt_up_temp5);
	}
	...
	if (rc) {
		...
		chg_debug("qcom,wrap_strategy2_batt_up_temp6 is %d\n",
			chip->wrap_strategy2_batt_up_temp6);
	}
	...
	if (rc) {
		...
		chg_debug("qcom,wrap_strategy2_high0_current is %d\n",
			chip->wrap_strategy2_high0_current);
	}
	...
	if (rc) {
		...
		chg_debug("qcom,wrap_strategy2_high1_current is %d\n",
			chip->wrap_strategy2_high1_current);
	}
	...
	if (rc) {
		...
		chg_debug("qcom,wrap_strategy2_high2_current is %d\n",
			chip->wrap_strategy2_high2_current);
	}
	...
	if (rc) {
		...
		chg_debug("qcom,wrap_strategy2_high3_current is %d\n",
			chip->wrap_strategy2_high3_current);
	}
	...
	if (rc) {
		...
		chg_debug("qcom,wrap_batt_over_high_temp is %d\n",
			chip->wrap_batt_over_high_temp);
	}
	...
	if (rc) {
		...
		chg_debug("qcom,wrap_over_high_or_low_current is %d\n",
			chip->wrap_over_high_or_low_current);
	}
...
int oplus_wrap_asic_hwid_check(struct oplus_wrap_chip *chip)
{
	...
	asic_hwid_type = OPLUS_WRAP_ASIC_HWID_OP10;
	return OPLUS_WRAP_ASIC_HWID_OP10;
}

int oplus_wrap_mcu_hwid_check(struct oplus_wrap_chip *chip)
{
	int rc;
...
/*This is only for P60 P(17197 P)*/
	...
	if (chip->wrap_gpio.wrap_mcu_id_gpio < 0) {
		...
		if (gpio_is_valid(chip->wrap_gpio.wrap_mcu_id_gpio)) {
			rc = gpio_request(chip->wrap_gpio.wrap_mcu_id_gpio, "wrap-mcu-id-gpio");
			if (rc) {
				chg_err("unable to request gpio [%d]\n",
					chip->wrap_gpio.wrap_mcu_id_gpio);
			}
		}
		chg_err("chip->wrap_gpio.wrap_mcu_id_gpio =%d\n",
			chip->wrap_gpio.wrap_mcu_id_gpio);
	}
	...
	chip->wrap_gpio.gpio_wrap_mcu_id_default =
		pinctrl_lookup_state(chip->wrap_gpio.pinctrl, "wrap_mcu_id_default");
	if (IS_ERR_OR_NULL(chip->wrap_gpio.gpio_wrap_mcu_id_default)) {
		chg_err(": %d Failed to get the gpio_wrap_mcu_id_default\
	...
	usleep_range(10000, 10000);
	if (gpio_is_valid(chip->wrap_gpio.wrap_mcu_id_gpio)){
		if(gpio_get_value(chip->wrap_gpio.wrap_mcu_id_gpio) == 0){
			chg_debug("it is  n76e\n");
			mcu_hwid_type = OPLUS_WRAP_MCU_HWID_N76E;
			return OPLUS_WRAP_MCU_HWID_N76E;
		}
	}
...
int oplus_wrap_gpio_dt_init(struct oplus_wrap_chip *chip)
{
	int rc = 0;
	struct device_node *node = chip->dev->of_node;

	/* Parsing gpio swutch1*/
	chip->wrap_gpio.switch1_gpio = of_get_named_gpio(node,
		"qcom,charging_switch1-gpio", 0);
	if (chip->wrap_gpio.switch1_gpio < 0) {
		chg_err("chip->wrap_gpio.switch1_gpio not specified\n");
	} else {
		if (gpio_is_valid(chip->wrap_gpio.switch1_gpio)) {
			rc = gpio_request(chip->wrap_gpio.switch1_gpio,
				"charging-switch1-gpio");
			if (rc) {
				chg_err("unable to request gpio [%d]\n",
					chip->wrap_gpio.switch1_gpio);
			}
		}
		chg_err("chip->wrap_gpio.switch1_gpio =%d\n",
			chip->wrap_gpio.switch1_gpio);
	}

	chip->wrap_gpio.switch1_ctr1_gpio = of_get_named_gpio(node, "qcom,charging_switch1_ctr1-gpio", 0);
	if (chip->wrap_gpio.switch1_ctr1_gpio < 0) {
		chg_err("chip->wrap_gpio.switch1_ctr1_gpio not specified\n");
	} else {
		if (gpio_is_valid(chip->wrap_gpio.switch1_ctr1_gpio)) {
			rc = gpio_request(chip->wrap_gpio.switch1_ctr1_gpio, "charging-switch1-ctr1-gpio");
			if (rc) {
				chg_err("unable to request gpio [%d]\n", chip->wrap_gpio.switch1_ctr1_gpio);
			} else {
				gpio_direction_output(chip->wrap_gpio.switch1_ctr1_gpio, 0);
			}
		}
		chg_err("chip->wrap_gpio.switch1_ctr1_gpio =%d\n", chip->wrap_gpio.switch1_ctr1_gpio);
	}
	/* Parsing gpio swutch2*/
	/*if(get_PCB_Version()== 0)*/
	if (1) {
		chip->wrap_gpio.switch2_gpio = of_get_named_gpio(node,
			"qcom,charging_switch3-gpio", 0);
		if (chip->wrap_gpio.switch2_gpio < 0) {
			chg_err("chip->wrap_gpio.switch2_gpio not specified\n");
		} else {
			if (gpio_is_valid(chip->wrap_gpio.switch2_gpio)) {
				rc = gpio_request(chip->wrap_gpio.switch2_gpio,
					"charging-switch3-gpio");
				if (rc) {
					chg_err("unable to request gpio [%d]\n",
						chip->wrap_gpio.switch2_gpio);
				}
			}
			chg_err("chip->wrap_gpio.switch2_gpio =%d\n",
				chip->wrap_gpio.switch2_gpio);
		}
	} else {
		chip->wrap_gpio.switch2_gpio = of_get_named_gpio(node,
	...
	/* Parsing gpio reset*/
	chip->wrap_gpio.reset_gpio = of_get_named_gpio(node,
		"qcom,charging_reset-gpio", 0);
	if (chip->wrap_gpio.reset_gpio < 0) {
		chg_err("chip->wrap_gpio.reset_gpio not specified\n");
	} else {
		if (gpio_is_valid(chip->wrap_gpio.reset_gpio)) {
			rc = gpio_request(chip->wrap_gpio.reset_gpio,
				"charging-reset-gpio");
			if (rc) {
				chg_err("unable to request gpio [%d]\n",
					chip->wrap_gpio.reset_gpio);
			}
		}
		chg_err("chip->wrap_gpio.reset_gpio =%d\n",
			chip->wrap_gpio.reset_gpio);
	}
	/* Parsing gpio clock*/
	chip->wrap_gpio.clock_gpio = of_get_named_gpio(node, "qcom,charging_clock-gpio", 0);
	if (chip->wrap_gpio.clock_gpio < 0) {
		chg_err("chip->wrap_gpio.reset_gpio not specified\n");
	} else {
		if (gpio_is_valid(chip->wrap_gpio.clock_gpio)) {
			rc = gpio_request(chip->wrap_gpio.clock_gpio, "charging-clock-gpio");
			if (rc) {
				chg_err("unable to request gpio [%d], rc = %d\n",
					chip->wrap_gpio.clock_gpio, rc);
			}
		}
		chg_err("chip->wrap_gpio.clock_gpio =%d\n", chip->wrap_gpio.clock_gpio);
	}
	/* Parsing gpio data*/
	chip->wrap_gpio.data_gpio = of_get_named_gpio(node, "qcom,charging_data-gpio", 0);
	if (chip->wrap_gpio.data_gpio < 0) {
		chg_err("chip->wrap_gpio.data_gpio not specified\n");
	} else {
		if (gpio_is_valid(chip->wrap_gpio.data_gpio)) {
			rc = gpio_request(chip->wrap_gpio.data_gpio, "charging-data-gpio");
			if (rc) {
				chg_err("unable to request gpio [%d]\n", chip->wrap_gpio.data_gpio);
			}
		}
		chg_err("chip->wrap_gpio.data_gpio =%d\n", chip->wrap_gpio.data_gpio);
	}
	oplus_wrap_data_irq_init(chip);
	...
	chg_debug(" switch1_gpio = %d,switch2_gpio = %d, reset_gpio = %d,\
			...
			chip->wrap_gpio.switch1_gpio, chip->wrap_gpio.switch2_gpio,
			chip->wrap_gpio.reset_gpio, chip->wrap_gpio.clock_gpio,
			chip->wrap_gpio.data_gpio, chip->wrap_gpio.data_irq);
	return rc;
}

void opchg_set_clock_active(struct oplus_wrap_chip *chip)
{
	...
	pinctrl_select_state(chip->wrap_gpio.pinctrl, chip->wrap_gpio.gpio_clock_sleep); /* PULL_down */
	gpio_direction_output(chip->wrap_gpio.clock_gpio, 0);		/* out 0 */
	mutex_unlock(&chip->pinctrl_mutex);
}

void opchg_set_clock_sleep(struct oplus_wrap_chip *chip)
{
	if (chip->mcu_boot_by_gpio) {
		chg_debug(" mcu_boot_by_gpio,return\n");
		return;
	...
	mutex_lock(&chip->pinctrl_mutex);
	pinctrl_select_state(chip->wrap_gpio.pinctrl, chip->wrap_gpio.gpio_clock_active);/* PULL_up */
	gpio_direction_output(chip->wrap_gpio.clock_gpio, 1);	/* out 1 */
	mutex_unlock(&chip->pinctrl_mutex);
}

void opchg_set_data_active(struct oplus_wrap_chip *chip)
{
	mutex_lock(&chip->pinctrl_mutex);
	gpio_direction_input(chip->wrap_gpio.data_gpio);	/* in */
	pinctrl_select_state(chip->wrap_gpio.pinctrl, chip->wrap_gpio.gpio_data_active); /* no_PULL */
	mutex_unlock(&chip->pinctrl_mutex);
}

void opchg_set_data_sleep(struct oplus_wrap_chip *chip)
{
	mutex_lock(&chip->pinctrl_mutex);
	pinctrl_select_state(chip->wrap_gpio.pinctrl, chip->wrap_gpio.gpio_data_sleep);/* PULL_down */
	gpio_direction_output(chip->wrap_gpio.data_gpio, 0);	/* out 1 */
	mutex_unlock(&chip->pinctrl_mutex);
}

void opchg_set_reset_sleep(struct oplus_wrap_chip *chip)
{
	...
	mutex_lock(&chip->pinctrl_mutex);
	gpio_direction_output(chip->wrap_gpio.switch1_gpio, 0);	/* in 0*/
	gpio_direction_output(chip->wrap_gpio.reset_gpio, 0); /* out 0 */
	usleep_range(10000, 10000);
#ifdef CONFIG_OPLUS_CHARGER_MTK
	pinctrl_select_state(chip->wrap_gpio.pinctrl, chip->wrap_gpio.gpio_reset_sleep); /* PULL_down */
#else
	pinctrl_select_state(chip->wrap_gpio.pinctrl, chip->wrap_gpio.gpio_reset_active); /* PULL_up */
#endif
	gpio_set_value(chip->wrap_gpio.reset_gpio, 1);
	usleep_range(10000, 10000);
	gpio_direction_output(chip->wrap_gpio.reset_gpio, 0); /* out 0 */
...
void opchg_set_reset_active(struct oplus_wrap_chip *chip)
{
...
void opchg_set_reset_active_force(struct oplus_wrap_chip *chip)
{
	int active_level = 0;
	int sleep_level = 1;
	int mcu_hwid_type = OPLUS_WRAP_MCU_HWID_UNKNOW;

	mcu_hwid_type = get_wrap_mcu_type(chip);
	...
	mutex_lock(&chip->pinctrl_mutex);
	gpio_direction_output(chip->wrap_gpio.switch1_gpio, 0);	/* in 0*/
	gpio_direction_output(chip->wrap_gpio.reset_gpio, active_level);		/* out 1 */

#ifdef CONFIG_OPLUS_CHARGER_MTK
	pinctrl_select_state(chip->wrap_gpio.pinctrl,
		chip->wrap_gpio.gpio_reset_sleep);	/* PULL_down */
#else
	pinctrl_select_state(chip->wrap_gpio.pinctrl,
		chip->wrap_gpio.gpio_reset_active);	/* PULL_up */
#endif

	gpio_set_value(chip->wrap_gpio.reset_gpio, active_level);
	usleep_range(10000, 10000);
	gpio_set_value(chip->wrap_gpio.reset_gpio, sleep_level);
	usleep_range(10000, 10000);
	gpio_set_value(chip->wrap_gpio.reset_gpio, active_level);
	usleep_range(2500, 2500);
	if (chip->dpdm_switch_mode == WRAP_CHARGER_MODE) {
		gpio_direction_output(chip->wrap_gpio.switch1_gpio, 1);	/* in 1*/
	}
	mutex_unlock(&chip->pinctrl_mutex);
	chg_debug("%s\n", __func__);
}

int oplus_wrap_get_reset_gpio_val(struct oplus_wrap_chip *chip)
{
	return gpio_get_value(chip->wrap_gpio.reset_gpio);
}

bool oplus_is_power_off_charging(struct oplus_wrap_chip *chip)
{
#ifdef CONFIG_OPLUS_CHARGER_MTK
...
bool oplus_is_charger_reboot(struct oplus_wrap_chip *chip)
{
#ifdef CONFIG_OPLUS_CHARGER_MTK
	int charger_type;
...
static void delay_reset_mcu_work_func(struct work_struct *work)
{
	...
	struct oplus_wrap_chip *chip = container_of(dwork,
		struct oplus_wrap_chip, delay_reset_mcu_work);
	opchg_set_clock_sleep(chip);
	opchg_set_reset_active(chip);
}

void oplus_wrap_delay_reset_mcu_init(struct oplus_wrap_chip *chip)
{
	INIT_DELAYED_WORK(&chip->delay_reset_mcu_work, delay_reset_mcu_work_func);
}

static void oplus_wrap_delay_reset_mcu(struct oplus_wrap_chip *chip)
{
	schedule_delayed_work(&chip->delay_reset_mcu_work,
		round_jiffies_relative(msecs_to_jiffies(1500)));
}

static bool is_allow_fast_chg_real(struct oplus_wrap_chip *chip)
{
	int temp = 0;
	int cap = 0;
	int chg_type = 0;
	...
	if (temp < chip->wrap_low_temp) {
		return false;
	}
	if (temp >= chip->wrap_high_temp) {
		return false;
	}
	mcu_hwid_type = get_wrap_mcu_type(chip);
	...
	if (mcu_hwid_type == OPLUS_WRAP_ASIC_HWID_RK826
		...
		if (!chip->suspend_charger) {
			if (temp < oplus_chg_get_cool_bat_decidegc()) {
				if (volt < chip->wrap_cool_bat_volt) {
					return false;
				}
			} else if (temp < oplus_chg_get_little_cool_bat_decidegc()) {
				if(volt < chip->wrap_little_cool_bat_volt) {
					return false;
				}
			} else if (temp < oplus_chg_get_normal_bat_decidegc()) {
				if(volt < chip->wrap_normal_bat_volt) {
					return false;
				}
			} else if (temp < chip->wrap_high_temp) {
				if (volt < chip->wrap_warm_bat_volt) {
					return false;
				}
			}
		} else {
			...
			if (temp < oplus_chg_get_cool_bat_decidegc()) {
				if (volt < chip->wrap_cool_bat_suspend_volt) {
					return false;
				}
			} else if (temp < oplus_chg_get_little_cool_bat_decidegc()) {
				if(volt < chip->wrap_little_cool_bat_suspend_volt) {
					return false;
				}
			} else if (temp < oplus_chg_get_normal_bat_decidegc()) {
				if(volt < chip->wrap_normal_bat_suspend_volt) {
					return false;
				}
			} else if (temp < chip->wrap_high_temp) {
				if (volt < chip->wrap_warm_bat_suspend_volt) {
					return false;
				}
			}
		}
	...
	if (cap < chip->wrap_low_soc) {
		return false;
	}
	if (cap > chip->wrap_high_soc) {
		return false;
	}
	if (oplus_wrap_get_fastchg_to_normal() == true) {
		chg_debug(" oplus_wrap_get_fastchg_to_normal is true\n");
		return false;
	}
...
static bool is_allow_fast_chg_dummy(struct oplus_wrap_chip *chip)
{
	int chg_type = 0;
	...
	if ((oplus_wrap_get_fastchg_dummy_started() == true || oplus_wrap_get_fastchg_to_warm() == true) && (!allow_real)) {
		chip->reset_adapter = false;
	...
	if (mcu_hwid_type == OPLUS_WRAP_ASIC_HWID_RK826
		|| mcu_hwid_type == OPLUS_WRAP_ASIC_HWID_OP10
		|| mcu_hwid_type == OPLUS_WRAP_ASIC_HWID_RT5125) {
		if ((oplus_wrap_get_fastchg_dummy_started() == true || oplus_wrap_get_fastchg_to_warm() == true)
	...
	oplus_wrap_set_fastchg_allow(allow_real);
	return true;
}
...
void switch_fast_chg(struct oplus_wrap_chip *chip)
{
	...
	if (chip->dpdm_switch_mode == WRAP_CHARGER_MODE
		&& gpio_get_value(chip->wrap_gpio.switch1_gpio) == 1)
		{
	...
	if (is_allow_fast_chg_dummy(chip) == true) {
		if (oplus_wrap_get_adapter_update_status() == ADAPTER_FW_UPDATE_FAIL) {
			oplus_wrap_delay_reset_mcu(chip);
			opchg_set_switch_mode(chip, WRAP_CHARGER_MODE);
		} else {
			if (oplus_wrap_get_fastchg_allow() == false
					&& oplus_wrap_get_fastchg_to_warm() == true) {
				chg_err(" fastchg_allow false, to_warm true, don't switch to wrap mode\n");
			} else {
				opchg_set_clock_sleep(chip);
				opchg_set_reset_active(chip);
				opchg_set_switch_mode(chip, WRAP_CHARGER_MODE);
				...
				if(opchg_get_mcu_update_state() == false
					&& oplus_wrap_check_asic_fw_status() == 0) {
					chg_err("check fw fail, go to update fw!\n");
					oplus_chg_set_chargerid_switch_val(0);
					opchg_set_switch_mode(chip, NORMAL_CHARGER_MODE);
					oplus_chg_clear_chargerid_info();
					oplus_wrap_fw_update_work_plug_in();
				}
			}
		}
	}
	chg_err(" end, allow_fast_chg:%d\n", oplus_wrap_get_fastchg_allow());
}

int oplus_wrap_get_ap_clk_gpio_val(struct oplus_wrap_chip *chip)
{
	return gpio_get_value(chip->wrap_gpio.clock_gpio);
}

int opchg_get_gpio_ap_data(struct oplus_wrap_chip *chip)
{
	return gpio_get_value(chip->wrap_gpio.data_gpio);
}

int opchg_get_clk_gpio_num(struct oplus_wrap_chip *chip)
{
	return chip->wrap_gpio.clock_gpio;
}

int opchg_get_data_gpio_num(struct oplus_wrap_chip *chip)
{
	return chip->wrap_gpio.data_gpio;
}

int opchg_read_ap_data(struct oplus_wrap_chip *chip)
{
	int bit = 0;
	...
	bit = gpio_get_value(chip->wrap_gpio.data_gpio);
	return bit;
}

void opchg_reply_mcu_data(struct oplus_wrap_chip *chip, int ret_info, int device_type)
{
	int i = 0;
	int reply_counts = 0;

	if (chip->wrap_multistep_adjust_current_support == false) {
		reply_counts = 3;
	} else {
		if (chip->wrap_reply_mcu_bits == 4) {
			reply_counts = 4;
		} else if (chip->wrap_reply_mcu_bits == 7) {
			reply_counts = 7;
		} else {
			reply_counts = 3;
		}
	...
	if (chip->w_soc_temp_to_mcu == false) {
		for (i = 1; i < reply_counts; i++) {
				gpio_set_value(chip->wrap_gpio.data_gpio, (ret_info >> (reply_counts - i - 1)) & 0x01);
		...
		gpio_set_value(chip->wrap_gpio.data_gpio, device_type);
...
void opchg_reply_mcu_data_4bits
		(struct oplus_wrap_chip *chip, int ret_info, int device_type)
{
	int i = 0;
	for (i = 0; i < 4; i++) {
		if (i == 0) {					/*tell mcu1503 device_type*/
			gpio_set_value(chip->wrap_gpio.data_gpio, ret_info >> 2);
			chg_err("first_send_bit = %d\n", ret_info >> 2);
		} else if (i == 1) {
			gpio_set_value(chip->wrap_gpio.data_gpio, (ret_info >> 1) & 0x1);
			chg_err("second_send_bit = %d\n", (ret_info >> 1) & 0x1);
		} else if (i == 2) {
			gpio_set_value(chip->wrap_gpio.data_gpio, ret_info & 0x1);
...
void opchg_set_switch_fast_charger(struct oplus_wrap_chip *chip)
{
	gpio_direction_output(chip->wrap_gpio.switch1_gpio, 1);	/* out 1*/
	if (chip->wrap_gpio.switch1_ctr1_gpio > 0) {//asic rk826
		gpio_direction_output(chip->wrap_gpio.switch1_ctr1_gpio, 0);        /* out 0*/
	}
}

void opchg_set_wrap_chargerid_switch_val(struct oplus_wrap_chip *chip, int value)
{
	int level = 0;
	...
	if (value == 1)
		...
		return;
	/*
	if (chip->wrap_gpio.switch1_ctr1_gpio > 0) {//asic rk826/op10
		if (level == 1) {
			gpio_direction_output(chip->wrap_gpio.reset_gpio, 1);
			gpio_set_value(chip->wrap_gpio.reset_gpio, 1);
		}
		gpio_direction_output(chip->wrap_gpio.switch1_ctr1_gpio, level);
		chg_err("switch1_gpio[%d], switch1_ctr1_gpio[%d]\n",
			gpio_get_value(chip->wrap_gpio.switch1_gpio),
			gpio_get_value(chip->wrap_gpio.switch1_ctr1_gpio));
		if (level == 0) {
			gpio_direction_output(chip->wrap_gpio.reset_gpio, 0);
			gpio_set_value(chip->wrap_gpio.reset_gpio, 0);
		}
	}
	*/
}

void opchg_set_switch_normal_charger(struct oplus_wrap_chip *chip)
{
	if (chip->wrap_gpio.switch1_gpio > 0) {
		gpio_direction_output(chip->wrap_gpio.switch1_gpio, 0);	/* in 0*/
	}
...
void opchg_set_switch_earphone(struct oplus_wrap_chip *chip)
{
	return;
}

void opchg_set_switch_mode(struct oplus_wrap_chip *chip, int mode)
{
	int retry = 10;
	int mcu_hwid_type = OPLUS_WRAP_MCU_HWID_UNKNOW;
	...
	if (mode == WRAP_CHARGER_MODE && chip->mcu_update_ing) {
		chg_err(" mcu_update_ing, don't switch to wrap mode\n");
		return;
	}
	...
	switch (mode) {
	case WRAP_CHARGER_MODE:	       /*11*/
		if (mcu_hwid_type == OPLUS_WRAP_ASIC_HWID_RK826
			|| mcu_hwid_type == OPLUS_WRAP_ASIC_HWID_OP10
			|| mcu_hwid_type == OPLUS_WRAP_ASIC_HWID_RT5125) {
			do {
				if (gpio_get_value(chip->wrap_gpio.reset_gpio) == 1) {
					opchg_set_switch_fast_charger(chip);
					chg_err(" wrap mode, switch1_gpio:%d\n", gpio_get_value(chip->wrap_gpio.switch1_gpio));
					break;
				}
...
int oplus_wrap_get_switch_gpio_val(struct oplus_wrap_chip *chip)
{
	return gpio_get_value(chip->wrap_gpio.switch1_gpio);
}
...
void reset_fastchg_after_usbout(struct oplus_wrap_chip *chip)
{
	if (oplus_wrap_get_fastchg_started() == false) {
		...
		oplus_wrap_set_fastchg_type_unknow();
		opchg_set_switch_mode(chip, NORMAL_CHARGER_MODE);
		if (oplus_wrap_get_fastchg_dummy_started() == false) {
			oplus_wrap_set_mcu_sleep();
		}
	}
	oplus_wrap_set_fastchg_to_normal_false();
	oplus_wrap_set_fastchg_to_warm_false();
	oplus_wrap_set_fastchg_low_temp_full_false();
	oplus_wrap_set_fastchg_dummy_started_false();
}

static irqreturn_t irq_rx_handler(int irq, void *dev_id)
{
	oplus_wrap_shedule_fastchg_work();
	return IRQ_HANDLED;
}
...
void oplus_wrap_data_irq_init(struct oplus_wrap_chip *chip)
{
	...
	if (node) {
		if (node_new) {
			...
			chg_err("wrap_gpio.data_irq:%d\n", chip->wrap_gpio.data_irq);
		} else {
			of_property_read_u32_array(node , "interrupts", intr, ARRAY_SIZE(intr));
			chg_debug(" intr[0]  = %d, intr[1]  = %d\r\n", intr[0], intr[1]);
			chip->wrap_gpio.data_irq = irq_of_parse_and_map(node, 0);
		}
	} else {
		chg_err(" node not exist!\r\n");
		chip->wrap_gpio.data_irq = CUST_EINT_MCU_AP_DATA;
	}
#else
	chip->wrap_gpio.data_irq = gpio_to_irq(chip->wrap_gpio.data_gpio);
#endif
...
void oplus_wrap_eint_register(struct oplus_wrap_chip *chip)
{
#ifdef CONFIG_OPLUS_CHARGER_MTK
	...
	if (node) {
		...
		if (!register_status) {
			...
			enable_irq(chip->wrap_gpio.data_irq);
		}
	}
#else
	int retval = 0;
	opchg_set_data_active(chip);
	retval = request_irq(chip->wrap_gpio.data_irq, irq_rx_handler, IRQF_TRIGGER_RISING, "mcu_data", chip);
	if (retval < 0) {
...
void oplus_wrap_eint_unregister(struct oplus_wrap_chip *chip)
{
#ifdef CONFIG_OPLUS_CHARGER_MTK
	struct device_node *node = NULL;
	node = of_find_compatible_node(NULL, NULL, "mediatek, WRAP_EINT_NEW_FUNCTION");
	...
	if (node) {
		...
		disable_irq(chip->wrap_gpio.data_irq);
	}
#else
	free_irq(chip->wrap_gpio.data_irq, chip);
#endif
