...
static bool n76e_fw_check_frontline(struct oplus_wrap_chip *chip)
{
	...
	for (fw_line = 0; fw_line < (total_line - 1); fw_line++) {
		...
		data_buf[0] = 0;
		oplus_wrap_i2c_write(chip->client, 0x03, 1, &data_buf[0]);
...
static bool n76e_fw_check_lastline(struct oplus_wrap_chip *chip)
{
...
static int n76e_fw_write(struct oplus_wrap_chip *chip,
	const unsigned char *data_buf, unsigned int offset, unsigned int length)
{
	...
	while (count < (offset + length)) {
		...
		oplus_wrap_i2c_write(chip->client, 0x02, BYTES_TO_WRITE,
...
static int n76e_fw_update(struct oplus_wrap_chip *chip)
{
	unsigned char zero_buf[1] = {0};
	unsigned char addr_buf[2] = {0x00, 0x00};
	unsigned char temp_buf[1] = {0};
...
static int n76e_get_fw_verion_from_ic(struct oplus_wrap_chip *chip)
{
	unsigned char addr_buf[2] = {0x3B, 0xF0};
...
static int n76e_fw_check_then_recover(struct oplus_wrap_chip *chip)
{
	int update_result = 0;
	int ret = 0;
...
struct oplus_wrap_operations oplus_n76e_ops = {
...
static int n76e_driver_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
...
#endif
#endif
	if(get_wrap_mcu_type(chip) != OPLUS_WRAP_MCU_HWID_N76E){
	...
	if (chip->wrap_fw_type == WRAP_FW_TYPE_N76E_4400_AVOID_OVER_TEMP_NTC61C) {
	...
	} else if (chip->wrap_fw_type == WRAP_FW_TYPE_N76E_4400_WRAP_FFC_15C) {
		chip->firmware_data = n76e_fw_data_4400_wrap_ffc_15c;
		chip->fw_data_count = sizeof(n76e_fw_data_4400_wrap_ffc_15c);
		chip->fw_data_version = n76e_fw_data_4400_wrap_ffc_15c[chip->fw_data_count - 4];
	} else if (chip->wrap_fw_type == WRAP_FW_TYPE_N76E_4400_WRAP_FFC_15C_FV4450) {
		chip->firmware_data = n76e_fw_data_4400_wrap_ffc_15c_fv4450;
		chip->fw_data_count = sizeof(n76e_fw_data_4400_wrap_ffc_15c_fv4450);
		chip->fw_data_version = n76e_fw_data_4400_wrap_ffc_15c_fv4450[chip->fw_data_count - 4];
	}
...
MODULE_DESCRIPTION("Driver for oplus wrap n76e fast mcu");
