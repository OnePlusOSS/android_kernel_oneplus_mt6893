...
static struct oplus_wrap_chip *the_chip = NULL;
...
static int i2c_dma_read(struct i2c_client *client, u8 addr, s32 len, u8 *rxbuf)
{
	...
	/*chg_err("wrap dma i2c read: 0x%x, %d bytes(s)\n", addr, len);*/
...
static int oplus_wrap_i2c_read(struct i2c_client *client, u8 addr, s32 len, u8 *rxbuf)
{
#ifdef CONFIG_OPLUS_CHARGER_MTK
#if GTP_SUPPORT_I2C_DMA
...
static int oplus_wrap_i2c_write(struct i2c_client *client, u8 addr, s32 len, u8 const *txbuf)
{
#ifdef CONFIG_OPLUS_CHARGER_MTK
#if GTP_SUPPORT_I2C_DMA
...
static bool stm8s_fw_check_frontline(struct oplus_wrap_chip *chip)
{
	...
	rc = oplus_wrap_i2c_write(chip->client, 0x01, 2, &addr_buf[0]);
	if (rc < 0) {
	...
	for (i = 0;i < READ_COUNT;i++) {	/*1508:448, 1503:192*/
		oplus_wrap_i2c_read(chip->client, 0x03, 16, &data_buf[0]);
		msleep(2);
		oplus_wrap_i2c_read(chip->client, 0x03, 16, &data_buf[16]);
		addr = 0x8800 + i * 32;
...
static bool stm8s_fw_check_lastline(struct oplus_wrap_chip *chip)
{
...
static int stm8s_fw_write(struct oplus_wrap_chip *chip,
		const unsigned char *data_buf, unsigned int offset, unsigned int length)
{
	...
	while (count < (offset + length)) {
		...
		oplus_wrap_i2c_write(chip->client, 0x02, BYTES_TO_WRITE, &data_buf[count+BYTE_OFFSET]);
		oplus_wrap_i2c_write(chip->client, 0x05, 1, &zero_buf[0]);
		oplus_wrap_i2c_read(chip->client, 0x05, 1, &temp_buf[0]);
		/*chg_err("lfc read 0x05, temp_buf[0]:0x%x\n", temp_buf[0]);*/

		/*write 16 bytes data to stm8s again*/
		oplus_wrap_i2c_write(chip->client, 0x02, BYTES_TO_WRITE, &data_buf[count+BYTE_OFFSET+BYTES_TO_WRITE]);
		oplus_wrap_i2c_write(chip->client, 0x05, 1, &zero_buf[0]);
		oplus_wrap_i2c_read(chip->client, 0x05, 1, &temp_buf[0]);
...
static int stm8s_fw_update(struct oplus_wrap_chip *chip)
{
	unsigned char zero_buf[1] = {0};
	unsigned char addr_buf[2] = {0x88, 0x00};
	unsigned char temp_buf[1] = {0};
...
update_fw:
	...
	for (i = 0; i < ERASE_COUNT; i++) {
		...
		oplus_wrap_i2c_write(chip->client, 0x04, 1, &zero_buf[0]);
		msleep(1);
		oplus_wrap_i2c_read(chip->client, 0x04, 1, &temp_buf[0]);
	...
	oplus_wrap_i2c_write(chip->client, 0x06, 1, &zero_buf[0]);
	oplus_wrap_i2c_read(chip->client, 0x06, 1, &temp_buf[0]);
	/*jump to app code end*/
	chip->have_updated = 1;
	chip->mcu_update_ing = false;
	/*pull down GPIO96 to power off MCU1503/1508*/
	chip->wrap_fw_check = true;
...
static int stm8s_get_fw_verion_from_ic(struct oplus_wrap_chip *chip)
{
...
//	}
	...
	msleep(2);
	oplus_wrap_i2c_read(chip->client,0x03,4,data_buf);
...
static int stm8s_fw_check_then_recover(struct oplus_wrap_chip *chip)
{
	int update_result = 0;
	int try_count = 5;
	int ret = 0;
...
struct oplus_wrap_operations oplus_stm8s_ops = {
	.fw_update = stm8s_fw_update,
	.fw_check_then_recover = stm8s_fw_check_then_recover,
	.set_switch_mode = opchg_set_switch_mode,
	.eint_regist = oplus_wrap_eint_register,
	.eint_unregist = oplus_wrap_eint_unregister,
	...
	.get_reset_gpio_val = oplus_wrap_get_reset_gpio_val,
	.get_switch_gpio_val = oplus_wrap_get_switch_gpio_val,
	.get_ap_clk_gpio_val = oplus_wrap_get_ap_clk_gpio_val,
...
static void register_wrap_devinfo(void)
{
	int ret = 0;
	char *version;
	char *manufacture;
	...
	ret = register_device_proc("wrap", version, manufacture);
	if (ret) {
		chg_err(" fail\n");
	}
}
...
static ssize_t wrap_fw_check_read(struct file *filp,
		char __user *buff, size_t count, loff_t *off)
{
	...
	if (the_chip && the_chip->wrap_fw_check == true) {
		read_data[0] = '1';
	} else {
		read_data[0] = '0';
	}
...
static const struct file_operations wrap_fw_check_proc_fops = {
	.read = wrap_fw_check_read,
	.llseek = noop_llseek,
};

static int init_proc_wrap_fw_check(void)
{
	struct proc_dir_entry *p = NULL;

	p = proc_create("wrap_fw_check", 0444, NULL, &wrap_fw_check_proc_fops);
	if (!p) {
		chg_err("proc_create wrap_fw_check_proc_fops fail!\n");
	}
	return 0;
}
		...
		const struct i2c_device_id *id)
{
	...
	chip->wrap_fw_check = false;
	mutex_init(&chip->pinctrl_mutex);

	oplus_wrap_fw_type_dt(chip);
	...
	switch (chip->wrap_fw_type) {
		case WRAP_FW_TYPE_STM8S_4400_AVOID_FAKE_ADAPTER:
			...
			break;
		case WRAP_FW_TYPE_STM8S_4400_AVOID_FG_I2C_ERR:
			...
			break;
		case WRAP_FW_TYPE_STM8S_4400_AVOID_OVER_TEMP:
			...
			break;
		case WRAP_FW_TYPE_STM8S_4400_AVOID_OVER_TEMP_NTC61C:
			...
			break;
		case WRAP_FW_TYPE_STM8S_4400_WRAP_FFC_09C:
			chip->firmware_data = Stm8s_fw_data_4400_wrap_ffc_09c;
			chip->fw_data_count = sizeof(Stm8s_fw_data_4400_wrap_ffc_09c);
			chip->fw_data_version = Stm8s_fw_data_4400_wrap_ffc_09c[chip->fw_data_count - 4];
			break;
		case WRAP_FW_TYPE_STM8S_4400_WRAP_FFC_15C:
			chip->firmware_data = Stm8s_fw_data_4400_wrap_ffc_15c;
			chip->fw_data_count = sizeof(Stm8s_fw_data_4400_wrap_ffc_15c);
			chip->fw_data_version = Stm8s_fw_data_4400_wrap_ffc_15c[chip->fw_data_count - 4];
			break;
		case WRAP_FW_TYPE_STM8S_4400_WRAP_FFC_15C_FV4450:
			chip->firmware_data = Stm8s_fw_data_4400_wrap_ffc_15c_fv4450;
			chip->fw_data_count = sizeof(Stm8s_fw_data_4400_wrap_ffc_15c_fv4450);
			chip->fw_data_version = Stm8s_fw_data_4400_wrap_ffc_15c_fv4450[chip->fw_data_count - 4];
			break;
		case WRAP_FW_TYPE_STM8S_4450_FFC:
			...
			break;
		case WRAP_FW_TYPE_STM8S_4450_FFC_5V6A:
			...
			break;
		case WRAP_FW_TYPE_STM8S_4400_WRAP_FFC_15C_TI411:
			chip->firmware_data = Stm8s_fw_data_4400_wrap_ffc_15c_ti411;
			chip->fw_data_count = sizeof(Stm8s_fw_data_4400_wrap_ffc_15c_ti411);
			chip->fw_data_version = Stm8s_fw_data_4400_wrap_ffc_15c_ti411[chip->fw_data_count - 4];
			break;
		case WRAP_FW_TYPE_STM8S_4450_FFC_5C_WRAP:
			chip->firmware_data = Stm8s_fw_data_4450_ffc_5c_wrap;
			chip->fw_data_count = sizeof(Stm8s_fw_data_4450_ffc_5c_wrap);
			chip->fw_data_version = Stm8s_fw_data_4450_ffc_5c_wrap[chip->fw_data_count - 4];
			break;
		case WRAP_FW_TYPE_STM8S_4400_FFC_5V5P9A:
			...
			break;
		case WRAP_FW_TYPE_STM8S_4450_FFC_5V6A_WRAP_4052MA_3BIT:
			...
			break;
		case WRAP_FW_TYPE_STM8S_4450_FFC_5V4A_WRAP_4052MA_3BIT:
			chip->firmware_data = Stm8s_fw_data_4450_wrap_ffc_5v4a_3bit;
			chip->fw_data_count = sizeof(Stm8s_fw_data_4450_wrap_ffc_5v4a_3bit);
			chip->fw_data_version = Stm8s_fw_data_4450_wrap_ffc_5v4a_3bit[chip->fw_data_count - 4];
			break;
		case WRAP_FW_TYPE_STM8S_4400_WRAP_FFC_15C_18041:
			chip->firmware_data = Stm8s_fw_data_4400_wrap_ffc_15c_18041;
			chip->fw_data_count = sizeof(Stm8s_fw_data_4400_wrap_ffc_15c_18041);
			chip->fw_data_version = Stm8s_fw_data_4400_wrap_ffc_15c_18041[chip->fw_data_count - 4];
			break;
		case WRAP_FW_TYPE_STM8S_4450_WRAP_FFC_5V6A_19365:
			chip->firmware_data = Stm8s_fw_data_4450_WRAP_FFC_5V6A_19365;
			chip->fw_data_count = sizeof(Stm8s_fw_data_4450_WRAP_FFC_5V6A_19365);
			chip->fw_data_version = Stm8s_fw_data_4450_WRAP_FFC_5V6A_19365[chip->fw_data_count - 4];
			break;
		case WRAP_FW_TYPE_STM8S_4450_FFC_SHORT_RESET_WINDOW:
			...
			break;
		case WRAP_FW_TYPE_STM8S_4400_SWRAP_3500MA:
			chip->firmware_data = Stm8s_fw_data_4400_swrap_3500MA;
			chip->fw_data_count = sizeof(Stm8s_fw_data_4400_swrap_3500MA);
			chip->fw_data_version = Stm8s_fw_data_4400_swrap_3500MA[chip->fw_data_count - 4];
			break;
		case WRAP_FW_TYPE_STM8S_4400_SWRAP_1000MA:
			chip->firmware_data = Stm8s_fw_data_4400_swrap_1000MA;
			chip->fw_data_count = sizeof(Stm8s_fw_data_4400_swrap_1000MA);
			chip->fw_data_version = Stm8s_fw_data_4400_swrap_1000MA[chip->fw_data_count - 4];
			break;
		case WRAP_FW_TYPE_STM8S_4400_SWRAP_5000MA :
			chip->firmware_data = Stm8s_fw_data_4400_swrap_5000MA;
			chip->fw_data_count = sizeof(Stm8s_fw_data_4400_swrap_5000MA);
			chip->fw_data_version = Stm8s_fw_data_4400_swrap_5000MA[chip->fw_data_count - 4];
			break;
		case WRAP_FW_TYPE_STM8S_4400_SWRAP_6500MA:
			chip->firmware_data = Stm8s_fw_data_4400_swrap_6500MA;
			chip->fw_data_count = sizeof(Stm8s_fw_data_4400_swrap_6500MA);
			chip->fw_data_version = Stm8s_fw_data_4400_swrap_6500MA[chip->fw_data_count - 4];
			break;
		case WRAP_FW_TYPE_STM8S_4450_SWRAP_6500MA:
			chip->firmware_data = Stm8s_fw_data_4450_swrap_6500MA;
			chip->fw_data_count = sizeof(Stm8s_fw_data_4450_swrap_6500MA);
			chip->fw_data_version = Stm8s_fw_data_4450_swrap_6500MA[chip->fw_data_count - 4];
			break;
		case WRAP_FW_TYPE_STM8S_4450_SWRAP_6500MA_FV4490:
			chip->firmware_data = Stm8s_fw_data_4450_swrap_6500MA_fv4490;
			chip->fw_data_count = sizeof(Stm8s_fw_data_4450_swrap_6500MA_fv4490);
			chip->fw_data_version = Stm8s_fw_data_4450_swrap_6500MA_fv4490[chip->fw_data_count - 4];
			break;
		case WRAP_FW_TYPE_STM8S_4450_SWRAP_6500MA_disableI2C:
			chip->firmware_data = Stm8s_fw_data_4450_swrap_6500MA_disableI2C;
			chip->fw_data_count = sizeof(Stm8s_fw_data_4450_swrap_6500MA_disableI2C);
			chip->fw_data_version = Stm8s_fw_data_4450_swrap_6500MA_disableI2C[chip->fw_data_count - 4];
			break;
		case WRAP_FW_TYPE_STM8S_4400_SWRAP_6500MA_8250:
			chip->firmware_data = Stm8s_fw_data_4400_swrap_6500MA_8250;
			chip->fw_data_count = sizeof(Stm8s_fw_data_4400_swrap_6500MA_8250);
			chip->fw_data_version = Stm8s_fw_data_4400_swrap_6500MA_8250[chip->fw_data_count - 4];
			break;
		case WRAP_FW_TYPE_STM8S_4450_SWRAP_6500MA_8250:
			chip->firmware_data = Stm8s_fw_data_4450_swrap_6500MA_8250;
			chip->fw_data_count = sizeof(Stm8s_fw_data_4450_swrap_6500MA_8250);
			chip->fw_data_version = Stm8s_fw_data_4450_swrap_6500MA_8250[chip->fw_data_count - 4];
			break;
	case WRAP_FW_TYPE_STM8S_4400_SWRAP_6500MA_8250_LINK:
		chip->firmware_data = Stm8s_fw_data_4400_swrap_6500MA_8250_link;
		chip->fw_data_count = sizeof(Stm8s_fw_data_4400_swrap_6500MA_8250_link);
		chip->fw_data_version = Stm8s_fw_data_4400_swrap_6500MA_8250_link[chip->fw_data_count - 4];
		break;
	case WRAP_FW_TYPE_STM8S_4400_SWRAP_6500MA_8250_LINK_LITE:
		chip->firmware_data = Stm8s_fw_data_4400_swrap_6500MA_8250_link_lite;
		chip->fw_data_count = sizeof(Stm8s_fw_data_4400_swrap_6500MA_8250_link_lite);
		chip->fw_data_version = Stm8s_fw_data_4400_swrap_6500MA_8250_link_lite[chip->fw_data_count - 4];
		break;
		default:
			break;
	}
	...
	oplus_wrap_gpio_dt_init(chip);
	opchg_set_clock_sleep(chip);
	oplus_wrap_delay_reset_mcu_init(chip);

	if(chip->wrap_fw_update_newmethod) {
		if(oplus_is_rf_ftm_mode()){
			oplus_wrap_fw_update_work_init(chip);
		}
	} else {
		oplus_wrap_fw_update_work_init(chip);
	}

	oplus_wrap_init(chip);
	register_wrap_devinfo();
	init_proc_wrap_fw_check();
	the_chip = chip;
...
MODULE_DESCRIPTION("Driver for oplus wrap stm8s fast mcu");
