...
static int op10_get_fw_old_version(struct oplus_wrap_chip *chip, u8 version_info[]);
...
static int check_flash_idle(struct oplus_wrap_chip *chip, u32 try_count)
{
	u8 rx_buf;
	int rc = 0;
	...
	do {
		rx_buf = 0xff;
		rc = oplus_wrap_i2c_read(chip->client, CMD_DEV_STATUS,1, &rx_buf);
		if (rc < 0) {
...
static int check_crc32_available(struct oplus_wrap_chip *chip, u32 try_count)
{
	u8 rx_buf;
	int rc = 0;
...
u32 crc32_sram(struct oplus_wrap_chip *chip)
{
...
static bool op10_fw_update_check(struct oplus_wrap_chip *chip)
{
	int i = 0;
	int ret = 0;
	...
	/* chip version */
	ret = oplus_wrap_i2c_read(chip->client, CMD_GET_VERSION, 1, rx_buf);
	if (ret < 0) {
	...
	oplus_wrap_i2c_write(chip->client, CMD_SET_ADDR, 2, rx_buf);
	msleep(1);
	memset(rx_buf, 0, 4);
	oplus_wrap_i2c_read(chip->client, CMD_XFER_R_DATA, 4, rx_buf);
	...
	*((u32 *)rx_buf) = chip->fw_data_count;
	oplus_wrap_i2c_write(chip->client, CMD_SET_CKSM_LEN, 4, rx_buf);
	msleep(5);
	...
	memset(rx_buf, 0, 4);
	oplus_wrap_i2c_read(chip->client, CMD_GET_CRC32, 4, rx_buf);
	...
	/*oplus_wrap_i2c_read(chip->client, CMD_USER_BOOT, 1, rx_buf);*/

	return true;
}
static int op10_fw_update(struct oplus_wrap_chip *chip)
{
	u32 check_status_try_count = 100;//try 2s
	...
	/* chip erase */
	rc = oplus_wrap_i2c_read(chip->client, CMD_CHIP_ERASE, 1, rx_buf);
	if (rc < 0) {
	...
	while (fw_len) {
		...
		*((u32 *)rx_buf) = fw_offset;
		...
		while (write_len_temp) {
			...
			oplus_wrap_i2c_write(chip->client, CMD_XFER_W_DAT, chunk_len, chip->firmware_data + fw_offset + chunk_index * I2C_RW_LEN_MAX);
			msleep(1);

			write_len_temp -= chunk_len;
			chunk_index++;
		}
		oplus_wrap_i2c_read(chip->client, CMD_PRG_START, 1, rx_buf);
		msleep(5);
	...
	*((u32 *)rx_buf) = chip->fw_data_count;
	...
	if (*((u32 *)rx_buf) != new_fw_crc32) {
		...
		*((u32 *)rx_buf) = 0x4641494C;/*FAIL*/
		oplus_wrap_i2c_write(chip->client, CMD_XFER_W_DAT, 4, rx_buf);
		msleep(1);
		oplus_wrap_i2c_read(chip->client, CMD_PRG_START, 1, rx_buf);
		msleep(10);
		if (check_flash_idle(chip, write_done_try_count) == -1) {
...
static int op10_get_fw_old_version(struct oplus_wrap_chip *chip, u8 version_info[])
{
	...
	if (*((u32 *)rx_buf) < fw_len_address) {
		...
		oplus_wrap_i2c_write(chip->client, CMD_SET_ADDR, 2, rx_buf);
		msleep(1);
		oplus_wrap_i2c_read(chip->client, CMD_XFER_R_DATA, FW_VERSION_LEN, version_info);
	} else {
...
static int op10_get_fw_verion_from_ic(struct oplus_wrap_chip *chip)
{
...
static int op10_fw_check_then_recover(struct oplus_wrap_chip *chip)
{
	int update_result = 0;
	int try_count = 5;
	int ret = 0;
	...
	if (!oplus_wrap_get_fastchg_allow())
		opchg_set_reset_sleep(chip);
...
int op10_set_battery_temperature_soc(int temp_bat, int soc_bat)
{
	...
	ret = oplus_wrap_i2c_write(the_chip->client, (u8)0xE, 2, read_buf);
	if (ret < 0) {
...
struct oplus_wrap_operations oplus_op10_ops = {
...
static int op10_parse_fw_from_dt(struct oplus_wrap_chip *chip)
{
...
static int op10_parse_fw_from_array(struct oplus_wrap_chip *chip)
{
	if (chip->batt_type_4400mv) {
		chip->firmware_data = op10_fw_data_4400_wrap_ffc_15c;
		chip->fw_data_count = sizeof(op10_fw_data_4400_wrap_ffc_15c);
		chip->fw_data_version = op10_fw_data_4400_wrap_ffc_15c[chip->fw_data_count - 4];
	} else {
		chip->firmware_data = op10_fw_data_4400_wrap_ffc_15c;
		chip->fw_data_count = sizeof(op10_fw_data_4400_wrap_ffc_15c);
		chip->fw_data_version = op10_fw_data_4400_wrap_ffc_15c[chip->fw_data_count - 4];
	...
	if (chip->wrap_fw_type == WRAP_FW_TYPE_OP10_4400_WRAP_FFC_15C) {
		chip->firmware_data = op10_fw_data_4400_wrap_ffc_15c;
		chip->fw_data_count = sizeof(op10_fw_data_4400_wrap_ffc_15c);
		chip->fw_data_version = op10_fw_data_4400_wrap_ffc_15c[chip->fw_data_count - 4];
	}
	...
	case WRAP_FW_TYPE_OP10_4400_WRAP_FFC_15C:
		chip->firmware_data = op10_fw_data_4400_wrap_ffc_15c;
		chip->fw_data_count = sizeof(op10_fw_data_4400_wrap_ffc_15c);
		chip->fw_data_version = op10_fw_data_4400_wrap_ffc_15c[chip->fw_data_count - 4];
		break;
	case WRAP_FW_TYPE_OP10_4450_WRAP_FFC_5V4A_4BIT:
		chip->firmware_data = op10_fw_data_4450_wrap_ffc_5v4a_4bit;
		chip->fw_data_count = sizeof(op10_fw_data_4450_wrap_ffc_5v4a_4bit);
		chip->fw_data_version = op10_fw_data_4450_wrap_ffc_5v4a_4bit[chip->fw_data_count - 4];
		break;
	case WRAP_FW_TYPE_OP10_4250_WRAP_FFC_10V6A_4BIT:
		chip->firmware_data = op10_fw_data_4250_wrap_ffc_10v6a_4bit;
		chip->fw_data_count = sizeof(op10_fw_data_4250_wrap_ffc_10v6a_4bit);
		chip->fw_data_version = op10_fw_data_4250_wrap_ffc_10v6a_4bit[chip->fw_data_count - 4];
		break;
	case WRAP_FW_TYPE_OP10_4450_WRAP_FFC_5V6A_4BIT:
		chip->firmware_data = op10_fw_data_4450_wrap_ffc_5v6a_4bit;
		chip->fw_data_count = sizeof(op10_fw_data_4450_wrap_ffc_5v6a_4bit);
		chip->fw_data_version = op10_fw_data_4450_wrap_ffc_5v6a_4bit[chip->fw_data_count - 4];
	default:
		break;
	}
...
#endif
#endif
	if (get_wrap_mcu_type(chip) != OPLUS_WRAP_ASIC_HWID_OP10) {
...
MODULE_DESCRIPTION("Driver for oplus wrap op10 fast mcu");
