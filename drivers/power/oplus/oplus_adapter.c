...
static void adapter_update_work_func(struct work_struct *work)
{
		...
		tx_gpio = oplus_wrap_get_uart_tx();
		rx_gpio = oplus_wrap_get_uart_rx();
		
        adapter_xlog_printk(CHG_LOG_CRTI, " begin\n");
		oplus_wrap_uart_init();
        ...
        if (update_result) {
				oplus_wrap_set_adapter_update_real_status(ADAPTER_FW_UPDATE_SUCCESS);
        } else {
				oplus_wrap_set_adapter_update_real_status(ADAPTER_FW_UPDATE_FAIL);
				oplus_wrap_set_adapter_update_report_status(ADAPTER_FW_UPDATE_FAIL);
				
        }
        msleep(20);
		
		oplus_wrap_uart_reset();
		
        if (update_result) {
                msleep(2000);
				oplus_wrap_set_adapter_update_report_status(ADAPTER_FW_UPDATE_SUCCESS);
        }
