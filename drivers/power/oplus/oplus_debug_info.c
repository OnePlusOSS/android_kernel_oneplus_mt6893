...
#define OPLUS_CHG_DEBUG_WRAP_MAX_VOLT  4400
...
static int oplus_chg_get_wrap_adapter_type_index(struct oplus_chg_chip *chip);
...
enum oplus_chg_debug_info_notify_flag {
	...
	OPLUS_NOTIFY_CHG_SLOW_WRAP_NON_START,
	OPLUS_NOTIFY_CHG_SLOW_WRAP_ADAPTER_NON_MAX_POWER,
...
#define SOC_LOAD_DELAY (60 * 1000)
	...
	int wrap_max_input_volt;
	int wrap_max_input_current;
	int fcc_design;
...
static struct oplus_chg_debug_notify_policy oplus_chg_debug_notify_policy[] = {
	...
	[OPLUS_NOTIFY_CHG_SLOW_WRAP_NON_START]               = {OPLUS_CHG_NOTIFY_ONCE,         1,  1,   -1,"FastChgNotStart"},
	[OPLUS_NOTIFY_CHG_SLOW_WRAP_ADAPTER_NON_MAX_POWER]   = {OPLUS_CHG_NOTIFY_ONCE,         1,  1,   -1,"AdatpterNotMaxPower"},
	...
	[OPLUS_NOTIFY_CHG_WRAP_BREAK]             		= {OPLUS_CHG_NOTIFY_ONCE,          1,    1,      -1,"FastChgBreak"},
...
static struct wrap_adapter_name wrap_adapter_name[] = {
	[OPLUS_FASTCHG_OUTPUT_POWER_20W]		= {"WRAP_20W"},
	[OPLUS_FASTCHG_OUTPUT_POWER_30W]		= {"WRAP_30W"},
	[OPLUS_FASTCHG_OUTPUT_POWER_33W]		= {"WRAP_33W"},
	[OPLUS_FASTCHG_OUTPUT_POWER_50W]		= {"SWRAP_50W"},
	[OPLUS_FASTCHG_OUTPUT_POWER_65W]		= {"SWRAP_65W"},
};


struct wrap_charger_type wrap_charger_type_list[] = {
...
static void oplus_chg_send_info_dwork(struct work_struct *work)
{
	...
	if ((ret > 0) && (oplus_chg_debug_info.retry_cnt > 0)) {
		...
		oplus_chg_debug_info.wrap_mcu_error = 0;
...
static int oplus_chg_get_wrap_adapter_name_index(struct oplus_chg_chip *chip) {
	int index = -1;
	int index_adapter = -1;
	if(oplus_chg_debug_info.fast_chg_type == 0) {
		return -1;
	}
	index = oplus_chg_get_wrap_adapter_type_index(chip);
	index_adapter = wrap_charger_type_list[index].output_power_type;
...
{
	int ret = 0;
	struct oplus_wrap_chip *wrap_chip = NULL;
	struct timespec ts;
	struct rtc_time tm;
		...
		oplus_chg_debug_notify_flag_is_set(OPLUS_NOTIFY_CHARGER_INFO))
	{
		...
		if(oplus_chg_get_wrap_adapter_name_index(chip) >= 0) {
			ret += snprintf(&oplus_chg_debug_msg[ret], OPLUS_CHG_DEBUG_MSG_LEN - ret,
					"$$wrap_adapter_name@@%s", wrap_adapter_name[oplus_chg_get_wrap_adapter_name_index(chip)]);
		}
		...
		//add for wrap_mcu_error
		if(oplus_chg_debug_info.wrap_mcu_error != 0)
			ret += snprintf(&oplus_chg_debug_msg[ret], OPLUS_CHG_DEBUG_MSG_LEN - ret,
					"$$wrap_mcu_error@@0x%x", oplus_chg_debug_info.wrap_mcu_error);
		...
		if (wrap_chip) {
			ret += snprintf(&oplus_chg_debug_msg[ret], OPLUS_CHG_DEBUG_MSG_LEN - ret,
					"WRAP[ %d / %d / %d / %d / %d / %d]",
					wrap_chip->fastchg_allow, wrap_chip->fastchg_started, wrap_chip->fastchg_dummy_started,
					wrap_chip->fastchg_to_normal, wrap_chip->fastchg_to_warm, wrap_chip->btb_temp_over);
		}
...
{
	int i;
	int arr_size = sizeof(wrap_charger_type_list) / sizeof(wrap_charger_type_list[0]);

	for(i = 0; i < arr_size; i++) {
		if (oplus_chg_debug_info.fast_chg_type == wrap_charger_type_list[i].chg_type) {
			break;
		}
	}
...
static int oplus_chg_get_wrap_adapter_is_low_input_power(struct oplus_chg_chip *chip)
{
	int adapter_input_power = 0;
	int wrap_adapter_type_index = -1;

	wrap_adapter_type_index = oplus_chg_get_wrap_adapter_type_index(chip);

	adapter_input_power = wrap_charger_type_list[wrap_adapter_type_index].vol * wrap_charger_type_list[wrap_adapter_type_index].cur;

	if (adapter_input_power < (oplus_chg_debug_info.wrap_max_input_volt * oplus_chg_debug_info.wrap_max_input_current))
		return 1;

	return 0;
}

static int oplus_chg_get_wrap_chg_max_input_power(struct oplus_chg_chip *chip)
{
	int wrap_adapter_type_index = -1;
	int adapter_input_power = 0;
	...
	return (oplus_chg_debug_info.wrap_max_input_volt * oplus_chg_debug_info.wrap_max_input_current * 8 / 10);
}

static int oplus_chg_get_wrap_input_power(struct oplus_chg_chip *chip)
{
	int i;
	int input_power = 0;
	int vol,cur;
	...
	i = oplus_chg_get_wrap_adapter_type_index(chip);

	vol = oplus_chg_debug_info.wrap_max_input_volt < wrap_charger_type_list[i].vol ?
		oplus_chg_debug_info.wrap_max_input_volt : wrap_charger_type_list[i].vol;
	cur = oplus_chg_debug_info.wrap_max_input_current < wrap_charger_type_list[i].cur ?
		oplus_chg_debug_info.wrap_max_input_current : wrap_charger_type_list[i].cur;
...
static int oplus_chg_set_chg_flag(int index)
{
	...
	if ((oplus_chg_debug_info.fast_chg_type == FASTCHG_CHARGER_TYPE_UNKOWN)
			|| (oplus_chg_debug_info.fast_chg_type == CHARGER_SUBTYPE_FASTCHG_WRAP)) {
...
static int oplus_chg_wrap_charge_strategy_init(struct oplus_chg_chip *chip)
{
	...
	if (!oplus_chg_chg_is_normal_path(chip)) {
		if (oplus_wrap_get_fastchg_started()) {
			index = oplus_chg_get_wrap_adapter_type_index(chip);
			output_power_type = wrap_charger_type_list[index].output_power_type;
		}
	}
...
static int oplus_chg_chg_is_normal_path(struct oplus_chg_chip *chip)
{
	...
	if (wrap_chip->support_wrap_by_normal_charger_path
...
static int oplus_chg_break_check(struct oplus_chg_chip *chip)
{
	if (chip == NULL)
		return 0;
#ifdef OPLUS_CHG_DEBUG_TEST
	if(chip->detect_detach_unexpeactly == 1 || break_flag == 1){
#else
	if(chip->detect_detach_unexpeactly == 1){
#endif
		oplus_chg_set_chg_flag(OPLUS_NOTIFY_CHG_WRAP_BREAK);
		strcpy(oplus_chg_debug_info.flag_reason,
					oplus_chg_debug_notify_policy[OPLUS_NOTIFY_CHG_WRAP_BREAK].reason);
#ifdef OPLUS_CHG_DEBUG_TEST
	}else if(chip->detect_detach_unexpeactly == 2 || break_flag == 2) {
#else
...
static int oplus_chg_fastchg_consume_power_check(struct oplus_chg_chip *chip)
{
	...
	input_power = oplus_chg_get_wrap_input_power(chip);
	charge_power = oplus_chg_get_charge_power(chip);
...
{
	int wrap_chg_max_input_power;
	int charge_power;

	wrap_chg_max_input_power = oplus_chg_get_wrap_chg_max_input_power(chip);
	charge_power = oplus_chg_get_charge_power(chip);
#ifdef OPLUS_CHG_DEBUG_TEST
	chg_err("wrap_chg_max_input_power = %d, charge_power = %d\n",
			wrap_chg_max_input_power, charge_power);
#endif


	if ((-1 * charge_power * 10) < (wrap_chg_max_input_power)) {
...
static int oplus_chg_input_power_limit_check(struct oplus_chg_chip *chip)
{
	...
	if (wrap_chip) {
		if (chip->charger_type == POWER_SUPPLY_TYPE_USB_DCP) {
			if ((chip->soc < wrap_chip->wrap_high_soc) && (chip->soc > wrap_chip->wrap_low_soc)) {
				if (chip->temperature >= wrap_chip->wrap_high_temp) {
					oplus_chg_set_chg_flag(OPLUS_NOTIFY_CHG_SLOW_BATT_WARM_TEMP);
				}

				if (chip->temperature <= wrap_chip->wrap_low_temp) {
			...
			switch(oplus_chg_get_real_charger_type(chip))
			{
				case POWER_SUPPLY_TYPE_USB_DCP:
					if (oplus_wrap_get_fast_chg_type() == CHARGER_SUBTYPE_FASTCHG_WRAP) {
						default_input_current = chip->limits.default_input_current_wrap_ma_normal;
					}
					else {
...
static int oplus_get_chg_time_ref(struct oplus_chg_chip *chip) {
	...
	if(chip->wrap_project == 1) {// 5V4A and 5V6A
		current_high = 5000;
		current_mid = 3500;
		current_low = 2500;
		current_lowest = 1000;
	}else if(chip->wrap_project == 2) {// 10V5A
		current_high = 3200;
		current_mid = 2250;
		current_low = 1800;
		current_lowest = 1000;
	}else if(chip->wrap_project == 3) {// 10V6.5A
		current_high = 3200;//second/1*soc
		current_mid = 2250;
		current_low = 1800;
		current_lowest = 1000;
	}else if(chip->wrap_project == 4) {//10V5A one batt
		current_high = 4500;
		current_mid = 3500;
		current_low = 2500;
		current_lowest = 1500;
	}else if(chip->wrap_project == 5) {//11V3A
		current_high = 5000;
		current_mid = 3500;
		current_low = 2500;
		current_lowest = 1000;
...
static int oplus_chg_chg_slow_check(struct oplus_chg_chip *chip)
{
	...
	if (oplus_wrap_get_fastchg_started()) {
		if (oplus_chg_debug_info.fast_chg_type == FASTCHG_CHARGER_TYPE_UNKOWN) {
			oplus_chg_debug_info.fast_chg_type = oplus_wrap_get_fast_chg_type();
	...
	if (chip->wrap_project && (oplus_wrap_get_fastchg_started() == true)) {
		...
		if (oplus_chg_get_wrap_adapter_is_low_input_power(chip)) {
			oplus_chg_set_chg_flag(OPLUS_NOTIFY_CHG_SLOW_WRAP_ADAPTER_NON_MAX_POWER);
		}
	}else if(chip->wrap_project && (oplus_wrap_get_fastchg_started() == false) &&
		oplus_chg_debug_info.chg_cnt[OPLUS_NOTIFY_FAST_CHARGER_TIME] < 3 &&
		chip->total_time > 30) {
		oplus_chg_set_chg_flag(OPLUS_NOTIFY_CHG_SLOW_WRAP_NON_START);
	}
...
void oplus_chg_wrap_mcu_error( int error ) {
	if(error){
		if (!oplus_chg_debug_notify_flag_is_set(OPLUS_NOTIFY_CHARGER_INFO)) {
			oplus_chg_set_chg_flag(OPLUS_NOTIFY_CHARGER_INFO);
			strcpy(oplus_chg_debug_info.flag_reason,
				oplus_chg_debug_notify_policy[OPLUS_NOTIFY_CHARGER_INFO].reason);
			oplus_chg_debug_info.wrap_mcu_error = error;
			oplus_chg_print_debug_info(g_debug_oplus_chip);
			send_info_flag = SEND_INFO_FLAG_IRQ;
		}
	}
...
{
	struct oplus_wrap_chip *wrap_chip = NULL;
	struct device_node *wrap_node = NULL;
	...
	wrap_node = wrap_chip->dev->of_node;
	if (!wrap_node)
		return 0;
	...
	for(i = 0; i < OPLUS_FASTCHG_OUTPUT_POWER_MAX; i++) {
		node = of_find_node_by_name(wrap_node, wrap_adpater_name[i]);
		if (!node)
			continue;
		...
		for_each_child_of_node(node, child)
		{
			...
			if (size >= 0) {
				for(k = 0; (k < (size / 3)) && (k < OPLUS_CHG_WRAP_INPUT_CURRENT_MAX_CNT); k++)
				{
