...
extern void __attribute__((weak)) tp_ftm_extra(void) { return; };
...
static void tm_panel_init(struct tianma *ctx)
{
	...
	tm_dcs_write_seq_static(ctx,0x11,0x70); //SSC
	tm_dcs_write_seq_static(ctx,0x10,0x0D); //SSC
	...
	//tm_dcs_write_seq_static(ctx,0x51,0x0F,0xFF);
	tm_dcs_write_seq_static(ctx,0x53,0x24);
	tm_dcs_write_seq_static(ctx,0x55,0x02); //11.5%
	//tm_dcs_write_seq_static(ctx,0x55,0x03); //25%
...
static struct mipi_dsi_driver jdi_driver = {
	...
	.remove = jdi_remove,
	.driver = {
			.name = "panel-oplus20001_ili7807s_fhdp_dsi_vdo_dphy",
			.owner = THIS_MODULE,
