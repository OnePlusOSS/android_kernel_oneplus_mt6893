...
static int panel_osc_freq_change(void *dsi, dcs_write_gce cb, void *handle, bool en)
{
	...
	if (en == 0) {
		osc_tb7[1] = 0x8E, osc_tb7[2] = 0x63;    //OSC 167M base MIPI 300.25M
	} else if (en == 1) {
		osc_tb7[1] = 0x90, osc_tb7[2] = 0x18;    //OSC 169M base MIPI 300.25M
	}
		...
		unsigned int dst_mode, enum MTK_PANEL_MODE_SWITCH_STAGE stage)
{
	...
	if (stage == BEFORE_DSI_POWERDOWN) {
		if (cur_mode == 0 && dst_mode == 1) { /* 60 switch to 90 */
			pr_err("[TP] ready charge to 90 hz");
...
static int lcm_panel_poweron(struct drm_panel *panel)
{
	...
	pr_err("debug for ali lcm %s, ctx->prepared=%d\n", __func__, ctx->prepared);
...
static int lcm_probe(struct mipi_dsi_device *dsi)
{
	...
	pr_info("[soso]21127 samsung probe\n");
	if (dsi_node) {
