...
#define NXPTFA_MAXLINE (256)
/* maximum buffer size to hold the container */
#define NXPTFA_MAXBUFFER (50*1024)

/*
 * buffer types for setting parameters
 */
struct nxpTfa98xxParamsType {
	tfa_patch_params,
	tfa_speaker_params,
	tfa_preset_params,
	tfa_config_params,
...
enum Tfa98xx_Error tfa98xx_verify_speaker_range(int idx,
				float imp[2],
				int spkr_count);
...
int start_timer(int time, void (*)(void));
...
enum Tfa98xx_Error tfa_send_calibration_values(void);
